...
SillyTavern is an LLM (Large Language Model) frontend for power users. It provides a web-based interface for interacting with various LLM backends (OpenAI, Claude, local models via KoboldAI/Text Generation WebUI, etc.) with extensive customization, character cards, world info, extensions, and multi-user support.
