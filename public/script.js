...
export function getSlideToggleOptions() {
    return {
        ...
        transitionFunction:
            animation_duration > 0 ? "ease-in-out" : "step-start",
    };
}
        ...
        if (entity.type === "tag") {
            ...
            if (doFilter) {
                ...
                subEntities = entitiesFilter.applyFilters(subEntities, {
                    ...
                    tempOverrides: {
                        [FILTER_TYPES.FOLDER]: FILTER_STATES.UNDEFINED,
                    },
...
export async function getOneCharacter(avatarUrl) {
    ...
    if (response.ok) {
        ...
        if (indexOf !== -1) {
            ...
            toastr.error(
                t`Character ${avatarUrl} not found in the list`,
                t`Error`,
...
export async function showMoreMessages(messagesToLoad = null) {
    ...
    while (messageId > 0 && count > 0) {
        ...
        addOneMessage(chat[newMessageId], {
            insertBefore: messageId >= chat.length ? null : messageId,
            scroll: false,
...
export function messageFormatting(
    ...
    // PromptManager bias replacement should be applied on the raw message
...
function insertSVGIcon(mes, extra) {
    ...
    if (
        extra.api === "openai" &&
        extra.model?.toLowerCase().includes("claude")
    ) {
        modelName = "claude";
    }
    // OpenAI on OpenRouter
        ...
        extra.api === "openai" &&
        extra.model?.toLowerCase().includes("openai")
    ) {
        modelName = "openai";
    }
    // OpenRouter website model or other models
        ...
        extra.api === "openai" &&
        (extra.model === null || extra.model?.toLowerCase().includes("/"))
    ) {
    ...
    const createModelImage = (className, targetSelector, insertBefore) => {
        ...
        image.title = `${extra?.api ? extra.api + " - " : ""}${
            extra?.model ?? ""
        }`;
...
export function appendMediaToMessage(mes, messageElement, adjustScroll = true) {
    ...
    if (mes.extra?.image) {
        ...
        if (Array.isArray(imageSwipes) && imageSwipes.length > 0) {
            ...
            swipeLeft.off("click").on("click", function () {
                eventSource.emit(event_types.IMAGE_SWIPED, {
                    message: mes,
    ...
    if (mes.extra?.video) {
        const container = $(
            "#message_video_template .mes_video_container"
        ).clone();
    ...
    if (mes.extra?.file) {
        ...
        const template = $(
            "#message_file_template .mes_file_container"
        ).clone();
        template.find(".mes_file_name").text(mes.extra.file.name);
        template
            .find(".mes_file_size")
            .text(humanFileSize(mes.extra.file.size));
...
export function addOneMessage(
    ...
    const sanitizerOverrides = mes.uses_system_ui
    ...
    //shows or hides the PromptManager display button
    ...
    if (
        ...
        const itemizedPrompt = itemizedPrompts.find(
            (x) => Number(x.mesId) === Number(mesIdToFind)
        );
        if (itemizedPrompt) {
...
export function substituteParams(
    ...
    if (_replaceCharacterCard) {
        ...
        environment.mesExamples = () => {
            const isInstruct =
                power_user.instruct.enabled && main_api !== "openai";
            const mesExamplesArray = parseMesExamples(
                fields.mesExamples,
                isInstruct
            );
            if (isInstruct) {
...
export function getCharacterCardFields({ chid = null } = {}) {
    ...
    result.persona = baseChatReplace(
    ...
    result.description = baseChatReplace(
        character.description?.trim(),
        name1,
        name2
    );
    result.personality = baseChatReplace(
        character.personality?.trim(),
        name1,
        name2
    );
    result.scenario = baseChatReplace(scenarioText.trim(), name1, name2);
    result.mesExamples = baseChatReplace(
        character.mes_example?.trim(),
        name1,
        name2
    );
    result.system = power_user.prefer_character_prompt
        ? baseChatReplace(character.data?.system_prompt?.trim(), name1, name2)
        : "";
    result.jailbreak = power_user.prefer_character_jailbreak
        ? baseChatReplace(
    ...
    result.charDepthPrompt = baseChatReplace(
        character.data?.extensions?.depth_prompt?.prompt?.trim(),
        name1,
        name2
    );
    result.creatorNotes = baseChatReplace(
        character.data?.creator_notes?.trim(),
        name1,
        name2
    );

    if (selected_group) {
        const groupCards = getGroupCharacterCards(
            selected_group,
            Number(currentChid)
        );
...
class StreamingProcessor {
     ...
     * @param {PromptReasoning} promptReasoning PromptManager reasoning instance
     */
    constructor(
        type,
        forceName2,
    ...
    async onStartStreaming(text) {
        ...
        if (this.type == "impersonate") {
            ...
            this.sendTextarea.dispatchEvent(
    ...
    async onProgressStreaming(messageId, text, isFinal) {
        ...
        if (isImpersonate) {
            ...
            chat[messageId]["extra"]["time_to_first_token"] =
            ...
            const tokenCountText =
                this.reasoningHandler.reasoning + processedText;
    ...
    async onFinishStreaming(messageId, text) {
        ...
        if (this.image) {
            await processImageAttachment(chat[messageId], {
                imageUrl: this.image,
            });
    ...
    async generate() {
        ...
        try {
            ...
            for await (const {
                ...
                this.reasoningHandler.updateReasoning(
                    this.messageId,
                    state?.reasoning
                );
            ...
            console.warn(
                `Stream stats: ${timestamps.length} tokens, ${seconds.toFixed(
                    2
                )} seconds, rate: ${Number(timestamps.length / seconds).toFixed(
                    2
                )} TPS`
            );
        } catch (err) {
 ...
 * @returns {string | object[]} PromptManager ready for use in generation. If using TC, this will be a string. If using CC, this will be an array of chat-style messages.
 */
export function createRawPrompt(
    ...
    if (api !== "openai") {
        ...
        prompt =
            api === "novel" ? adjustNovelInstructionPrompt(prompt) : prompt;
        prompt =
            prompt +
            (isInstruct
                ? formatInstructModePrompt(
 ...
 * @prop {string | object[]} [prompt] PromptManager to generate a message from. Can be a string or an array of chat-style messages, i.e. [{role: '', content: ''}, ...]
...
export async function Generate(
    ...
    if (
        !(dryRun || type == "regenerate" || type == "swipe" || type == "quiet")
    ) {
    ...
    if (applyStoryStringInject) {
        ...
        const role =
            power_user.context.story_string_role ??
    ...
    if (isContinue && (chat2.length > 1 || main_api === "openai")) {
        ...
        injectedIndices = injectedIndices
            .map(shiftDownByOne)
            .filter((x) => x >= 0);
    ...
    if (generatedPromptCache.length == 0 || type === "continue") {
        ...
        arrMes.forEach(function (item, i, arr) {
            ...
            if (i === arrMes.length - 1 && type !== "continue") {
                if (
                    !isInstruct ||
                    (power_user.instruct.wrap && type !== "quiet")
                ) {
                    item = item.replace(/\n?$/, "");
                }
            }
    ...
    function setPromptString() {
        ...
        console.debug("--setting PromptManager string");
        mesExmString =
    ...
    async function checkPromptSize() {
        console.debug("---checking PromptManager size");
        setPromptString();
    ...
    if (generatedPromptCache.length > 0 && main_api !== "openai") {
        console.debug(
            "---Generated PromptManager Cache length: " +
                generatedPromptCache.length
        );
        await checkPromptSize();
    } else {
        console.debug(
            "---calling setPromptString " + generatedPromptCache.length
        );
        setPromptString();
    }
    ...
    async function getCombinedPrompt(isNegative) {
        ...
        finalMesSend.forEach((item, i) => {
            item.injected = injectedIndices.includes(
                finalMesSend.length - i - 1
            );
        });
    ...
    async function onSuccess(data) {
        ...
        if (data.error) {
            ...
            if (data?.response) {
                toastr.error(data.response, t`API Error`, {
...
async function doChatInject(messages, isContinue) {
    ...
    const injectedIndices = injectedMessages.map((msg) =>
...
export function shouldAutoContinue(messageChunk, isImpersonate) {
    ...
    if (textareaText.length > 0) {
        console.log(
            "Not triggering auto-continue because user input is not empty"
        );
        return false;
    }
    ...
    if (messageChunk.trim().length > USABLE_LENGTH && chat.length) {
        ...
        console.log(
            "Last generated chunk was empty, not triggering auto-continue"
        );
        return false;
    }
}
...
export function getMaxContextSize(overrideResponseLength = null) {
    ...
    if (main_api == "novel") {
        ...
        this_max_context =
            this_max_context - (overrideResponseLength || amount_gen);
    }
    if (main_api == "openai") {
        this_max_context =
            oai_settings.openai_max_context -
            (overrideResponseLength || oai_settings.openai_max_tokens);
    }
        ...
        case "openai":
            {
                switch (
                    ...
                    case chat_completion_sources.MAKERSUITE:
                        {
                            const inlineData =
                                data?.responseContent?.parts?.find(
                                    (x) => x.inlineData
                                )?.inlineData;
                            if (inlineData) {
...
export function extractMessageFromData(data, activeApi = null) {
    function getResult() {
        ...
        switch (activeApi ?? main_api) {
            ...
            case "openai":
                return (
                    data?.content?.find((p) => p.type === "text")?.text ??
                    data?.choices?.[0]?.message?.content ??
                    data?.choices?.[0]?.text ??
                    data?.text ??
        ...
        const sequences = [
            {
                ...
                apply:
                    isImpersonate &&
                    isNotEmpty(power_user.instruct.input_sequence),
            },
            {
                value: power_user.instruct.output_sequence,
                apply:
                    !isImpersonate &&
                    isNotEmpty(power_user.instruct.output_sequence),
            },
            {
...
export async function saveReply({
    ...
    if (arguments.length > 1 && typeof arguments[0] !== "object") {
        ...
        [type, getMessage, fromStreaming, title, swipes, reasoning, imageUrl] =
            arguments;
    }
    ...
    if (
        ...
        (!chat[chat.length - 1]["extra"] ||
            typeof chat[chat.length - 1]["extra"] !== "object")
    ) {
        chat[chat.length - 1]["extra"] = {};
    }
    ...
    if (type === "swipe") {
        ...
        if (
            ...
            if (power_user.message_token_count_enabled) {
                const tokenCountText =
                    (reasoning || "") + chat[chat.length - 1]["mes"];
                chat[chat.length - 1]["extra"]["token_count"] =
                    await getTokenCountAsync(tokenCountText, 0);
            }
            const chat_id = chat.length - 1;
            !fromStreaming &&
...
export function syncMesToSwipe(messageId = null) {
    ...
    if (
        !targetMessage.swipes[targetMessage.swipe_id] ||
...
export function syncSwipeToMes(messageId = null, swipeId = null) {
    ...
    if (
        ...
        !targetMessage.swipe_info[targetSwipeId]
    ) {
...
export async function renameCharacter(
    ...
    try {
        if (response.ok) {
            ...
            const oldName = getCharaFilename(null, {
                manualAvatarKey: oldAvatar,
            });
            const newName = getCharaFilename(null, {
                manualAvatarKey: newAvatar,
            });
            ...
            const charLore = world_info.charLore?.find(
                (x) => x.name == oldName
            );
            if (charLore) {
...
export function saveChatDebounced() {
    ...
    chatSaveTimeout = setTimeout(async () => {
        if (selectedGroup !== selected_group) {
            console.warn(
                "Chat save timeout triggered, but group changed. Aborting."
            );
            return;
        }
        ...
        if (chid !== this_chid) {
            console.warn(
                "Chat save timeout triggered, but chid changed. Aborting."
            );
            return;
        }
...
export async function saveChat({
    ...
    if (!fileName) {
        console.warn(
            "saveChat called without chat_name and no chat file found"
        );
        return;
    }
    ...
    chat.forEach(function (item, i) {
        if (item["is_group"]) {
            toastr.error(
                t`Trying to save group chat with regular saveChat function. Aborting to prevent corruption.`
            );
...
async function read_avatar_load(input) {
    if (input.files && input.files[0]) {
        ...
        await fetch(
            getThumbnailUrl("avatar", formData.get("avatar_url").toString()),
            {
                method: "GET",
                cache: "reload",
            }
...
export async function getChat() {
    ...
    try {
        ...
        setTimeout(function () {
            if (
                $(document.activeElement).is("input:visible, textarea:visible")
            ) {
                return;
            }
...
async function doOnboarding(avatarId) {
    ...
    let userName = await callGenericPopup(
        ...
        { rows: 2, wider: true, cancelButton: false }
    );
...
export async function getSettings() {
    ...
    if (data.result != "file not find" && data.settings) {
        ...
        if (data.enable_extensions) {
            const enableAutoUpdate = Boolean(
                data.enable_extensions_auto_update
            );
...
function updateMessage(div) {
    ...
    let text =
        mesBlock.find(".edit_textarea").val() ??
...
async function displayChats(
    ...
    try {
        ...
        for (const chat of filteredData) {
            const isSelected =
                trimExtension(currentChat) === trimExtension(chat.file_name);
            const template = $(
                "#past_chat_template .select_chat_block_wrapper"
            ).clone();
            template
                .find(".select_chat_block")
                .attr("file_name", chat.file_name);
        ...
        if (
            ...
            try {
                ...
                waitUntilCondition(
                    ...
                    if (element.length === 0) {
                        console.log(
                            `Could not find element for character ${charId}`
                        );
                        return;
                    }
...
export function select_selected_character(chid, { switchMenu = true } = {}) {
    ...
    $("#creator_notes_spoiler").html(
        formatCreatorNotes(
            characters[chid].data?.creator_notes ||
                characters[chid].creatorcomment,
 ...
 * @param {string} key PromptManager injection id.
 * @param {string} value PromptManager injection value.
...
export function showSwipeButtons() {
    ...
    const currentMessage = chatElement
        .children()
        .filter(`[mesid="${chat.length - 1}"]`);
    ...
    if (
        swipeId !== undefined &&
        (chat[chat.length - 1].swipes.length > 1 || swipeId > 0)
    ) {
...
function updateEditArrowClasses() {
    ...
    if (this_edit_mes_id !== undefined) {
        const down = chatElement.find(
            `.mes[mesid="${this_edit_mes_id}"] .mes_edit_down`
        );
        const up = chatElement.find(
            `.mes[mesid="${this_edit_mes_id}"] .mes_edit_up`
        );
...
async function openCharacterWorldPopup() {
    ...
    world_names.forEach((item, i) => {
        ...
        extrasSelect.append(
            new Option(item, String(i), isSelected, isSelected)
        );
    });
...
export async function createOrEditCharacter(e) {
    ...
    if ($("#form_create").attr("actiontype") == "createcharacter") {
        ...
        try {
            ...
            formData.append(
                "extensions",
                JSON.stringify(create_save.extensions)
            );
            ...
            const fields = [
                ...
                },
                {
                    ...
                    callback: (value) =>
                        (create_save.depth_prompt_prompt = value),
                },
                {
                    id: "#depth_prompt_depth",
                    callback: (value) =>
                        (create_save.depth_prompt_depth = value),
                    defaultValue: depth_prompt_depth_default,
                },
                {
                    id: "#depth_prompt_role",
                    callback: (value) =>
                        (create_save.depth_prompt_role = value),
            ...
            if (
                ...
                const fileName = getCharaFilename(null, {
                    manualAvatarKey: avatarId,
                });
                const charLore = world_info.charLore ?? [];
                charLore.push({
                    name: fileName,
                    extraBooks: create_save.extra_books,
                });
...
export function swipe_left(_event, { source, repeated } = {}) {
    ...
    if (chat[chat.length - 1]["swipe_id"] >= 0) {
        ...
        chat[chat.length - 1]["extra"] = structuredClone(
            chat[chat.length - 1].swipe_info[chat[chat.length - 1]["swipe_id"]]
            ...
            .transition({
                ...
                complete: async function () {
                    ...
                    if (power_user.message_token_count_enabled) {
                        ...
                        const swipeMessage = $("#chat").find(
                            `[mesid="${chat.length - 1}"]`
                        );
                        const tokenCountText =
                            (chat[chat.length - 1]?.extra?.reasoning || "") +
                            chat[chat.length - 1].mes;
                        const tokenCount = await getTokenCountAsync(
                            tokenCountText,
                            0
                        );
                        chat[chat.length - 1]["extra"]["token_count"] =
        ...
        this_mes_div.children(".mes_block").transition({
            ...
            complete: async function () {
                const is_animation_scroll =
                    ...
                    chatElement.prop("scrollHeight") -
                        chatElement.outerHeight() -
                        10;
                //console.log(parseInt(chat[chat.length-1]['swipe_id']));
                //console.log(chat[chat.length-1]['swipes'].length);
                const swipeMessage = chatElement.find(
                    '[mesid="' + (chat.length - 1) + '"]'
                );
                if (
                    run_generate &&
                ...
                this_mes_div.children(".mes_block").transition({
                    ...
                    complete: function () {
                        this_mes_div.children(".mes_block").transition({
                            x: "0px",
                            duration:
                                animation_duration > 0 ? swipe_duration : 0,
                            easing: animation_easing,
                            queue: false,
                            complete: async function () {
                                appendMediaToMessage(
...
async function importCharacter(
    ...
    try {
        ...
        if (data.file_name !== undefined) {
            ...
            if (exists) {
                toastr.success(
                    t`Character Replaced: ${String(data.file_name).replace(
                        ".png",
                        ""
                    )}`
                );
                ...
                toastr.success(
                    t`Character Created: ${String(data.file_name).replace(
                        ".png",
                        ""
                    )}`
                );
...
function addDebugFunctions() {
    const doBackfill = async () => {
        for (const message of chat) {
            ...
            const tokenCountText =
                (message?.extra?.reasoning || "") + message.mes;
            message.extra.token_count = await getTokenCountAsync(
                tokenCountText,
                0
            );
        }
        ...
        () => {
            ...
            toastr.info(
                "Regenerate warning is now " +
                    (accountStorage.getItem("RegenerateWithCtrlEnter") ===
                    "true"
                        ? "disabled"
                        : "enabled")
            );
...
API Type: ${
                getSettingsContents[getSettingsContents.main_api + "_settings"]
                    .type
            }
...
\`\`\`
    `;
            ...
            try {
                ...
                toastr.info(
                    "Your ST API setup data has been copied to the clipboard."
                );
            } catch (error) {
...
jQuery(async function () {
    ...
    $("#confirm_btn").on("click", async function () {
        ...
        if (interruptedByCommand) {
            ...
            setTimeout(function () {
                $("#shadow_select__hidden_floor_popup").css("display", "none");
            }, animation_duration);
    ...
    $("#form_create").on("submit", (e) =>
        createOrEditCharacter(e.originalEvent)
    );
    ...
    const elementsToUpdate = {
        ...
        "#creator_notes_textarea": function () {
            create_save.creator_notes = String(
                $("#creator_notes_textarea").val()
            );
        },
        ...
        "#system_prompt_textarea": function () {
            create_save.system_prompt = String(
                $("#system_prompt_textarea").val()
            );
        },
        ...
        "#firstmessage_textarea": function () {
            create_save.first_message = String(
                $("#firstmessage_textarea").val()
            );
        },
        "#talkativeness_slider": function () {
            create_save.talkativeness = Number(
                $("#talkativeness_slider").val()
            );
        },
        "#depth_prompt_prompt": function () {
            create_save.depth_prompt_prompt = String(
                $("#depth_prompt_prompt").val()
            );
        },
        "#depth_prompt_depth": function () {
            create_save.depth_prompt_depth = Number(
                $("#depth_prompt_depth").val()
            );
        },
        "#depth_prompt_role": function () {
            create_save.depth_prompt_role = String(
                $("#depth_prompt_role").val()
            );
        },
    };
    ...
    $("#creator_notes_textarea").on("input", function () {
        ...
        const avatar =
            menu_type === "create" ? "" : characters[this_chid]?.avatar;
    ...
    $("#options [id]").on("click", async function (event, customData) {
        ...
        const additionalPrompt =
            customData?.additionalPrompt?.trim() || undefined;
        ...
        if (id == "option_toggle_Summary") {
            const identifierID = "ca2c0460-1189-4c18-a641-be58079ee849";
             //获取"神秘总结"的prompt
            const promptList = window.promptManager.prompts;
            const summaryPrompt = promptList.find(
                (prompt) => prompt.identifier === identifierID
            );
            const { name } = summaryPrompt;
            let lastEnabledEntry = null;
            //查看当前是否选中了一个世界书
            const currentWorldName = document.querySelector(
                "#world_popup_entries_list"
            );
            //查看当前世界书是否有值
            const hasChildren = currentWorldName.childNodes.length > 1;
            if (hasChildren) {
                 /****
                 * 流程一
                 * 开启神秘大总结的预设
                 */
                setToggleAction(identifierID);
                //如果包含该条目，就不新增
                 if([...currentWorldName.children].some(entry => entry.getAttribute('title') == name)){
                     //获取新增条目按钮
                    const worldPopupNew =
                        document.querySelector("#world_popup_new");
                    //模拟点击，新增条目
                    worldPopupNew.click();
                 }
                setTimeout(() => {
                    const entries = currentWorldName.children;

                    for (let i = entries.length - 1; i >= 0; i--) {
                        const entry = entries[i];
                        //找到“currentWorldName”下最后一个且没有“disabledWIEntry”类的元素
                        if (!entry.className.includes("disabledWIEntry")) {
                            lastEnabledEntry = entry;
                            lastEnabledEntry.setAttribute('title', name);
                            break;
                        }
                    }
                    //找到目标元素
                    if (lastEnabledEntry) {
                        ...
                        const entryNameInput =
                            lastEnabledEntry.querySelector(".text_pole");
                        entryNameInput.value = name;

                        //选择状态为开启
                        const entryEnabledCheckbox =
                            lastEnabledEntry.querySelector(".widthNatural");
                        entryEnabledCheckbox.value = "constant";
                        //模拟点击下拉按钮“inline-drawer-toggle”，呼出内容编辑界面
                        const drawerToggle = lastEnabledEntry.querySelector(
                            ".inline-drawer-toggle"
                        );
                        drawerToggle.click();
                        //获取内容编辑“text_pole mdHotkeys”，并赋值
                        const contentEdit = lastEnabledEntry.querySelector(
                            ".text_pole.mdHotkeys"
                        );
                        //模拟用户输入“总结”，并且发送
                        $("#send_textarea").val("总结");
                        sendTextareaMessage();
                        //设置一个监听器，监听streamingProcessor && streamingProcessor.isFinished条件成立
                        const intervalId = setInterval(() => {
                            try {
                                if (isGeneratingEnd) {
                                    clearInterval(intervalId);
                                    const generatedSummary = document.querySelector("#chat .last_mes").querySelector(".mes_text").innerText
                                    console.log(
                                        generatedSummary,
                                        "generatedSummary"
                                    );
                                    contentEdit.value = generatedSummary;
                                    setToggleAction(identifierID);
                                }
        ...
        if (id == "option_select_chat") {
            ...
            if ((selected_group || this_chid !== undefined) && !is_send_press) {
                ...
                const result = await Popup.show.confirm(
                    ...
                    await renderTemplateAsync("newChatConfirm"),
                    {
                        onClose: () => {
                            deleteCurrentChat =
                                !!$("#del_chat_checkbox").prop("checked");
                        },
                    }
                );
                if (!result) {
            ...
            setTimeout(
                () => openMessageDelete(fromSlashCommand),
                animation_duration
            );
    ...
    //神秘总结预设开关
    function setToggleAction(identifierID) {
        // 流程一开启神秘大总结的预设
        const toggleBtnList = document.querySelectorAll(
            ".completion_prompt_manager_prompt"
        );
        //神秘大总结的ID
        for (const element of toggleBtnList) {
            //获取元素的data-pm-identifier属性值
            const attributeValue = element.getAttribute("data-pm-identifier");
            if (attributeValue === identifierID) {
                //模拟用户点击该元素的子元素prompt-manager-toggle-action
                const toggleAction = element.querySelector(
    ...
    $("#main_api").on("change", async function () {
        ...
        changeMainAPI();
        saveSettingsDebounced();
        await eventSource.emit(event_types.MAIN_API_CHANGED, {
            apiId: main_api,
        });
    });
    ...
    $(document).on("click", ".mes_edit", async function () {
        ...
        if (
            ...
            const reasoningEdit = $(this)
                .closest(".mes_block")
                .find(".mes_reasoning_edit:visible");
            ...
            const textAreaElement = /** @type {HTMLTextAreaElement} */ (
                edit_textarea[0]
            );
    ...
    $(document).on("click", function (e) {
        ...
        if (
            !$(e.target).closest(".extraMesButtons, .extraMesButtonsHint")
                .length
        ) {
    ...
    $(document).on(
        ...
        async function (event, customData) {
            ...
            const canDeleteSwipe =
                Array.isArray(chat[this_edit_mes_id].swipes) &&
                chat[this_edit_mes_id].swipes.length > 1 &&
                !chat[this_edit_mes_id].is_user &&
                parseInt(this_edit_mes_id) === chat.length - 1;
    ...
    $("html").on("touchstart mousedown", async function (e) {
        ...
        const targetParentHasOpenDrawer =
    ...
    $(document).on("click", ".inline-drawer-maximize", function () {
        const icon = $(this).find(
            ".inline-drawer-icon, .floating_panel_maximize"
        );
    ...
    $(document).on("click", ".mes .avatar", function () {
        ...
        const targetAvatarImg = thumbURL.substring(
                ...
                (e) => {
                    if (e.target.closest(".dragClose")) {
                        $(`.zoomed_avatar[forChar="${charname}"]`).fadeOut(
                            ...
                            () => {
                                $(
                                    `.zoomed_avatar[forChar="${charname}"]`
                                ).remove();
                            }
                        );
                    }
    ...
    $(document).on(
        ...
        function () {
            ...
            if (
                ...
                control.select();
                console.debug(
                    "Auto-selecting content of input control",
                    control
                );
            }
        }
            ...
            case "character_source":
                {
                    ...
                    if (source && isValidUrl(source)) {
                        ...
                        toastr.info(
                            "This character doesn't seem to have a source."
                        );
                    }
                }
                break;
            case "replace_update":
                {
                    ...
                    const result = await Popup.show.confirm(
                        ...
                        `<p>${t`Choose a new character card to replace this character with.`}</p>` +
                            `<p>${t`You can also replace this character with the one from the online source.`}${
                                onlineUrl
                                    ? `<br />This character was downloaded from: <var>${onlineUrl}</var>`
                                    : ""
                            }</p>` +
            ...
            if (e.key === "Enter") {
                ...
                if (isManualInput) {
                    ...
                    if (
                        ...
                        toastr.warning(
                            `Invalid value. Must be between ${$(this).attr(
                                "min"
                            )} and ${$(this).attr("max")}`
                        );
