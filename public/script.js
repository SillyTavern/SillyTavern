...
jQuery(async function () {
    ...
    $(document).on('click', '.mes .avatar', function () {
        ...
        if ($(`.zoomed_avatar[forChar="${charname}"]`).length) {
            ...
            // izoomify binds its own touch handlers to the zoomed image when
            // magnification is enabled, so only add custom touch gestures when
            // it is off, otherwise both would react to the same touches.
    ...
    function enableZoomedAvatarTouchGestures(element) {
        ...
        // Take over touch handling so dragging/pinching the avatar does not
        // scroll or zoom the page instead.
