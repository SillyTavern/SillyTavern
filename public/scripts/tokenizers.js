...
const objectStore = localforage.createInstance({
    name: "SillyTavern_ChatCompletions",
});
...
async function loadTokenCache() {
    try {
        ...
        console.log(
            "Chat Completions: unable to load token cache, using default value",
            e,
        );
        tokenCache = {};
    }
...
async function resetTokenCache() {
    try {
        ...
        toastr.success(
            "Token cache cleared. Please reload the chat to re-tokenize it.",
        );
    } catch (e) {
...
export function getFriendlyTokenizerName(forApi) {
    ...
    if (forApi !== "openai" && tokenizerId === tokenizers.BEST_MATCH) {
        ...
        switch (tokenizerId) {
            ...
            default:
                tokenizerName = $(
                    `#tokenizer option[value="${tokenizerId}"]`,
                ).text();
                break;
        }
    }
...
export function getTokenizerBestMatch(forApi) {
    ...
    if (
        ...
        const hasValidEndpoint = sessionStorage.getItem(
...
export function getTokenizerModel() {
        ...
        const model =
            main_api == "openai"
                ...
                : openRouterModels.find(
                      (x) => x.id === textgen_settings.openrouter_model,
                  );
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.GROQ) {
        ...
        if (
            oai_settings.groq_model.includes("llama-3") ||
            oai_settings.groq_model.includes("llama3")
        ) {
            return llama3Tokenizer;
        }
        if (
    ...
    switch (tokenizerType) {
        ...
        default: {
            ...
            if (!endpointUrl) {
                apiFailureTokenCount(str);
                console.warn(
                    "This tokenizer type does not support encoding",
                    tokenizerType,
                );
                return [];
            }
...
export function decodeTextTokens(tokenizerType, ids) {
    ...
    if (!endpointUrl) {
        console.warn(
            "This tokenizer type does not support decoding",
            tokenizerType,
        );
        return { text: "", chunks: [] };
    }
