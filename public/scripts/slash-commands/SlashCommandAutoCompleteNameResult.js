...
export class SlashCommandAutoCompleteNameResult extends AutoCompleteNameResult {
    ...
    constructor(executor, scope, commands) {
        super(
            ...
            Object.keys(commands).map(
                (key) =>
                    new SlashCommandCommandAutoCompleteOption(
                        commands[key],
                        key,
                    ),
            ),
    ...
    getSecondaryNameAt(text, index, isSelect) {
        ...
        if (
            !namedResult ||
            namedResult.optionList.length == 0 ||
            !namedResult.isRequired
        ) {
            const unnamedResult = this.getUnnamedArgumentAt(
                text,
                index,
                isSelect,
            );
            ...
            if (namedResult && unnamedResult) {
                ...
                combinedResult.isRequired =
                    namedResult.isRequired || unnamedResult.isRequired;
                combinedResult.forceMatch =
                    namedResult.forceMatch && unnamedResult.forceMatch;
    ...
    getNamedArgumentAt(text, index, isSelect) {
        ...
        const notProvidedNamedArguments =
            this.executor.command.namedArgumentList.filter(
                (arg) =>
                    !this.executor.namedArgumentList.find(
                        (it) => it.name == arg.name,
                    ),
            );
        let name;
        ...
        let cmdArg;
        let argAssign;
        const unamedArgLength =
            this.executor.endUnnamedArgs - this.executor.startUnnamedArgs;
        const namedArgsFollowedBySpace =
            text[this.executor.endNamedArgs] == " ";
        if (
            this.executor.startNamedArgs <= index &&
            this.executor.endNamedArgs + (namedArgsFollowedBySpace ? 1 : 0) >=
                index
        ) {
            ...
            if (argAssign) {
                const [argName, ...v] = text
                    .slice(argAssign.start, index)
                    .split(getSplitRegex());
                name = argName;
                value = v.join("");
            ...
            if (
                typeof this.executor.unnamedArgumentList[0]?.value == "string"
            ) {
                if (
                    index <=
        ...
        if (name.includes("=") && cmdArg) {
            ...
            const enumList =
                cmdArg?.enumProvider?.(this.executor, this.scope) ??
                cmdArg?.enumList;
            if (cmdArg && enumList?.length) {
                ...
                const result = new AutoCompleteSecondaryNameResult(
                    ...
                    start + name.length,
                    enumList.map((it) =>
                        SlashCommandEnumAutoCompleteOption.from(
        ...
        if (notProvidedNamedArguments.length > 0) {
            const result = new AutoCompleteSecondaryNameResult(
                ...
                notProvidedNamedArguments.map(
                    (it) =>
                        new SlashCommandNamedArgumentAutoCompleteOption(
                            it,
                            this.executor.command,
                        ),
                ),
            ...
            result.isRequired =
                notProvidedNamedArguments.find((it) => it.isRequired) != null;
            return result;
        }
    ...
    getUnnamedArgumentAt(text, index, isSelect) {
        ...
        const lastArgIsBlank =
            this.executor.unnamedArgumentList.slice(-1)[0]?.value == "";
        const notProvidedArguments =
            this.executor.command.unnamedArgumentList.slice(
                this.executor.unnamedArgumentList.length -
                    (lastArgIsBlank ? 1 : 0),
            );
        let value;
        let start;
        ...
        let argAssign;
        if (
            this.executor.startUnnamedArgs <= index &&
            this.executor.endUnnamedArgs + 1 >= index
        ) {
            ...
            if (idx > -1) {
                ...
                if (
                    cmdArg === undefined &&
                    this.executor.command.unnamedArgumentList.slice(-1)[0]
                        ?.acceptsMultiple
                ) {
                    cmdArg =
                        this.executor.command.unnamedArgumentList.slice(-1)[0];
                ...
                if (cmdArg && enumList.length > 0) {
                    value = argAssign.value
                        .toString()
                        .slice(0, index - argAssign.start);
                    start = argAssign.start;
                } else {
                    return null;
                }
