...
export class SlashCommandBrowser {
    ...
    renderInto(parent) {
                        ...
                        const inp = document.createElement("input");
                        {
                            ...
                            inp.addEventListener("input", () => {
                                ...
                                for (const cmd of this.cmdList) {
                                    ...
                                    if (
                                        fuzzyList.length + quotedList.length ===
                                            0 ||
                                        find()
                                    ) {
                                        this.itemMap[cmd.name].classList.remove(
                                            "isFiltered",
                                        );
                                    } else {
                                        this.itemMap[cmd.name].classList.add(
                                            "isFiltered",
                                        );
                                    }
                                }
                ...
                const container = document.createElement("div");
                {
                    ...
                    const list = document.createElement("div");
                    {
                        ...
                        this.cmdList = Object.keys(SlashCommandParser.commands)
                            .filter(
                                (key) =>
                                    SlashCommandParser.commands[key].name ===
                                    key,
                            ) // exclude aliases
                            .sort((a, b) =>
                        ...
                        for (const cmd of this.cmdList) {
                            ...
                            item.addEventListener("click", () => {
                                ...
                                if (this.details !== details) {
                                    ...
                                    const rect =
                                        item.children[0].getBoundingClientRect();
        ...
        this.mo.observe(document.querySelector("#chat"), {
            childList: true,
            subtree: true,
        });
    ...
    handleKeyDown(evt) {
        if (
            ...
            if (this.dom.closest(".mes") && !this.dom.closest(".last_mes"))
                return;
            evt.preventDefault();
            evt.stopPropagation();
