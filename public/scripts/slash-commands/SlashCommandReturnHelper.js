...
export const slashCommandReturnHelper = {
    ...
    async doReturn(
        ...
        value,
        {
            objectToStringFunc = (o) => o?.toString(),
            objectToHtmlFunc = null,
        } = {},
    ) {
        const shouldHtml = type.endsWith("html");
        const actualConverterFunc =
            shouldHtml && objectToHtmlFunc
                ? objectToHtmlFunc
                : objectToStringFunc;
        const stringValue =
            typeof value !== "string" ? actualConverterFunc(value) : value;
        ...
        switch (type) {
            ...
            case "toast-html": {
                const htmlOrNotHtml = shouldHtml
                    ? DOMPurify.sanitize(
                          new showdown.Converter().makeHtml(stringValue),
                      )
                ...
                if (type.startsWith("toast"))
                    toastr.info(htmlOrNotHtml, null, {
                        escapeHtml: !shouldHtml,
                    });
