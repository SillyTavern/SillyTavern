...
async function populateChatHistory(
    ...
    if (type === "continue" && cyclePrompt && !oai_settings.continue_prefill) {
        const promptObject = {
            ...
            role: "system",
            content: substituteParamsExtended(
...
async function populateDialogueExamples(
    ...
    if (Array.isArray(messageExamples) && messageExamples.length) {
        ...
        for (const dialogue of [...messageExamples]) {
            ...
            if (
                !chatCompletion.canAffordAll([newExampleChat, ...chatMessages])
            ) {
                break;
            }
...
async function populateChatCompletion(
    ...
    const addToChatCompletion = async (source, target = null) => {
        ...
        if (
            ...
            promptManager.log(
                `Skipping prompt ${source} because it is disabled`,
            );
            return;
        }
    ...
    if (ToolManager.canPerformToolCalls(type)) {
        ...
        const toolMessage = [
            { role: "user", content: JSON.stringify(toolData) },
        ];
    ...
    systemPrompts.forEach((prompt) => {
        ...
        if (collectionPrompt) {
            ...
            prompt.injection_position =
                collectionPrompt.injection_position ??
                prompt.injection_position;
            // Depth for In-Chat
...
function checkModerationError(data, { quiet = false } = {}) {
    const moderationError = data?.error?.message?.includes(
        "requires moderation",
    );
...
function saveModelList(data) {
    ...
    if (
        ...
        model_list = openRouterSortBy(
            model_list,
            oai_settings.openrouter_sort_models,
        );

        $("#model_openrouter_select").empty();

        if (true === oai_settings.openrouter_group_models) {
            appendOpenRouterOptions(
                openRouterGroupByVendor(model_list),
                oai_settings.openrouter_group_models,
            );
        } else {
    ...
    if (
        oai_settings.chat_completion_source == chat_completion_sources.ZEROONEAI
    ) {
        ...
        $("#model_01ai_select")
            .val(oai_settings.zerooneai_model)
            .trigger("change");
    }
    ...
    if (
        ...
        const mistralModelSelect = document.querySelector(
            "#model_mistralai_select",
        );
        if (mistralModelSelect) {
            const options = Array.from(mistralModelSelect.options);
            options.forEach((option) => {
                const existingModel = model_list.find(
            ...
            const otherOptionsGroup = mistralModelSelect.querySelector(
                "#mistralai_other_models",
            );
            for (const model of model_list.filter(
                (model) => model?.capabilities?.completion_chat,
            )) {
                if (
                    !options.some((option) => option.value === model.id) &&
                    otherOptionsGroup
                ) {
...
function appendOpenRouterOptions(
    model_list,
    groupModels = false,
    sort = false,
) {
...
async function sendAltScaleRequest(messages, logit_bias, signal, type) {
    ...
    for (let msg of messages) {
        if (msg.role === "system") {
            firstSysMsgs.push(
                substituteParams(
                    msg.name ? msg.name + ": " + msg.content : msg.content,
                ),
            );
        } else {
            break;
...
async function sendOpenAIRequest(type, messages, signal) {
    ...
    const isTextCompletion =
        isOAI && textCompletionModels.includes(oai_settings.openai_model);
    ...
    const stream =
        ...
        !isQuiet &&
        !isScale &&
        !(isOAI && ["o1-2024-12-17", "o1"].includes(oai_settings.openai_model));
    ...
    if (
        (isOAI && isVision(oai_settings.openai_model)) ||
        (isOpenRouter && isVision(oai_settings.openrouter_model))
    ) {
    ...
    if (
        (isOAI && oai_settings.openai_model.includes("gpt-4.5")) ||
        (isOpenRouter && oai_settings.openrouter_model.includes("gpt-4.5"))
    ) {
        delete generate_data.logprobs;
    }
    ...
    if (isClaude) {
        ...
        generate_data["claude_use_sysprompt"] =
            oai_settings.claude_use_sysprompt;
        ...
        if (!isQuiet && !(isContinue && oai_settings.continue_prefill)) {
            generate_data["assistant_prefill"] = isImpersonate
                ? substituteParams(oai_settings.assistant_impersonation)
                : substituteParams(oai_settings.assistant_prefill);
        }
    }
    ...
    if (isGoogle || isVertexAI) {
        ...
        generate_data["use_makersuite_sysprompt"] =
            oai_settings.use_makersuite_sysprompt;
        if (isVertexAI) {
            generate_data["vertexai_auth_mode"] =
                oai_settings.vertexai_auth_mode;
    ...
    if (isCohere) {
        ...
        generate_data["top_p"] = Math.min(
            Math.max(Number(oai_settings.top_p_openai), 0.01),
            0.99,
        );
        generate_data["top_k"] = Number(oai_settings.top_k_openai);
        // Clamp to 0 -> 1
        generate_data["frequency_penalty"] = Math.min(
            Math.max(Number(oai_settings.freq_pen_openai), 0),
            1,
        );
        generate_data["presence_penalty"] = Math.min(
            Math.max(Number(oai_settings.pres_pen_openai), 0),
            1,
        );
    ...
    if (isPerplexity) {
        ...
        generate_data["frequency_penalty"] =
            Math.max(0, Number(oai_settings.freq_pen_openai)) + 1;
        generate_data["presence_penalty"] = Number(
        ...
        return async function* streamData() {
            ...
            while (true) {
                ...
                if (
                    ...
                    swipes[swipeIndex] =
                        (swipes[swipeIndex] || "") +
                        getStreamingReply(parsed, state, {
...
export function getStreamingReply(
    ...
    if (chat_completion_source === chat_completion_sources.CLAUDE) {
        ...
        if (show_thoughts) {
            state.reasoning +=
                data.choices?.filter((x) => x?.delta?.reasoning_content)?.[0]
...
function parseOpenAITextLogprobs(logprobs) {
    ...
    return tokens.map((token, i) => {
        ...
        // convert to a list of [token, logprob] pairs
        /** @type {any[]} */
        const topLogprobs = top_logprobs[i]
            ? Object.entries(top_logprobs[i])
            : [];
        const chosenTopToken = topLogprobs.some(
            ([topToken]) => token === topToken,
        );
        if (!chosenTopToken) {
...
class Message {
    ...
    async getImageTokenCost(dataUrl, quality) {
        ...
        const squares =
            Math.ceil(finalWidth / 512) * Math.ceil(finalHeight / 512);
...
class MessageCollection {
    ...
    constructor(identifier, ...items) {
        for (let item of items) {
            if (
                !(item instanceof Message || item instanceof MessageCollection)
            ) {
                throw new Error(
        ...
        return this.collection.reduce((acc, message) => {
            if (message.content || message.tool_calls) {
                acc.push({
                    ...
                    ...(message.role === "tool" && {
                        tool_call_id: message.identifier,
                    }),
                });
            }
            return acc;
...
export class ChatCompletion {
    ...
    insert(message, identifier, position = "end") {
        ...
        if (message.content || message.tool_calls) {
            if ("start" === position)
                ...
                this.messages.collection[index].collection.splice(
                    position,
                    0,
                    message,
                );
    ...
    canAffordAll(messages) {
        return (
            ...
            this.tokenBudget -
                messages.reduce(
                    (total, message) => total + message.getTokens(),
                    0,
                )
        );
    }
    ...
    validateMessageCollection(collection) {
        if (!(collection instanceof MessageCollection)) {
            ...
            throw new Error(
                "Argument must be an instance of MessageCollection",
            );
        }
    }
    ...
    reserveBudget(message) {
        const tokens =
            typeof message === "number" ? message : message.getTokens();
...
function loadOpenAISettings(data, settings) {
    ...
    oai_settings.top_a_openai =
        settings.top_a_openai ?? default_settings.top_a_openai;
    oai_settings.min_p_openai =
        settings.min_p_openai ?? default_settings.min_p_openai;
    oai_settings.repetition_penalty_openai =
        settings.repetition_penalty_openai ??
        default_settings.repetition_penalty_openai;
    oai_settings.stream_openai =
    ...
    oai_settings.scenario_format =
        settings.scenario_format ?? default_settings.scenario_format;
    oai_settings.personality_format =
        settings.personality_format ?? default_settings.personality_format;
    oai_settings.group_nudge_prompt =
        settings.group_nudge_prompt ?? default_settings.group_nudge_prompt;
    ...
    oai_settings.openrouter_group_models =
        settings.openrouter_group_models ??
        default_settings.openrouter_group_models;
    oai_settings.openrouter_sort_models =
        settings.openrouter_sort_models ??
        default_settings.openrouter_sort_models;
    oai_settings.openrouter_use_fallback =
        settings.openrouter_use_fallback ??
        default_settings.openrouter_use_fallback;
    oai_settings.openrouter_allow_fallbacks =
        settings.openrouter_allow_fallbacks ??
        default_settings.openrouter_allow_fallbacks;
    oai_settings.openrouter_middleout =
        settings.openrouter_middleout ?? default_settings.openrouter_middleout;
    oai_settings.ai21_model =
        settings.ai21_model ?? default_settings.ai21_model;
    oai_settings.mistralai_model =
        settings.mistralai_model ?? default_settings.mistralai_model;
    oai_settings.cohere_model =
        settings.cohere_model ?? default_settings.cohere_model;
    oai_settings.perplexity_model =
        settings.perplexity_model ?? default_settings.perplexity_model;
    oai_settings.groq_model =
        settings.groq_model ?? default_settings.groq_model;
    oai_settings.nanogpt_model =
        settings.nanogpt_model ?? default_settings.nanogpt_model;
    oai_settings.deepseek_model =
        settings.deepseek_model ?? default_settings.deepseek_model;
    oai_settings.zerooneai_model =
        settings.zerooneai_model ?? default_settings.zerooneai_model;
    oai_settings.xai_model = settings.xai_model ?? default_settings.xai_model;
    oai_settings.pollinations_model =
        settings.pollinations_model ?? default_settings.pollinations_model;
    oai_settings.custom_model =
        settings.custom_model ?? default_settings.custom_model;
    oai_settings.custom_url =
        settings.custom_url ?? default_settings.custom_url;
    oai_settings.custom_include_body =
        settings.custom_include_body ?? default_settings.custom_include_body;
    oai_settings.custom_exclude_body =
        settings.custom_exclude_body ?? default_settings.custom_exclude_body;
    oai_settings.custom_include_headers =
        settings.custom_include_headers ??
        default_settings.custom_include_headers;
    oai_settings.custom_prompt_post_processing =
        settings.custom_prompt_post_processing ??
        default_settings.custom_prompt_post_processing;
    oai_settings.google_model =
        settings.google_model ?? default_settings.google_model;
    oai_settings.vertexai_model =
        settings.vertexai_model ?? default_settings.vertexai_model;
    oai_settings.chat_completion_source =
    ...
    oai_settings.assistant_impersonation =
        settings.assistant_impersonation ??
        default_settings.assistant_impersonation;
    oai_settings.image_inlining =
        settings.image_inlining ?? default_settings.image_inlining;
    oai_settings.inline_image_quality =
        settings.inline_image_quality ?? default_settings.inline_image_quality;
    oai_settings.bypass_status_check =
        settings.bypass_status_check ?? default_settings.bypass_status_check;
    oai_settings.show_thoughts =
        settings.show_thoughts ?? default_settings.show_thoughts;
    oai_settings.reasoning_effort =
        settings.reasoning_effort ?? default_settings.reasoning_effort;
    oai_settings.enable_web_search =
        settings.enable_web_search ?? default_settings.enable_web_search;
    oai_settings.request_images =
        settings.request_images ?? default_settings.request_images;
    ...
    oai_settings.prompt_order =
        settings.prompt_order ?? default_settings.prompt_order;

    oai_settings.new_chat_prompt =
        settings.new_chat_prompt ?? default_settings.new_chat_prompt;
    oai_settings.new_group_chat_prompt =
        settings.new_group_chat_prompt ??
        default_settings.new_group_chat_prompt;
    oai_settings.new_example_chat_prompt =
        settings.new_example_chat_prompt ??
        default_settings.new_example_chat_prompt;
    oai_settings.continue_nudge_prompt =
        settings.continue_nudge_prompt ??
        default_settings.continue_nudge_prompt;
    oai_settings.squash_system_messages =
        settings.squash_system_messages ??
        default_settings.squash_system_messages;
    oai_settings.continue_prefill =
        settings.continue_prefill ?? default_settings.continue_prefill;
    oai_settings.names_behavior =
        settings.names_behavior ?? default_settings.names_behavior;
    oai_settings.continue_postfix =
        settings.continue_postfix ?? default_settings.continue_postfix;
    oai_settings.function_calling =
        settings.function_calling ?? default_settings.function_calling;
    oai_settings.openrouter_providers =
        settings.openrouter_providers ?? default_settings.openrouter_providers;
    oai_settings.bind_preset_to_connection =
        settings.bind_preset_to_connection ??
        default_settings.bind_preset_to_connection;
    ...
    if (settings.claude_use_sysprompt !== undefined)
        oai_settings.claude_use_sysprompt = !!settings.claude_use_sysprompt;
    if (settings.use_makersuite_sysprompt !== undefined)
        oai_settings.use_makersuite_sysprompt =
            !!settings.use_makersuite_sysprompt;
    if (settings.vertexai_auth_mode !== undefined)
        oai_settings.vertexai_auth_mode = settings.vertexai_auth_mode;
    if (settings.vertexai_region !== undefined)
        oai_settings.vertexai_region = settings.vertexai_region;
    if (settings.use_alt_scale !== undefined) {
        oai_settings.use_alt_scale = !!settings.use_alt_scale;
        updateScaleForm();
    }
    ...
    $("#claude_assistant_impersonation").val(
        oai_settings.assistant_impersonation,
    );
    $("#openai_image_inlining").prop("checked", oai_settings.image_inlining);
    $("#openai_bypass_status_check").prop(
        "checked",
        oai_settings.bypass_status_check,
    );
    ...
    $(
        `#openai_inline_image_quality option[value="${oai_settings.inline_image_quality}"]`,
    ).prop("selected", true);

    $("#model_openai_select").val(oai_settings.openai_model);
    $(`#model_openai_select option[value="${oai_settings.openai_model}"`).prop(
        "selected",
        true,
    );
    $("#model_claude_select").val(oai_settings.claude_model);
    $(`#model_claude_select option[value="${oai_settings.claude_model}"`).prop(
        "selected",
        true,
    );
    ...
    $(`#model_google_select option[value="${oai_settings.google_model}"`).prop(
        "selected",
        true,
    );
    $("#model_vertexai_select").val(oai_settings.vertexai_model);
    $(
        `#model_vertexai_select option[value="${oai_settings.vertexai_model}"`,
    ).prop("selected", true);
    $("#model_ai21_select").val(oai_settings.ai21_model);
    $(`#model_ai21_select option[value="${oai_settings.ai21_model}"`).prop(
        "selected",
        true,
    );
    ...
    $(`#model_cohere_select option[value="${oai_settings.cohere_model}"`).prop(
        "selected",
        true,
    );
    $("#model_perplexity_select").val(oai_settings.perplexity_model);
    $(
        `#model_perplexity_select option[value="${oai_settings.perplexity_model}"`,
    ).prop("selected", true);
    $("#model_groq_select").val(oai_settings.groq_model);
    $(`#model_groq_select option[value="${oai_settings.groq_model}"`).prop(
        "selected",
        true,
    );
    $("#model_nanogpt_select").val(oai_settings.nanogpt_model);
    $(
        `#model_nanogpt_select option[value="${oai_settings.nanogpt_model}"`,
    ).prop("selected", true);
    $("#model_deepseek_select").val(oai_settings.deepseek_model);
    $(
        `#model_deepseek_select option[value="${oai_settings.deepseek_model}"`,
    ).prop("selected", true);
    $("#model_01ai_select").val(oai_settings.zerooneai_model);
    $("#model_xai_select").val(oai_settings.xai_model);
    $(`#model_xai_select option[value="${oai_settings.xai_model}"`).prop(
        "selected",
        true,
    );
    ...
    $("#claude_use_sysprompt").prop(
        "checked",
        oai_settings.claude_use_sysprompt,
    );
    $("#use_makersuite_sysprompt").prop(
        "checked",
        oai_settings.use_makersuite_sysprompt,
    );
    ...
    $("#openrouter_use_fallback").prop(
        "checked",
        oai_settings.openrouter_use_fallback,
    );
    $("#openrouter_group_models").prop(
        "checked",
        oai_settings.openrouter_group_models,
    );
    $("#openrouter_allow_fallbacks").prop(
        "checked",
        oai_settings.openrouter_allow_fallbacks,
    );
    $("#openrouter_providers_chat")
        .val(oai_settings.openrouter_providers)
        .trigger("change");
    $("#openrouter_middleout").val(oai_settings.openrouter_middleout);
    $("#squash_system_messages").prop(
        "checked",
        oai_settings.squash_system_messages,
    );
    $("#continue_prefill").prop("checked", oai_settings.continue_prefill);
    $("#openai_function_calling").prop(
        "checked",
        oai_settings.function_calling,
    );
    ...
    $(
        `#openai_reasoning_effort option[value="${oai_settings.reasoning_effort}"]`,
    ).prop("selected", true);
    ...
    if (
        oai_settings.custom_prompt_post_processing ===
        custom_prompt_post_processing_types.CLAUDE
    ) {
        oai_settings.custom_prompt_post_processing =
            custom_prompt_post_processing_types.MERGE;
    }
    ...
    $("#custom_prompt_post_processing").val(
        oai_settings.custom_prompt_post_processing,
    );
    $(
        `#custom_prompt_post_processing option[value="${oai_settings.custom_prompt_post_processing}"]`,
    ).prop("selected", true);
}
...
function showWindowExtensionError() {
    toastr.error(
        t`Get it here:` +
            ' <a href="https://windowai.io/" target="_blank">windowai.io</a>',
        t`Extension is not installed`,
        {
            escapeHtml: false,
            timeOut: 0,
            extendedTimeOut: 0,
...
async function saveOpenAIPreset(name, settings, triggerUi = true) {
    ...
    const savePresetSettings = await fetch(
        `/api/presets/save-openai?name=${encodeURIComponent(name)}`,
        {
            method: "POST",
            headers: getRequestHeaders(),
...
function onLogitBiasPresetChange() {
    const value = String(
        $("#openai_logit_bias_preset").find(":selected").val(),
    );
...
async function onExportPresetClick() {
    ...
    const exportConnectionTemplate = $(
        await renderTemplateAsync("exportPreset"),
    );
...
function onSettingsPresetChange() {
    ...
    if (
        preset.names_in_completion === true &&
        preset.names_behavior === undefined
    ) {
    ...
    if (
        preset.assistant_prefill !== undefined &&
        preset.assistant_impersonation === undefined
    ) {
...
function getMaxContextOpenAI(value) {
    if (oai_settings.max_context_unlocked) {
        ...
        value.includes("chatgpt-4o-latest") ||
        value.includes("gpt-4-turbo") ||
        value.includes("gpt-4o") ||
        value.includes("gpt-4-1106") ||
        value.includes("gpt-4-0125") ||
        value.includes("gpt-4-vision")
    ) {
        return max_128k;
...
async function onModelChange() {
    ...
    if (
        ...
        if (oai_settings.max_context_unlocked) {
            ...
            value.includes("gemini-1.5-flash") ||
            value.includes("gemini-2.0-flash") ||
            value.includes("gemini-2.0-pro") ||
            value.includes("gemini-exp") ||
            value.includes("gemini-2.5-flash") ||
            value.includes("gemini-2.5-pro") ||
            value.includes("learnlm-2.0-flash")
        ) {
            ...
            value.includes("learnlm-1.5-pro-experimental")
        ) {
            $("#openai_max_context").attr("max", max_32k);
        } else {
            $("#openai_max_context").attr("max", max_32k);
        }
        let makersuite_max_temp =
    ...
    if (oai_settings.chat_completion_source == chat_completion_sources.CLAUDE) {
        if (oai_settings.max_context_unlocked) {
            ...
            value == "claude-2.1" ||
            value.startsWith("claude-3") ||
            value.startsWith("claude-opus") ||
            value.startsWith("claude-sonnet")
        ) {
            $("#openai_max_context").attr("max", max_200k);
        } else if (
            value.endsWith("100k") ||
            value.startsWith("claude-2") ||
    ...
    if (
        ...
        if (oai_settings.max_context_unlocked) {
            ...
            ["c4ai-aya-23", "c4ai-aya-expanse-32b", "command-nightly"].includes(
                oai_settings.cohere_model,
            )
        ) {
            $("#openai_max_context").attr("max", max_128k);
    ...
    if (
        ...
        if (oai_settings.max_context_unlocked) {
            ...
            [
                ...
                "sonar-reasoning-pro",
                "r1-1776",
            ].includes(oai_settings.perplexity_model)
        ) {
    ...
    if (
        ...
        if (oai_settings.max_context_unlocked) {
            ...
            ["deepseek-reasoner", "deepseek-chat"].includes(
                oai_settings.deepseek_model,
            )
        ) {
...
async function onConnectButtonClick(e) {
    ...
    if (
        ...
        const api_key_openrouter = String(
            $("#api_key_openrouter").val(),
        ).trim();
    ...
    if (
        ...
        const api_key_makersuite = String(
            $("#api_key_makersuite").val(),
        ).trim();
    ...
    if (
        ...
        if (oai_settings.vertexai_auth_mode === "express") {
            ...
            const api_key_vertexai = String(
                $("#api_key_vertexai").val(),
            ).trim();
            ...
            if (!secret_state[SECRET_KEYS.VERTEXAI_SERVICE_ACCOUNT]) {
                toastr.error(
                    "Service Account JSON is required for Vertex AI full version. Please validate and save your Service Account JSON.",
                );
                return;
            }
        }
    ...
    if (
        ...
        const api_key_perplexity = String(
            $("#api_key_perplexity").val(),
        ).trim();
...
export function isImageInliningSupported() {
    ...
    switch (oai_settings.chat_completion_source) {
            ...
            return visionSupportedModels.some(
                (model) =>
                    ...
                    ["gpt-4-turbo-preview", "o1-mini", "o3-mini"].some(
                        (x) => !oai_settings.openai_model.includes(x),
                    ),
            );
        case chat_completion_sources.MAKERSUITE:
        ...
        case chat_completion_sources.ZEROONEAI:
            return visionSupportedModels.some((model) =>
                oai_settings.zerooneai_model.includes(model),
            );
        case chat_completion_sources.MISTRALAI:
            return visionSupportedModels.some((model) =>
                oai_settings.mistralai_model.includes(model),
            );
        case chat_completion_sources.COHERE:
            return visionSupportedModels.some((model) =>
        ...
        case chat_completion_sources.POLLINATIONS:
            return (
                ...
                model_list.find((m) => m.id === oai_settings.pollinations_model)
                    ?.vision
            );
        default:
            return false;
...
async function onVertexAIValidateServiceAccount() {
    ...
    try {
        ...
        if (serviceAccount.type !== "service_account") {
            toastr.error(
                t`Invalid service account type. Expected "service_account"`,
            );
            updateVertexAIServiceAccountStatus(
                false,
...
export function initOpenAI() {
    ...
    $("#claude_use_sysprompt").on("change", function () {
        oai_settings.claude_use_sysprompt = !!$("#claude_use_sysprompt").prop(
            "checked",
        );
        saveSettingsDebounced();
    });
    ...
    $("#newchat_prompt_textarea").on("input", function () {
        oai_settings.new_chat_prompt = String(
            $("#newchat_prompt_textarea").val(),
        );
        saveSettingsDebounced();
    });
    ...
    $("#scenario_format_textarea").on("input", function () {
        oai_settings.scenario_format = String(
            $("#scenario_format_textarea").val(),
        );
        saveSettingsDebounced();
    });
    ...
    $("#customize_additional_parameters").on(
        "click",
