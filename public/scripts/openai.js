...
function setOpenAIMessages(chat) {
    ...
    for (let i = chat.length - 1; i >= 0; i--) {
        ...
        const signature = originApi === currentApi && originModel === currentModel ? chat[j]?.extra?.reasoning_signature : null;
...
async function populateChatHistory(messages, prompts, chatCompletion, type = null, cyclePrompt = null) {
    ...
    const includeReasoningSignature = isReasoningSignaturesSupported();
    ...
    for (let index = 0; index < chatPool.length; index++) {
        ...
        if (includeReasoningSignature && chatPrompt.signature) {
 ...
 */
export function isReasoningSignaturesSupported(settings = oai_settings) {
