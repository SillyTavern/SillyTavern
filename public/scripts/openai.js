...
async function prepareOpenAIMessages(
    ...
    if (oai_settings.jailbreak_system && oai_settings.jailbreak_prompt) {
        const jailbreakMessage = {
            role: "system",
            content: substituteParams(oai_settings.jailbreak_prompt),
        };
...
function loadOpenAISettings(data, settings) {
    ...
    oai_settings.use_window_ai =
        settings.use_window_ai ?? default_settings.use_window_ai;
    ...
    $(`#model_openai_select option[value="${oai_settings.openai_model}"`)
        .attr("selected", true)
        .trigger("change");
...
async function showApiKeyUsage() {
    try {
        ...
        if (response.ok) {
            const data = await response.json();
            const text = `<h3>Total usage this month: $${Number(
...
function onModelChange() {
    ...
    if (oai_settings.max_context_unlocked) {
    ...
    } else if (value == "gpt-3.5-turbo-16k" || value == "gpt-3.5-turbo-16k") {
        ...
        oai_settings.openai_max_context = Math.max(
            oai_settings.openai_max_context,
            gpt3_max
        );
        $("#openai_max_context")
