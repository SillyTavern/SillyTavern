...
export async function loadFeatherlessModels(data) {
    ...
    const modelCardBlock = document.getElementById(
        "featherless_model_card_block",
    );
    ...
    function setupPagination(
        models,
        perPage,
        pageNumber = featherlessCurrentPage,
    ) {
        paginationContainer.pagination({
            ...
            callback: function (modelsOnPage, pagination) {
                ...
                modelsOnPage.forEach((model) => {
                    ...
                    modelClassDiv.textContent =
                        t`Class` + `: ${model.model_class || "N/A"}`;

                    const contextLengthDiv = document.createElement("div");
                    contextLengthDiv.classList.add("model-context-length");
                    contextLengthDiv.textContent =
                        t`Context Length` + `: ${model.context_length}`;
            ...
            afterSizeSelectorChange: function (e) {
                ...
                ); // Use the stored current page number
            },
        });
    }
    ...
    function populateClassSelection(models) {
        const uniqueClasses = [
            ...new Set(
                models.map((model) => model.model_class).filter(Boolean),
            ),
    ...
    async function applyFiltersAndSort() {
        ...
        let filteredModels = originalModels.filter((model) => {
            ...
            const matchesClass = selectedClass
                ? model.model_class === selectedClass
                : true;
        ...
        featherlessCurrentPage =
            currentModelIndex >= 0 ? currentModelIndex / perPage + 1 : 1;
...
function getMancerModelTemplate(option) {
    ...
    const creditsPerPrompt =
        (model.limits?.context - model.limits?.completion) *
        model.pricing?.prompt;
    const creditsPerCompletion =
        model.limits?.completion * model.pricing?.completion;
    const creditsTotal = Math.round(
        creditsPerPrompt + creditsPerCompletion,
    ).toFixed(0);
...
async function downloadOllamaModel() {
    try {
        ...
        const name = await callPopup(html, "input", "", {
            okButton: "Download",
        });
        ...
        toastr.info(
            "Download may take a while, please wait...",
            "Working on it",
        );

        const response = await fetch(
            "/api/backends/text-completions/ollama/download",
            {
                method: "POST",
        ...
        toastr.success(
            "Download complete. Please select the model from the dropdown.",
        );
        $("#api_button_textgenerationwebui").trigger("click");
    } catch (err) {
        console.error(err);
...
async function downloadTabbyModel() {
    try {
        ...
        const popupResult = await callGenericPopup(
            ...
            POPUP_TYPE.CONFIRM,
            "",
            { okButton: "Download", cancelButton: "Cancel" },
        );
        ...
        const repoId = downloadHtml
            .find('input[name="hf_repo_id"]')
            .val()
            .toString();
        if (!repoId) {
            toastr.error(
                "A HuggingFace repo ID must be provided. Skipping Download.",
            );
            return;
        }
        ...
        const params = {
            repo_id: repoId,
            folder_name:
                downloadHtml.find('input[name="folder_name"]').val() ||
                undefined,
            revision:
                downloadHtml.find('input[name="revision"]').val() || undefined,
            token:
                downloadHtml.find('input[name="hf_token"]').val() || undefined,
        };
        ...
        toastr.info(
            "Downloading. Check the Tabby console for progress reports.",
        );

        const response = await fetch(
            "/api/backends/text-completions/tabby/download",
            {
                method: "POST",
                headers: getRequestHeaders(),
        ...
        console.error(err);
        toastr.error(
            "Failed to download HuggingFace model in TabbyAPI. Please try again.",
        );
    }
}
