...
async function* parseStreamData(json) {
    ...
    // MakerSuite
        ...
        for (let i = 0; i < json.candidates.length; i++) {
            ...
            const hasToolCalls = json?.candidates?.[0]?.content?.parts?.some(
                (p) => p?.functionCall,
            );
            const hasInlineData = json?.candidates?.[0]?.content?.parts?.some(
                (p) => p?.inlineData,
            );
            ...
            if (
                ...
                for (
                    let j = 0;
                    j < json.candidates[i].content.parts.length;
                    j++
                ) {
                    if (
                        typeof json.candidates[i].content.parts[j].text ===
                        "string"
                    ) {
                        for (
                            let k = 0;
                            k < json.candidates[i].content.parts[j].text.length;
                            k++
                        ) {
                            const moreThanOnePart =
                                json.candidates[i].content.parts.length > 1;
                            const isNotLastPart =
                                j !==
                                json.candidates[i].content.parts.length - 1;
                            const isLastSymbol =
                                k ===
                                json.candidates[i].content.parts[j].text
                                    .length -
                                    1;
                            const addNewline =
                                moreThanOnePart &&
                                ...
                                isLastSymbol;
                            const str =
                                json.candidates[i].content.parts[j].text[k] +
                                (addNewline ? "\n\n" : "");
                            const candidateClone = structuredClone(
                                json.candidates[0],
                            );
                            candidateClone.content.parts[j].text = str;
                            candidateClone.content.parts = [
                                candidateClone.content.parts[j],
                            ];
    ...
    // OpenAI-likes
        ...
        if (
            ...
            if (
                typeof json.choices[0].delta.text === "string" &&
                json.choices[0].delta.text.length > 0
            ) {
                ...
                for (
                    let j = 0;
                    j < json.choices[0].delta.reasoning_content.length;
                    j++
                ) {
                    const str = json.choices[0].delta.reasoning_content[j];
                    const isLastSymbol =
                        j ===
                        json.choices[0].delta.reasoning_content.length - 1;
                    const choiceClone = structuredClone(json.choices[0]);
                    choiceClone.delta.reasoning_content = str;
                    choiceClone.delta.content = isLastSymbol
                        ? choiceClone.delta.content
                        : "";
                    const choices = [choiceClone];
                    yield {
                        data: { ...json, choices },
                ...
                for (
                    let j = 0;
                    j < json.choices[0].delta.reasoning.length;
                    j++
                ) {
                    const str = json.choices[0].delta.reasoning[j];
                    const isLastSymbol =
                        j === json.choices[0].delta.reasoning.length - 1;
                    const choiceClone = structuredClone(json.choices[0]);
            ...
            if (
                ...
                for (
                    let j = 0;
                    j < json.choices[0].message.content.length;
                    j++
                ) {
