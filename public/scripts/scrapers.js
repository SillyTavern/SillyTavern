...
export class ScraperManager {
    ...
    static runDataBankScraper(scraperId) {
        const scraper = ScraperManager.#scrapers.find(
            (s) => s.id === scraperId,
        );
        if (!scraper) {
...
class Notepad {
    ...
    async scrape() {
        ...
        template
            .find('textarea[name="notepadFileContent"]')
            .on("input", function () {
        ...
        const file = new File([text], `Notepad - ${fileName}.txt`, {
            type: "text/plain",
        });
        return [file];
    }
...
class WebScraper {
    ...
    async scrape() {
        const template = $(
            await renderExtensionTemplateAsync("attachments", "web-scrape", {}),
        );
        const linksString = await callGenericPopup(
            template,
            POPUP_TYPE.INPUT,
            "",
            {
    ...
    async scrape() {
        ...
        if (output === "single") {
            const combinedContent = data
                ...
                .join("\n\n\n\n");
            const file = new File([combinedContent], `${url}.txt`, {
                type: "text/plain",
            });
            return [file];
        }
...
class FandomScraper {
    ...
    async scrape() {
        ...
        template
            .find('input[name="fandomScrapeInput"]')
            .on("input", function () {
                fandom = String($(this).val()).trim();
            });
        template
            .find('input[name="fandomScrapeFilter"]')
            .on("input", function () {
                filter = String($(this).val());
            });
        template
            .find('input[name="fandomScrapeOutput"]')
            .on("input", function () {
                output = String($(this).val());
            });
        ...
        if (output === "single") {
            const combinedContent = data
                ...
                .join("\n\n\n\n");
            const file = new File([combinedContent], `${fandom}.txt`, {
                type: "text/plain",
            });
            return [file];
        }
...
class YouTubeScraper {
            ...
            SlashCommand.fromProps({
                ...
                callback: async (args, url) => {
                    try {
                        if (!url) {
                            throw new Error(
                                "URL or ID of the YouTube video is required",
                            );
                        }

                        const lang = String(args?.lang || "");
                        const { transcript } = await this.getScript(
                            String(url).trim(),
                            lang,
                        );
                        return transcript;
                ...
                helpString:
                    "Scrape a transcript from a YouTube video by ID or URL.",
    ...
    async scrape() {
        ...
        const videoUrl = await callGenericPopup(
            template,
            POPUP_TYPE.INPUT,
            "",
            {
        ...
        template
            .find('input[name="youtubeLanguageCode"]')
            .on("input", function () {
