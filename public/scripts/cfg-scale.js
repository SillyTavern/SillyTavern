...
function setCharCfg(tempValue, setting) {
    ...
    if (extension_settings.cfg.chara) {
        existingCharaCfgIndex = extension_settings.cfg.chara.findIndex(
            (e) => e.name === avatarName,
        );
    ...
    if (extension_settings.cfg.chara && existingCharaCfg) {
        ...
        console.debug(
            "Character CFG error: No avatar name key could be found.",
        );
...
function onCfgMenuItemClick() {
    if (!selected_group && this_chid === undefined) {
        toastr.warning(
            "Select a character before trying to configure CFG",
            "",
            { timeOut: 2000 },
        );
        return;
...
function loadSettings() {
    ...
    $("#chat_cfg_guidance_scale").val(
        chat_metadata[metadataKeys.guidance_scale] ?? (1.0).toFixed(2),
    );
    $("#chat_cfg_guidance_scale_counter").val(
        chat_metadata[metadataKeys.guidance_scale]?.toFixed(2) ??
            (1.0).toFixed(2),
    );
    $("#chat_cfg_negative_prompt").val(
        chat_metadata[metadataKeys.negative_prompt] ?? "",
    );
    $("#chat_cfg_positive_prompt").val(
        chat_metadata[metadataKeys.positive_prompt] ?? "",
    );
    ...
    if (chat_metadata[metadataKeys.prompt_combine]?.length > 0) {
        chat_metadata[metadataKeys.prompt_combine].forEach((element) => {
            $(`input[name="cfg_prompt_combine"][value="${element}"]`).prop(
                "checked",
                true,
            );
        });
    ...
    $("#cfg_prompt_separator").val(
        promptSeparatorDisplay.length === 0
            ? ""
            : promptSeparatorDisplay.join(""),
    );

    $("#cfg_prompt_insertion_depth").val(
...
function migrateSettings() {
    ...
    if (power_user.negative_prompt) {
        extension_settings.cfg.global.negative_prompt =
            power_user.negative_prompt;
        delete power_user["negative_prompt"];
        performSettingsSave = true;
    }

    if (chat_metadata["cfg_negative_combine"]) {
        chat_metadata[metadataKeys.prompt_combine] =
            chat_metadata["cfg_negative_combine"];
    ...
    if (chat_metadata["cfg_negative_separator"]) {
        chat_metadata[metadataKeys.prompt_separator] =
            chat_metadata["cfg_negative_separator"];
...
export function initCfg() {
    ...
    $("#cfg_prompt_insertion_depth").on("input", function () {
        chat_metadata[metadataKeys.prompt_insertion_depth] = Number(
            $(this).val(),
        );
        saveMetadataDebounced();
    });
...
export function getGuidanceScale() {
    ...
    const groupchatCharOverride =
        chat_metadata[metadataKeys.groupchat_individual_chars] ?? false;

    if (
        chatGuidanceScale &&
        chatGuidanceScale !== 1 &&
        !groupchatCharOverride
    ) {
        return {
            type: cfgType.chat,
    ...
    if (
        extension_settings.cfg.global &&
        extension_settings.cfg.global?.guidance_scale !== 1
    ) {
        return {
            type: cfgType.global,
...
function getCustomSeparator() {
    ...
    try {
        ...
        console.warn(
            "Invalid JSON detected for prompt separator. Using default separator.",
        );
        return defaultSeparator;
    }
}
...
export function getCfgPrompt(guidanceScale, isNegative, quiet = false) {
    ...
    if (
        guidanceScale.type === cfgType.chat ||
        cfgPromptCombine.includes(cfgType.chat)
    ) {
        splitCfgPrompt.unshift(
            substituteParams(
                chat_metadata[
                    isNegative
                        ? metadataKeys.negative_prompt
                        : metadataKeys.positive_prompt
                ],
            ),
        );
    }
    ...
    if (
        guidanceScale.type === cfgType.chara ||
        cfgPromptCombine.includes(cfgType.chara)
    ) {
        splitCfgPrompt.unshift(
            substituteParams(
                isNegative
    ...
    if (
        guidanceScale.type === cfgType.global ||
        cfgPromptCombine.includes(cfgType.global)
    ) {
        splitCfgPrompt.unshift(
            substituteParams(
                isNegative
