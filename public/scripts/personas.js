...
export const personasFilter = new FilterHelper(
    debounce(getUserAvatars, debounce_timeout.quick),
);
...
function sortPersonas(personas) {
    ...
    if (option.attr("value") === "search") {
        personas.sort((a, b) => {
            const aScore = personasFilter.getScore(
                FILTER_TYPES.PERSONA_SEARCH,
                a,
            );
            const bScore = personasFilter.getScore(
                FILTER_TYPES.PERSONA_SEARCH,
                b,
            );
...
function verifyPersonaSearchSortRule() {
    const searchTerm = personasFilter.getFilterData(
        FILTER_TYPES.PERSONA_SEARCH,
    );
...
function getUserAvatarBlock(avatarId) {
    ...
    const personaDescription =
        power_user.persona_descriptions[avatarId]?.description;

    template.find(".ch_name").text(personaName || "[Unnamed Persona]");
    template
        .find(".ch_description")
        .text(
            personaDescription || $("#user_avatar_block").attr("no_desc_text"),
        )
        .toggleClass("text_muted", !personaDescription);
    template.attr("data-avatar-id", avatarId);
    template
        .find(".avatar")
        .attr("data-avatar-id", avatarId)
        .attr("title", avatarId);
...
export async function getUserAvatars(doRender = true, openPageAt = "") {
    ...
    if (response.ok) {
        ...
        $("#persona_pagination_container").pagination({
            ...
            formatSizeChanger: renderPaginationDropdown(
                perPage,
                sizeChangerOptions,
            ),
            prevText: "<",
...
async function changeUserAvatar(e) {
    ...
    if (!power_user.never_resize_avatars) {
        const dlg = new Popup(
            t`Set the crop position of the avatar image`,
            POPUP_TYPE.CROP,
            "",
            { cropImage: dataUrl },
        );
...
export function setPersonaDescription() {
    ...
    if (
        ...
        persona_description_positions.AFTER_CHAR
    ) {
        power_user.persona_description_position =
    ...
    $("#persona_lore_button").toggleClass(
        "world_set",
...
function getPersonasOfCurrentChat() {
    const personas = chat
        .filter((message) =>
            String(message.force_avatar).startsWith(USER_AVATAR_PATH),
        )
...
export function updatePersonaConnectionsAvatarList() {
    ...
    const connections =
        power_user.persona_descriptions[user_avatar]?.connections ?? [];
    const entities = connections
        .map((connection) => {
            if (connection.type === "character") {
                const character = characters.find(
                    (c) => c.avatar === connection.id,
                );
                if (character)
...
export async function askForPersonaSelection(
    ...
    if (personas.length > 0)
        buildPersonaAvatarList(personaListBlock, personas, {
            interactable: true,
        });
    ...
    personaListBlock
        .querySelectorAll('.avatar[data-type="persona"]')
        .forEach((block) => {
            if (!(block instanceof HTMLElement)) return;
            block.dataset.result = String(
                100 + personas.indexOf(block.dataset.pid),
            );
            ...
            if (
                personasToHighlight &&
                personasToHighlight.includes(block.dataset.pid)
            ) {
    ...
    if (targetedChar) {
        customButtons.push({
            ...
            result: 2,
            action: () => {
                for (const [personaId, description] of Object.entries(
                    ...
                    if (connections) {
                        power_user.persona_descriptions[personaId].connections =
                            connections.filter((c) => {
                                if (
                                    targetedChar.type == c.type &&
                                    targetedChar.id == c.id
                                )
                                    return false;
                                return true;
                            });
    ...
    if (personaName) {
        ...
        if (personaName !== name1) {
            ...
            setUserName(personaName, {
                toastPersonaNameChange:
                    !shouldAutoLock && toastPersonaNameChange,
            });
        }
        ...
        if (descriptor) {
            ...
            power_user.persona_description_depth =
                descriptor.depth ?? DEFAULT_DEPTH;
            power_user.persona_description_role =
                descriptor.role ?? DEFAULT_ROLE;
...
export function isPersonaLocked(type = "chat") {
    switch (type) {
        ...
        case "character": {
            return !!power_user.persona_descriptions[
                user_avatar
            ]?.connections?.some(isPersonaConnectionLocked);
        }
        default:
...
async function unlockPersona(type = "chat") {
    switch (type) {
        ...
        case "character": {
            ...
            if (connections) {
                console.log(
                    `Unlocking persona ${user_avatar} from this character ${name2}`,
                );
                power_user.persona_descriptions[user_avatar].connections =
                    connections.filter((c) => !isPersonaConnectionLocked(c));
                saveSettingsDebounced();
                updatePersonaConnectionsAvatarList();
                if (
                    power_user.persona_show_notifications &&
    ...
    switch (type) {
        ...
        case "character": {
            ...
            if (newConnection && newConnection.id) {
                console.log(
                    `Locking persona ${user_avatar} to this character ${name2}`,
                );
                power_user.persona_descriptions[user_avatar].connections = [
                    ...connections,
                    newConnection,
                ...
                if (!power_user.persona_allow_multi_connections) {
                    for (const [avatarId, description] of Object.entries(
                        power_user.persona_descriptions,
                    )) {
                ...
                if (power_user.persona_show_notifications) {
                    ...
                    if (additional || !isPersonaPanelOpen()) {
                        toastr.success(
                            t`User persona ${name1} is locked to character ${name2}${additional}`,
                            t`Persona Locked`,
                            { escapeHtml: false },
                        );
                    }
...
function onPersonaDescriptionInput() {
    ...
    $(`.avatar-container[imgfile="${user_avatar}"] .ch_description`)
        .text(
            power_user.persona_description ||
                $("#user_avatar_block").attr("no_desc_text"),
        )
...
function onPersonaDescriptionDepthRoleInput() {
    power_user.persona_description_role = Number(
        $("#persona_depth_role").find(":selected").val(),
    );
...
function updatePersonaUIStates({ navigateToCurrent = false } = {}) {
    ...
    $("#lock_user_name i.icon").toggleClass(
        "fa-lock",
    ...
    $("#lock_persona_to_char").toggleClass(
        "locked",
        personaStates.locked.character,
    );
    $("#lock_persona_to_char i.icon").toggleClass(
        "fa-lock",
        personaStates.locked.character,
    );
    $("#lock_persona_to_char i.icon").toggleClass(
...
function getPersonaTemporaryLockInfo() {
    const hasDifferentChatLock =
        !!chat_metadata["persona"] && chat_metadata["persona"] !== user_avatar;
    const hasDifferentDefaultLock =
        power_user.default_persona &&
        power_user.default_persona !== user_avatar;
    const isTemporary =
        hasDifferentChatLock ||
        (!chat_metadata["persona"] && hasDifferentDefaultLock);
...
async function loadPersonaForCurrentChat({ doRender = false } = {}) {
    ...
    if (chat_metadata["persona"]) {
        ...
        if (!userAvatars.includes(chatPersona)) {
            console.warn(
                "Chat-locked persona avatar not found, unlocking persona",
            );
            delete chat_metadata["persona"];
            saveSettingsDebounced();
            chatPersona = "";
    ...
    */
    ...
    if (
        ...
        !userAvatars.includes(chat_metadata["persona"])
    ) {
    ...
    if (chatPersona && user_avatar !== chatPersona) {
        ...
        if (power_user.persona_show_notifications) {
            ...
            toastr.success(message, t`Persona Auto Selected`, {
                escapeHtml: false,
            });
        }
    }
...
export function getConnectedPersonas(characterKey = undefined) {
    ...
    const connectedPersonas = Object.entries(power_user.persona_descriptions)
        .filter(([_, desc]) =>
            desc.connections?.some(
                (conn) => conn.type === "character" && conn.id === characterKey,
            ),
        )
        .map(([key, _]) => key);
    ...
    const selectedPersona = await askForPersonaSelection(
        ...
        connections,
        {
            ...
            shiftClickHandler: (element, ev) => {
                ...
                if (connections) {
                    console.log(
                        `Unlocking persona ${personaId} from current character ${name2}`,
                    );
                    power_user.persona_descriptions[personaId].connections =
                        connections.filter((c) => {
                            if (
...
async function onPersonasRestoreInput(e) {
    ...
    for (const [key, value] of Object.entries(data.personas)) {
        if (key in power_user.personas) {
            warnings.push(
                `Persona "${key}" (${value}) already exists, skipping`,
            );
            continue;
        }
        ...
        if (!avatarsList.includes(key)) {
            warnings.push(
                `Persona image "${key}" (${value}) is missing, uploading default avatar`,
            );
    ...
    if (data.default_persona) {
        if (data.default_persona in power_user.personas) {
            ...
            warnings.push(
                `Default persona "${data.default_persona}" does not exist, skipping`,
            );
        }
    }
    ...
    if (warnings.length) {
        ...
        console.warn(
            `PERSONA RESTORE REPORT\n====================\n${warnings.join("\n")}`,
        );
    } else {
        toastr.success(
            t`Personas restored successfully.`,
            t`Persona Management`,
        );
    }
...
function setNameCallback({ mode = "all" }, name) {
    ...
    if (["lookup", "all"].includes(mode)) {
        let persona = Object.entries(power_user.personas).find(
            ([avatar, _]) => avatar === name,
        )?.[1];
        if (!persona)
            persona = Object.entries(power_user.personas).find(
                ([_, personaName]) =>
                    personaName.toLowerCase() === name.toLowerCase(),
            )?.[1];
        if (persona) {
...
function registerPersonaSlashCommands() {
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            ...
            helpString:
                "Locks/unlocks a persona (name and avatar) to the current chat. Gets the current lock state for the given type if no state is provided.",
            namedArgumentList: [
                SlashCommandNamedArgument.fromProps({
                    name: "type",
                    description:
                        "The type of the lock, where it should apply to",
                    typeList: [ARGUMENT_TYPE.STRING],
                    defaultValue: "chat",
                    enumList: [
                        new SlashCommandEnumValue(
                            "chat",
                            "Lock the persona to the current chat.",
                        ),
                        new SlashCommandEnumValue(
                            "character",
                        ...
                        new SlashCommandEnumValue(
                            "default",
                            "Lock this persona as the default persona for all new chats.",
                        ),
                    ],
                }),
            ],
        ...
        SlashCommand.fromProps({
            ...
            callback: (args, value) => {
                if (!value) {
                    value = "toggle";
                    toastr.warning(
                        t`Using /lock without a provided state to toggle the persona is deprecated. Please use /persona-lock instead.
                        In the future this command with no state provided will return the current state, instead of toggling it.`,
        ...
        SlashCommand.fromProps({
            ...
            namedArgumentList: [
                new SlashCommandNamedArgument(
                    "mode",
                    'The mode for persona selection. ("lookup" = search for existing persona, "temp" = create a temporary name, set a temporary name, "all" = allow both in the same command)',
            ...
            helpString:
                "Selects the given persona with its name and avatar (by name or avatar url). If no matching persona exists, applies a temporary name.",
        }),
    );
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            ...
            helpString:
                "Syncs the user persona in user-attributed messages in the current chat.",
        }),
    );
}
...
export async function initPersonas() {
    ...
    $("#lock_persona_to_char").on("click", () =>
        togglePersonaLock("character"),
    );
    ...
    $("#personas_restore").on("click", () =>
        $("#personas_restore_input").trigger("click"),
    );
    ...
    $("#persona_duplicate_button").on("click", () =>
        duplicatePersona(user_avatar),
    );
