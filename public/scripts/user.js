...
async function deleteUser(handle, callback) {
    try {
        ...
        template
            .find('input[name="deleteUserHandle"]')
            .on("input", function () {
...
async function viewSettingsSnapshots() {
    ...
    async function renderSnapshots() {
        ...
        for (const snapshot of snapshots.sort((a, b) => b.date - a.date)) {
            const snapshotBlock = template
                .find(".snapshotTemplate .snapshot")
                .clone();
            snapshotBlock.find(".snapshotName").text(snapshot.name);
            snapshotBlock
                .find(".snapshotDate")
                .text(new Date(snapshot.date).toLocaleString());
            snapshotBlock
                .find(".snapshotSize")
                .text(humanFileSize(snapshot.size));
            snapshotBlock
                .find(".snapshotRestoreButton")
                .on("click", async (e) => {
                    e.stopPropagation();
    ...
    template
        .find(".makeSnapshotButton")
        .on("click", () => makeSnapshot(renderSnapshots));
    renderSnapshots();
}
...
async function openUserProfile() {
    ...
    template
        .find(".userCreated")
        .text(new Date(currentUser.created).toLocaleString());
    template.find(".hasPassword").toggle(currentUser.password);
    template.find(".noPassword").toggle(!currentUser.password);
    template
        .find(".userSettingsSnapshotsButton")
        .on("click", () => viewSettingsSnapshots());
    ...
    template
        .find(".userResetAllButton")
        .on("click", () => resetEverything(() => location.reload()));
    template
...
async function cropAndUploadAvatar(handle, file) {
    const dataUrl = await getBase64Async(
    ...
    async function renderUsers() {
        ...
        for (const user of users) {
            const userBlock = template
                .find(".userAccountTemplate .userAccount")
                .clone();
            ...
            userBlock
                .find(".userDelete")
                .on("click", () => deleteUser(user.handle, renderUsers));
            userBlock
                .find(".userChangeNameButton")
                .on("click", async () =>
                    changeName(user.handle, user.name, renderUsers),
                );
            ...
            userBlock
                .find(".userAvatarChange")
                .on("click", () =>
                    userBlock.find(".avatarUpload").trigger("click"),
                );
