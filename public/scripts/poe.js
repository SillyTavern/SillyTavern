...
async function sendChunkedMessage(finalPrompt, withStreaming, withSuggestions, signal) {
    ...
    for (let i = 0; i < promptChunks.length; i++) {
        ...
        if (i == promptChunks.length - 1) {
            ...
            const chunkReply = await sendMessage(promptChunk, false, false, signal, true);
...
async function sendMessage(prompt, withStreaming, withSuggestions, signal, isChunked = false) {
    //console.log(prompt);
