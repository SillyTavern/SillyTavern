    ...
    if (args.index !== undefined) {
        try {
            let localVariable = JSON.parse(
                chat_metadata.variables[name] ?? "null",
            );
            const numIndex = Number(args.index);
            if (Number.isNaN(numIndex)) {
    ...
    if (args.index !== undefined) {
        try {
            let globalVariable = JSON.parse(
                extension_settings.variables.global[name] ?? "null",
            );
            const numIndex = Number(args.index);
            if (Number.isNaN(numIndex)) {
...
async function listVariablesCallback(args) {
    ...
    const localVariables = includeLocalVariables
        ? Object.entries(chat_metadata.variables).map(
...
async function whileCallback(args, value) {
    if (args.guard instanceof SlashCommandClosure)
        throw new Error(
            "argument 'guard' cannot be a closure for command /while",
        );
...
async function timesCallback(args, value) {
    ...
    const iterations = Math.min(
        Number(repeats),
        isGuardOff ? Number.MAX_SAFE_INTEGER : MAX_LOOPS,
    );
    let result;
    for (let i = 0; i < iterations; i++) {
        if (command instanceof SlashCommandClosure) {
...
async function ifCallback(args, value) {
    ...
    if (result && command) {
        ...
        if (args.else instanceof SlashCommandClosure)
            return (await args.else.execute()).pipe;
        commandResult = await executeSubCommands(
            args.else,
            args._scope,
            args._parserFlags,
...
function existsLocalVariable(name) {
    return (
        chat_metadata.variables && chat_metadata.variables[name] !== undefined
    );
}
...
export function evalBoolean(rule, a, b) {
    ...
    switch (rule) {
        ...
        default:
            throw new Error(
                `Unknown boolean comparison rule for type number. Accepted: in, nin, eq, neq. Provided: ${rule}`,
            );
    }
}
...
function parseNumericSeries(value, scope = null) {
    ...
    const array = values
        ...
        .map((i) =>
            isNaN(Number(i))
                ? Number(resolveVariable(String(i), scope))
                : Number(i),
        )
        .filter((i) => !isNaN(i));
...
function powValuesCallback(args, value) {
    return performOperation(
        value,
        (array) => Math.pow(array[0], array[1]),
        false,
        args._scope,
    );
}
...
function letCallback(args, value) {
    ...
    if (args.key !== undefined) {
        ...
        if (args._hasUnnamedArgument) {
            const val =
                typeof value[0] == "string" ? value.join(" ") : value[0];
...
export function registerVariableCommands() {
            ...
            namedArgumentList: [
                ...
                SlashCommandNamedArgument.fromProps({
                    ...
                    enumList: [
                        ...
                        new SlashCommandEnumValue(
                            "none",
                            "Just return the variables as a JSON list.",
                            enumTypes.enum,
                            enumIcons.array,
                        ),
                    ],
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            name: "setvar",
            callback: (args, value) =>
                String(setLocalVariable(args.key || args.name, value, args)),
            aliases: ["setchatvar"],
            ...
            namedArgumentList: [
                ...
                SlashCommandNamedArgument.fromProps({
                    name: "as",
                    description:
                        "change the type of the value when used with index",
                    forceEnum: true,
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            name: "addvar",
            callback: (args, value) =>
                String(addLocalVariable(args.key || args.name, value)),
            aliases: ["addchatvar"],
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            name: "addglobalvar",
            callback: (args, value) =>
                String(addGlobalVariable(args.key || args.name, value)),
            returns: "the new variable value",
            namedArgumentList: [
                SlashCommandNamedArgument.fromProps({
                    name: "key",
        ...
        SlashCommand.fromProps({
            ...
            unnamedArgumentList: [
                SlashCommandNamedArgument.fromProps({
                    name: "key",
                    description:
                        "variable name or closure that returns a variable name",
                    typeList: [
                        ARGUMENT_TYPE.VARIABLE_NAME,
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            ...
            unnamedArgumentList: [
                SlashCommandArgument.fromProps({
                    description:
                        "values to subtract, starting form the first provided value",
                    typeList: [
                        ARGUMENT_TYPE.NUMBER,
                        ARGUMENT_TYPE.VARIABLE_NAME,
                        ARGUMENT_TYPE.LIST,
        ...
        SlashCommand.fromProps({
            ...
            namedArgumentList: [
                SlashCommandNamedArgument.fromProps({
                    name: "keysort",
                    description:
                        "whether to sort by key or value; ignored for lists",
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            name: "var",
            callback: (/** @type {NamedArguments} */ args, value) =>
                varCallback(args, value),
            returns: "the variable value",
            namedArgumentList: [
                SlashCommandNamedArgument.fromProps({
                    name: "key",
