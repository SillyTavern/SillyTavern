...
export class ToolManager {
  ...
  */
  ...
  static isToolCallingSupported() {
    ...
    if (oai_settings.chat_completion_source === chat_completion_sources.MEGANOVAAI && Array.isArray(model_list)) {
      const currentModel = model_list.find(model => model.id === oai_settings.meganovaai_model);
      if (currentModel && currentModel.metadata?.function_call) {
        return currentModel.metadata.function_call;
      }
    }
