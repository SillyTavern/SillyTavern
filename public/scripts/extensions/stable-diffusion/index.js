...
const writePromptFieldsDebounced = debounce(
    writePromptFields,
...
function processTriggers(chat, _, abort, type) {
    ...
    if (
        extension_settings.sd.function_tool &&
    ...
    try {
        ...
        console.log(
            `SD: Triggered by "${message}", detected subject: "${subject}"`,
        );

        outer: for (const [specialMode, triggers] of Object.entries(
            messageTrigger.specialCases,
        )) {
            for (const trigger of triggers) {
                if (subject === trigger) {
                    subject = triggerWords[specialMode][0];
                    console.log(
                        `SD: Detected special case "${trigger}", switching to mode ${specialMode}`,
                    );
                    break outer;
                }
            }
        ...
        abort(true);
        setTimeout(
            () => generatePicture(initiators.interactive, {}, subject, message),
            1,
        );
    } catch {
...
function getSdRequestBody() {
    switch (extension_settings.sd.source) {
        case sources.vlad:
            return {
                url: extension_settings.sd.vlad_url,
                auth: extension_settings.sd.vlad_auth,
            };
        case sources.auto:
            return {
...
async function loadSettings() {
    ...
    $("#sd_novel_decrisper").prop(
        "checked",
        extension_settings.sd.novel_decrisper,
    );
    $("#sd_pollinations_enhance").prop(
        "checked",
        extension_settings.sd.pollinations_enhance,
    );
    $("#sd_horde").prop("checked", extension_settings.sd.horde);
    $("#sd_horde_nsfw").prop("checked", extension_settings.sd.horde_nsfw);
    $("#sd_horde_karras").prop("checked", extension_settings.sd.horde_karras);
    $("#sd_horde_sanitize").prop(
        "checked",
        extension_settings.sd.horde_sanitize,
    );
    $("#sd_restore_faces").prop("checked", extension_settings.sd.restore_faces);
    $("#sd_enable_hr").prop("checked", extension_settings.sd.enable_hr);
    $("#sd_adetailer_face").prop(
        "checked",
        extension_settings.sd.adetailer_face,
    );
    $("#sd_refine_mode").prop("checked", extension_settings.sd.refine_mode);
    $("#sd_multimodal_captioning").prop(
        "checked",
        extension_settings.sd.multimodal_captioning,
    );
    ...
    $("#sd_interactive_mode").prop(
        "checked",
        extension_settings.sd.interactive_mode,
    );
    ...
    $("#sd_command_visible").prop(
        "checked",
        extension_settings.sd.command_visible,
    );
    $("#sd_interactive_visible").prop(
        "checked",
        extension_settings.sd.interactive_visible,
    );
    $("#sd_tool_visible").prop("checked", extension_settings.sd.tool_visible);
    $("#sd_stability_style_preset").val(
        extension_settings.sd.stability_style_preset,
    );
    $("#sd_huggingface_model_id").val(
        extension_settings.sd.huggingface_model_id,
    );
    $("#sd_function_tool").prop("checked", extension_settings.sd.function_tool);
    $("#sd_bfl_upsampling").prop(
        "checked",
        extension_settings.sd.bfl_upsampling,
    );
...
function onStyleSelect() {
    ...
    const styleObject = extension_settings.sd.styles.find(
        (x) => x.name === selectedStyle,
    );
...
async function onSaveStyleClick() {
    ...
    const existingStyle = extension_settings.sd.styles.find(
        (x) => x.name === name,
    );
...
async function onChatChanged() {
    ...
    let characterPrompt = key
        ? extension_settings.sd.character_prompts[key] || ""
        : "";
    let negativePrompt = key
        ? extension_settings.sd.character_negative_prompts[key] || ""
        : "";

    const context = getContext();
    const sharedPromptData =
        context?.characters[this_chid]?.data?.extensions?.sd_character_prompt;
    const hasSharedData =
        sharedPromptData && typeof sharedPromptData === "object";
...
function combinePrefixes(str1, str2, macro = "") {
    ...
    const result =
        macro && str1.includes(macro)
            ? str1.replace(macro, str2)
            : `${str1}, ${str2},`;
...
function onWandVisibleInput() {
    extension_settings.sd.wand_visible =
        !!$("#sd_wand_visible").prop("checked");
    saveSettingsDebounced();
}

function onCommandVisibleInput() {
    extension_settings.sd.command_visible = !!$("#sd_command_visible").prop(
        "checked",
    );
    saveSettingsDebounced();
}
...
function onInteractiveVisibleInput() {
    extension_settings.sd.interactive_visible = !!$(
        "#sd_interactive_visible",
    ).prop("checked");
    saveSettingsDebounced();
}

function onToolVisibleInput() {
    extension_settings.sd.tool_visible =
        !!$("#sd_tool_visible").prop("checked");
    saveSettingsDebounced();
}
...
function onADetailerFaceChange() {
    extension_settings.sd.adetailer_face =
        !!$("#sd_adetailer_face").prop("checked");
    saveSettingsDebounced();
}
...
const resolutionOptions = {
    ...
    sd_res_1536x1024: {
        width: 1536,
        height: 1024,
        name: "1536x1024 (3:2, ChatGPT)",
    },
    sd_res_1024x1536: {
        width: 1024,
        height: 1536,
        name: "1024x1536 (2:3, ChatGPT)",
    },
    sd_res_1024x1792: {
        width: 1024,
        height: 1792,
        name: "1024x1792 (4:7, DALL-E)",
    },
    sd_res_1792x1024: {
        width: 1792,
        height: 1024,
        name: "1792x1024 (7:4, DALL-E)",
    },
};
...
function onSchedulerChange() {
    extension_settings.sd.scheduler = $("#sd_scheduler")
        .find(":selected")
        .val();
    saveSettingsDebounced();
}
...
async function onOpenAiStyleSelect() {
    extension_settings.sd.openai_style = String(
        $("#sd_openai_style").find(":selected").val(),
    );
    saveSettingsDebounced();
}

async function onOpenAiQualitySelect() {
    extension_settings.sd.openai_quality = String(
        $("#sd_openai_quality").find(":selected").val(),
    );
    saveSettingsDebounced();
}
...
function onNovelSmDynInput() {
    extension_settings.sd.novel_sm_dyn =
        !!$("#sd_novel_sm_dyn").prop("checked");
    saveSettingsDebounced();
}

function onNovelDecrisperInput() {
    extension_settings.sd.novel_decrisper = !!$("#sd_novel_decrisper").prop(
        "checked",
    );
    saveSettingsDebounced();
}
...
function onDenoisingStrengthInput() {
    ...
    $("#sd_denoising_strength_value").val(
...
function onHrSecondPassStepsInput() {
    ...
    $("#sd_hr_second_pass_steps_value").val(
...
function onHFModelInput() {
    extension_settings.sd.huggingface_model_id = $(
        "#sd_huggingface_model_id",
    ).val();
    saveSettingsDebounced();
}

function onComfyWorkflowChange() {
    extension_settings.sd.comfy_workflow = $("#sd_comfy_workflow")
        .find(":selected")
        .val();
    saveSettingsDebounced();
}
...
function onBflUpsamplingInput() {
    extension_settings.sd.bfl_upsampling =
        !!$("#sd_bfl_upsampling").prop("checked");
    saveSettingsDebounced();
}

function onStabilityStylePresetChange() {
    extension_settings.sd.stability_style_preset = String(
        $("#sd_stability_style_preset").val(),
    );
    saveSettingsDebounced();
}
...
async function loadStabilityModels() {
    $("#sd_stability_key").toggleClass(
        "success",
        !!secret_state[SECRET_KEYS.STABILITY],
    );
...
async function loadFalaiModels() {
    $("#sd_falai_key").toggleClass(
        "success",
        !!secret_state[SECRET_KEYS.FALAI],
    );
...
async function loadXAIModels() {
    return [{ value: "grok-2-image-1212", text: "grok-2-image-1212" }];
}
...
async function loadSchedulers() {
    ...
    if (
        !extension_settings.sd.scheduler &&
        schedulers.length > 0 &&
        schedulers[0] !== "N/A"
    ) {
        extension_settings.sd.scheduler = schedulers[0];
        $("#sd_scheduler")
            .val(extension_settings.sd.scheduler)
            .trigger("change");
    }
}
...
function getRawLastMessage() {
    ...
    const character = context.groupId
        ? context.characters.find(
              (c) => c.avatar === lastMessage.original_avatar,
          )
...
async function generatePicture(initiator, args, trigger, message, callback) {
    ...
    if (!isValidState()) {
        toastr.warning(
            "Image generation is not available. Check your settings and try again.",
        );
        return;
    }
    ...
    console.log(
        `Image generation mode ${generationTypeKey} triggered with "${trigger}"`,
    );
    ...
    try {
        const combineNegatives = (prefix) => {
            negativePromptPrefix = combinePrefixes(
                negativePromptPrefix,
                prefix,
            );
        };
...
function setTypeSpecificDimensions(generationType) {
    ...
    if (
        (generationType === generationMode.FACE ||
    ...
    if (extension_settings.sd.snap) {
        ...
        const newPixelCount =
            extension_settings.sd.height * extension_settings.sd.width;

        if (prevPixelCount !== newPixelCount) {
            const ratio = Math.sqrt(prevPixelCount / newPixelCount);
            extension_settings.sd.height =
                Math.round((extension_settings.sd.height * ratio) / 64) * 64;
            extension_settings.sd.width =
                Math.round((extension_settings.sd.width * ratio) / 64) * 64;
            console.log(
            ...
            if (resolution) {
                ...
                console.warn(
                    "Snap to resolution failed, using custom dimensions",
                );
            }
        }
    }
...
async function getPrompt(
    ...
    switch (generationType) {
        ...
        case generationMode.USER_MULTIMODAL:
            prompt = await generateMultimodalPrompt(
                generationType,
                quietPrompt,
            );
            break;
        ...
        const getLastCharacterKey = () => {
            ...
            for (let i = context.chat.length - 1; i >= 0; i--) {
                ...
                if (
                    !message.is_user &&
                    !message.is_system &&
                    typeof message.original_avatar === "string"
                ) {
...
async function generateMultimodalPrompt(generationType, quietPrompt) {
    ...
    try {
        ...
        console.error(error);
        toastr.error(
            "Multimodal captioning failed. Please try again.",
            "Image Generation",
        );
...
function getCharacterAvatarUrl() {
    ...
    if (context.groupId) {
        const groupMembers = context.groups.find(
...
async function sendGenerationRequest(
    ...
    const negativePrefix = noCharPrefix.includes(generationType)
        ...
        : combinePrefixes(
              extension_settings.sd.negative_prompt,
              getCharacterNegativePrefix(),
          );

    const prefixedPrompt = substituteParams(
        combinePrefixes(prefix, prompt, "{prompt}"),
    );
    ...
    try {
        switch (extension_settings.sd.source) {
            case sources.extras:
                result = await generateExtrasImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
                break;
            case sources.horde:
                result = await generateHordeImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
                break;
            case sources.vlad:
                result = await generateAutoImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
                break;
            case sources.drawthings:
                result = await generateDrawthingsImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
            ...
            case sources.auto:
                ...
                break;
            case sources.novel:
                result = await generateNovelImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
            ...
            case sources.openai:
                ...
                break;
            case sources.comfy:
                result = await generateComfyImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
                break;
            case sources.togetherai:
                result = await generateTogetherAIImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
                break;
            case sources.pollinations:
                result = await generatePollinationsImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
            ...
            case sources.huggingface:
                ...
                break;
            case sources.nanogpt:
                result = await generateNanoGPTImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
            ...
            case sources.bfl:
                ...
                break;
            case sources.falai:
                result = await generateFalaiImage(
                    prefixedPrompt,
                    negativePrompt,
                    signal,
                );
...
async function generateAutoImage(prompt, negativePrompt, signal) {
    ...
    let payload = {
        ...
        override_settings: {
            ...
            forge_additional_modules: isValidVae
                ? [extension_settings.sd.vae]
                : undefined, // For SD Forge
        },
...
async function generateOpenAiImage(prompt, signal) {
    ...
    if (
        isDalle2 &&
        extension_settings.sd.width <= 512 &&
        extension_settings.sd.height <= 512
    ) {
        width = 512;
        height = 512;
    }
...
async function generateComfyImage(prompt, negativePrompt, signal) {
    ...
    let workflow = (await workflowResponse.json()).replaceAll(
        '"%prompt%"',
    ...
    workflow = workflow.replaceAll(
        '"%denoise%"',
        JSON.stringify(denoising_strength),
    );

    const clip_skip = isNaN(extension_settings.sd.clip_skip)
        ? -1
        : -extension_settings.sd.clip_skip;
    workflow = workflow.replaceAll('"%clip_skip%"', JSON.stringify(clip_skip));

    placeholders.forEach((ph) => {
        workflow = workflow.replaceAll(
            `"%${ph}%"`,
            JSON.stringify(extension_settings.sd[ph]),
        );
    });
    (extension_settings.sd.comfy_placeholders ?? []).forEach((ph) => {
        workflow = workflow.replaceAll(
            `"%${ph.find}%"`,
            JSON.stringify(substituteParams(ph.replace)),
        );
    });
    if (/%user_avatar%/gi.test(workflow)) {
        ...
        if (response.ok) {
            ...
            workflow = workflow.replaceAll(
                '"%user_avatar%"',
                JSON.stringify(avatarBase64),
            );
        } else {
            workflow = workflow.replaceAll(
                '"%user_avatar%"',
                JSON.stringify(PNG_PIXEL),
            );
        }
    }
...
async function onComfyOpenWorkflowEditorClick() {
    ...
    const editorHtml = $(
        await $.get(
            "scripts/extensions/stable-diffusion/comfyWorkflowEditor.html",
        ),
    );
    ...
    const checkPlaceholders = () => {
        workflow = $("#sd_comfy_workflow_editor_workflow").val().toString();
        $(
            ".sd_comfy_workflow_editor_placeholder_list > li[data-placeholder]",
        ).each(function () {
            const key = this.getAttribute("data-placeholder");
            const found = workflow.search(`"%${key}%"`) !== -1;
            this.classList[found ? "remove" : "add"](
                "sd_comfy_workflow_editor_not_found",
            );
        });
    };
    $("#sd_comfy_workflow_editor_name").text(
    ...
    const addPlaceholderDom = (placeholder) => {
        ...
        el.find(".sd_comfy_workflow_editor_custom_find").on(
            "input",
            function () {
                ...
                el.find(".sd_comfy_workflow_editor_custom_final").text(
                    `"%${this.value}%"`,
                );
        ...
        el.find(".sd_comfy_workflow_editor_custom_replace").val(
            placeholder.replace,
        );
        el.find(".sd_comfy_workflow_editor_custom_replace").on(
            "input",
            function () {
                if (!(this instanceof HTMLInputElement)) {
                    return;
...
async function onComfyDeleteWorkflowClick() {
    const confirm = await callGenericPopup(
        "Delete the workflow? This action is irreversible.",
        POPUP_TYPE.CONFIRM,
        "",
        { okButton: "Delete", cancelButton: "Cancel" },
    );
...
async function sendMessage(
    ...
    const template =
        extension_settings.sd.prompts[generationMode.MESSAGE] || "{{prompt}}";
...
async function addSDGenButtons() {
    ...
    const dropdownHtml = await renderExtensionTemplateAsync(
        "stable-diffusion",
        "dropdown",
    );
    ...
    $(document).on("click touchend", function (e) {
        ...
        if (
            (target.is(button) || target.closest(button).length) &&
            !dropdown.is(":visible")
        ) {
            e.preventDefault();
...
async function sdMessageButton(e) {
    ...
    try {
        ...
        if (hasSavedImage) {
            ...
            const negative = hasSavedNegative
                ? await refinePrompt(message.extra.negative, true)
                : "";
            message.extra.title = prompt;

            const generationType =
                message?.extra?.generationType ?? generationMode.FREE;
            console.log(
    ...
    function saveGeneratedImage(prompt, image, generationType, negative) {
        ...
        message.extra.inline_image = !(
            message.extra.image && !message.extra.inline_image
        );
...
async function onImageSwiped({ message, element, direction }) {
    ...
    if (direction === "right") {
        ...
        if (newIndex === swipes.length) {
            ...
            extension_settings.sd.seed =
                extension_settings.sd.seed >= 0
                    ? Math.round(Math.random() * (Math.pow(2, 32) - 1))
                    : -1;
            let imagePath = "";
            ...
            try {
                ...
                const prompt = await refinePrompt(message.extra.title, false);
                const negativePromptPrefix = hasNegative
                    ? await refinePrompt(message.extra.negative, true)
                    : "";
                const characterName = context.groupId
                    ? context.groups[
...
function applyCommandArguments(args) {
    ...
    for (const [param, setting] of Object.entries(settingMap)) {
        if (
            ...
            defaultSettings[setting] === undefined
        ) {
            continue;
        }
        ...
        switch (type) {
            case "boolean":
                overrideSettings[setting] =
                    isTrueBoolean(value) || !isFalseBoolean(value);
                break;
            case "number":
...
jQuery(async () => {
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            name: "imagine",
            returns:
                "URL of the generated image, or an empty string if the generation failed",
            callback: async (args, trigger) => {
                const currentSettings = applyCommandArguments(args);

                try {
                    return await generatePicture(
                        initiators.command,
                        args,
                        String(trigger),
                    );
            ...
            namedArgumentList: [
                ...
                SlashCommandNamedArgument.fromProps({
                    name: "multimodal",
                    description:
                        "use multimodal captioning (for portraits only)",
                    typeList: [ARGUMENT_TYPE.BOOLEAN],
                    enumProvider: commonEnumProviders.boolean("trueFalse"),
                    isRequired: false,
                    acceptsMultiple: false,
                ...
                SlashCommandNamedArgument.fromProps({
                    ...
                    acceptsMultiple: false,
                    forceEnum: true,
                    enumProvider: getSelectEnumProvider(
                        "sd_hr_upscaler",
                        false,
                    ),
                }),
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            ...
            callback: async (_args, name) => {
                ...
                const isKnownSource = Object.keys(sources).includes(
                    String(name),
                );
                if (!isKnownSource) {
                    throw new Error(
                        "The value provided is not a valid image generation source.",
                    );
                }
                const option = document.querySelector(
                    `#sd_source [value="${name}"]`,
                );
                if (!(option instanceof HTMLOptionElement)) {
                    throw new Error(
                        "Could not find the source option in the dropdown.",
                    );
                }
                option.selected = true;
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            ...
            callback: async (_args, name) => {
                ...
                if (!(option instanceof HTMLOptionElement)) {
                    throw new Error(
                        "Could not find the style option in the dropdown.",
                    );
                }
                option.selected = true;
                onStyleSelect();
    ...
    $("#sd_character_negative_prompt").on(
        "input",
        onCharacterNegativePromptInput,
    );
    ...
    $("#sd_comfy_open_workflow_editor").on(
        "click",
        onComfyOpenWorkflowEditorClick,
    );
    ...
    const getMacroValue = ({ isNegative }) => {
        ...
        const context = getContext();
        const sharedPromptData =
            context?.characters[this_chid]?.data?.extensions
                ?.sd_character_prompt;
    ...
    MacrosParser.registerMacro(
        ...
        t`Character's positive positive Image Generation prompt prefix`,
    );
    MacrosParser.registerMacro(
        "charNegativePrefix",
