...
async function getSourceContextSize() {
    ...
    if (extension_settings.memory.source === summary_sources.webllm) {
        ...
        return overrideLength > 0
            ? maxContext - overrideLength
            : Math.round(maxContext * 0.75);
    }
...
const formatMemoryValue = function (value) {
    ...
    if (extension_settings.memory.template) {
        return substituteParamsExtended(extension_settings.memory.template, {
            summary: value,
        });
    } else {
...
function loadSettings() {
    ...
    $("#memory_frozen")
        ...
        .trigger("input");
    $("#memory_skipWIAN")
        .prop("checked", extension_settings.memory.SkipWIAN)
        .trigger("input");
    ...
    $("#memory_max_messages_per_request")
        ...
        .trigger("input");
    $("#memory_include_wi_scan")
        .prop("checked", extension_settings.memory.scan)
        .trigger("input");
...
async function onPromptForceWordsAutoClick() {
    ...
    const allMessages = chat
        ...
        .map((m) => m.mes);
    const messagesWordCount = allMessages
        .map((m) => extractAllWords(m))
        .flat().length;
    const averageMessageWordCount = messagesWordCount / allMessages.length;
    const tokensPerWord =
        (await countSourceTokens(allMessages.join("\n"))) / messagesWordCount;
    ...
    const summaryPromptWords = extractAllWords(
        extension_settings.memory.prompt,
    ).length;
    ...
    const averageMessagesPerPrompt = Math.floor(
        promptAllowanceWords / averageMessageWordCount,
    );
    const maxMessagesPerSummary =
        extension_settings.memory.maxMessagesPerRequest || 0;
    const targetMessagesInPrompt =
        maxMessagesPerSummary > 0
            ? maxMessagesPerSummary
            : Math.max(0, averageMessagesPerPrompt);
...
async function onPromptIntervalAutoClick() {
    ...
    const targetSummaryTokens = Math.round(
        extension_settings.memory.promptWords * tokensPerWord,
    );
    const promptTokens = await countSourceTokens(
        extension_settings.memory.prompt,
    );
    const promptAllowance =
        maxPromptLength - promptTokens - targetSummaryTokens;
    const maxMessagesPerSummary =
        extension_settings.memory.maxMessagesPerRequest || 0;
    const averageMessagesPerPrompt = Math.floor(
    ...
    const adjustedAverageMessagesPerPrompt =
        targetMessagesInPrompt +
        (averageMessagesPerPrompt - targetMessagesInPrompt) / 4;
...
function onMaxMessagesPerRequestInput() {
    ...
    $("#memory_max_messages_per_request_value").text(
...
async function onChatEvent() {
    ...
    if (
        extension_settings.memory.source === summary_sources.webllm &&
        !isWebLlmSupported()
    ) {
        return;
    }
    ...
    summarizeChat(context)
        ...
        .finally(() => {
            ...
            lastMessageHash = getStringHash(
                (context.chat.length &&
                    context.chat[context.chat.length - 1]["mes"]) ??
                    "",
            );
        });
}
...
async function summarizeCallback(args, text) {
    ...
    const prompt = substituteParamsExtended(
        args.prompt || extension_settings.memory.prompt,
...
async function summarizeChatWebLLM(context, force) {
    ...
    const { rawPrompt, lastUsedIndex } = await getRawSummaryPrompt(
        context,
        prompt,
    );
    ...
    if (lastUsedIndex === null || lastUsedIndex === -1) {
        if (force) {
            toastr.info(
                "To try again, remove the latest summary.",
...
async function summarizeChatMain(context, force, skipWIAN) {
    ...
    if (prompt_builders.DEFAULT === extension_settings.memory.prompt_builder) {
        try {
            inApiCall = true;
            summary = await generateQuietPrompt(
                prompt,
                false,
                skipWIAN,
                "",
    ...
    if (
        [
            prompt_builders.RAW_BLOCKING,
            prompt_builders.RAW_NON_BLOCKING,
        ].includes(extension_settings.memory.prompt_builder)
    ) {
        const lock =
...
function doPopout(e) {
    ...
    if ($("#summaryExtensionPopout").length === 0) {
        ...
        $("#summaryExtensionPopoutClose")
            .off("click")
            .on("click", function () {
                $("#summaryExtensionDrawerContents").removeClass(
                    "scrollableInnerFull",
                );
...
function setupListeners() {
    ...
    $("#memory_prompt_words")
        .off("input")
        .on("input", onMemoryPromptWordsInput);
    $("#memory_prompt_interval")
        .off("input")
        .on("input", onMemoryPromptIntervalInput);
    ...
    $('input[name="memory_position"]')
        .off("change")
        .on("change", onMemoryPositionChange);
    $("#memory_prompt_words_force")
        .off("input")
        .on("input", onMemoryPromptWordsForceInput);
    $("#memory_prompt_builder_default")
        .off("input")
        .on("input", onMemoryPromptBuilderInput);
    $("#memory_prompt_builder_raw_blocking")
        .off("input")
        .on("input", onMemoryPromptBuilderInput);
    $("#memory_prompt_builder_raw_non_blocking")
        .off("input")
        .on("input", onMemoryPromptBuilderInput);
    $("#memory_prompt_restore")
        .off("click")
        .on("click", onMemoryPromptRestoreClick);
    $("#memory_prompt_interval_auto")
        .off("click")
        .on("click", onPromptIntervalAutoClick);
    $("#memory_prompt_words_auto")
        .off("click")
        .on("click", onPromptForceWordsAutoClick);
    $("#memory_override_response_length")
        .off("input")
        .on("input", onOverrideResponseLengthInput);
    $("#memory_max_messages_per_request")
        .off("input")
        .on("input", onMaxMessagesPerRequestInput);
    $("#memory_include_wi_scan")
        .off("input")
        .on("input", onMemoryIncludeWIScanInput);
        ...
        SlashCommand.fromProps({
            ...
            namedArgumentList: [
                ...
                SlashCommandNamedArgument.fromProps({
                    name: "quiet",
                    description:
                        "suppress the toast message when summarizing the chat",
