...
function getAttachmentByField(attachments, value) {
    const match = (a) =>
        String(a).trim().toLowerCase() === String(value).trim().toLowerCase();
...
function listDataBankAttachments(args) {
    ...
    return JSON.stringify(
        attachments.map((a) => (FIELDS.includes(field) ? a[field] : a.url)),
    );
}
...
async function uploadDataBankAttachment(args, value) {
    const source =
        args?.source && TYPES.includes(args.source) ? args.source : "chat";
...
async function updateDataBankAttachment(args, value) {
    ...
    const attachment = getAttachmentByFields(attachments, [
        args?.url,
        args?.name,
    ]);
...
async function enableDataBankAttachment(args, value) {
    ...
    const index = extension_settings.disabled_attachments.indexOf(
        attachment.url,
    );
    if (index === -1) {
        return "";
...
jQuery(async () => {
    ...
    const manageButton = await renderExtensionTemplateAsync(
        "attachments",
        "manage-button",
        {},
    );
    const attachButton = await renderExtensionTemplateAsync(
        "attachments",
        "attach-button",
        {},
    );
    ...
    const localEnumProviders = {
        ...
        attachments:
            ...
            (/** @type {SlashCommandExecutor} */ executor) => {
                const source =
                    executor.namedArgumentList.find((it) => it.name == "source")
                        ?.value ?? fallbackSource;
                if (source instanceof SlashCommandClosure)
                    throw new Error(
                        "Argument 'source' does not support closures",
                    );
                const attachments = getAttachments(source);
