...
function getRegexedString(
    ...
    if (typeof rawString !== "string") {
        console.warn(
            "getRegexedString: rawString is not a string. Returning empty string.",
        );
        return "";
    }
...
function runRegexScript(regexScript, rawString, { characterOverride } = {}) {
    ...
    const getRegexString = () => {
        switch (Number(regexScript.substituteRegex)) {
            ...
            case substitute_find_regex.ESCAPED:
                return substituteParamsExtended(
                    regexScript.findRegex,
                    {},
                    sanitizeRegexMacro,
                );
    ...
    newString = rawString.replace(findRegex, function (match) {
        ...
        const replaceString = regexScript.replaceString.replace(
            /{{match}}/gi,
            "$0",
        );
        const replaceWithGroups = replaceString.replaceAll(
            ...
            (_, num) => {
                ...
                const filteredMatch = filterString(
                    match,
