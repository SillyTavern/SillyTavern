...
const sanitizeFileName = (name) =>
    name.replace(/[\s.<>:"/\\|?*\x00-\x1F\x7F]/g, "_").toLowerCase();
...
function initializeData() {
    ...
    if (!extension_settings.regex_folders)
...
async function saveRegexScript(regexScript, existingScriptIndex, isScoped) {
    ...
    if (!regexScript.scriptName) {
        toastr.error(
            t`Could not save regex script: The script name was undefined or empty!`,
        );
        return;
    }
    ...
    if (isScoped) {
        ...
        if (
            !extension_settings.character_allowed_regex.includes(
                characters[this_chid].avatar,
            )
        ) {
            extension_settings.character_allowed_regex.push(
                characters[this_chid].avatar,
            );
        }
    }
...
async function cloneRegexScript({ id, isScoped }) {
    ...
    await loadRegexScripts();
    toastr.success(
        t`Script "${originalScript.scriptName}" cloned successfully.`,
    );
}
...
async function deleteRegexScript({ id, isScoped }) {
    const scriptList =
        (isScoped && this_chid
            ? characters[this_chid]?.data?.extensions?.regex_scripts
            : extension_settings.regex) ?? [];
    const existingScriptIndex = scriptList.findIndex(
        (script) => script.id === id,
    );
...
async function loadRegexScripts() {
    ...
    const folderTemplate = $(
        await renderExtensionTemplateAsync("regex", "folderTemplate"),
    );
    ...
    function renderScript(container, script) {
        ...
        scriptHtml
            ...
            .on("click", () => onRegexEditorOpenClick(script.id, isScoped));
        ...
        scriptHtml
            ...
            .on("click", async function () {
                if (this_chid === undefined)
                    return toastr.error(t`No character selected.`);
                if (selected_group)
                    return toastr.error(
                        t`Cannot edit scoped scripts in group chats.`,
                    );
                const confirm = await callGenericPopup(
        ...
        scriptHtml
            .find(".clone_regex")
            .on("click", () => cloneRegexScript({ id: script.id, isScoped }));
    ...
    function renderFolder(container, folder) {
        ...
        const scriptsInFolder = getRegexScripts().filter((s) =>
            folder.scriptIds.includes(s.id),
        );
        const allDisabled =
            scriptsInFolder.length > 0 &&
            scriptsInFolder.every((s) => s.disabled);

        folderHtml.attr("id", folder.id);
        folderHtml.data("folderId", folder.id);
        folderHtml.find(".folder-name").text(folder.name);
        folderHtml
            .find(".folder-script-count")
            .text(`(${scriptsInFolder.length})`);
        ...
        folderHtml.find(".folder-header").on("click", function (event) {
            if (
                $(event.target).closest(".folder-actions").length === 0 &&
                $(event.target).closest(".drag-handle").length === 0
            ) {
        ...
        folderHtml
            .find(".toggle-folder-scripts-enable")
            .on("click", () => toggleAction(true));
        folderHtml
            .find(".toggle-folder-scripts-disable")
            .on("click", () => toggleAction(false));

        folderHtml
            .find(".edit-folder")
            .on("click", () => onFolderEditorOpenClick(folder.id));

        folderHtml.find(".delete-folder").on("click", async function () {
            const confirm = await callGenericPopup(
                t`Are you sure you want to delete this folder? The scripts inside will NOT be deleted.`,
                POPUP_TYPE.CONFIRM,
            );
            if (confirm) {
                const folderIndex = extension_settings.regex_folders.findIndex(
                    (f) => f.id === folder.id,
                );
                if (folderIndex > -1) {
    ...
    const allScriptIdsInFolders = (
        extension_settings.regex_folders ?? []
    ).flatMap((f) => f.scriptIds);

    (extension_settings.regex_folders ?? []).forEach((folder) =>
        renderFolder(foldersContainer, folder),
    );

    globalScripts
        .filter((s) => !allScriptIdsInFolders.includes(s.id))
        .forEach((script) => renderScript(globalScriptsContainer, script));
    scopedScripts
        .filter((s) => !allScriptIdsInFolders.includes(s.id))
        .forEach((script) => renderScript(scopedScriptsContainer, script));

    const isAllowed = extension_settings?.character_allowed_regex?.includes(
        characters?.[this_chid]?.avatar,
    );
...
async function onFolderEditorOpenClick(folderId = null) {
    const editorHtml = $(
        await renderExtensionTemplateAsync("regex", "folderEditor"),
    );
    const isEdit = folderId !== null;
    let folder = isEdit
        ? extension_settings.regex_folders.find((f) => f.id === folderId)
        : null;
...
async function onRegexEditorOpenClick(existingId, isScoped) {
    ...
    if (existingId) {
        existingScriptIndex = array.findIndex(
            (script) => script.id === existingId,
        );
        if (existingScriptIndex !== -1) {
            ...
            editorHtml
                ...
                .val(
                    existingScript.substituteRegex ??
                        substitute_find_regex.NONE,
                );
            editorHtml
                .find('input[name="min_depth"]')
                .val(existingScript.minDepth ?? "");
            editorHtml
                .find('input[name="max_depth"]')
                .val(existingScript.maxDepth ?? "");
    ...
    try {
        ...
        flagInfo.push(
            regex.flags.includes("g")
                ? t`Applies to all matches`
                : t`Applies to the first match`,
        );
        flagInfo.push(
            regex.flags.includes("i") ? t`Case insensitive` : t`Case sensitive`,
        );
...
async function toggleRegexCallback(args, scriptName) {
    ...
    const script = scripts.find((s) =>
        equalsIgnoreCaseAndAccents(s.scriptName, scriptName),
    );
    ...
    const isScoped =
        this_chid &&
        characters[this_chid]?.data?.extensions?.regex_scripts?.some(
            (s) => s.id === script.id,
        );
    const array = isScoped
        ? characters[this_chid].data.extensions.regex_scripts
        : extension_settings.regex;
    const index = array.findIndex((s) => s.id === script.id);
...
async function onRegexImportFileChange(file, isScoped) {
    ...
    try {
        ...
        const scriptsToImport = Array.isArray(regexScripts)
            ? regexScripts
            : [regexScripts];
...
function purgeEmbeddedRegexScripts({ character }) {
    ...
    if (
        ...
        const index =
            extension_settings.character_allowed_regex.indexOf(avatar);
        if (index !== -1) {
    ...
    $(".regex-script-container")
        .sortable({
            ...
            stop: async function (event, ui) {
                ...
                const newFolderId = newContainer
                    .closest(".regex-folder")
                    .data("folderId");

                if (newFolderId) {
                    const folder = allFolders.find((f) => f.id === newFolderId);
                    if (folder) {
                        const newScriptIds = newContainer
                            .children()
                            .map(function () {
                    ...
                    const globalDomOrder = $("#saved_regex_scripts")
                        .children()
                        .map(function () {
                            return $(this).data("scriptId");
                        })
                        .get();
                    const newGlobalOrder = globalDomOrder
                        .map((id) => allScripts.find((s) => s.id === id))
                        .filter(Boolean);
                    const scriptsInGlobalFolders = allFolders
                        .flatMap((f) => f.scriptIds)
                        .map((id) => allScripts.find((s) => s.id === id))
                        .filter((s) => s && allGlobalScripts.includes(s));
                    extension_settings.regex = [
                        ...scriptsInGlobalFolders,
                        ...newGlobalOrder,
                    ];

                    if (this_chid) {
                        const scopedDomOrder = $("#saved_scoped_scripts")
                            .children()
                            .map(function () {
                                return $(this).data("scriptId");
                            })
                            .get();
                        const newScopedOrder = scopedDomOrder
                            .map((id) => allScripts.find((s) => s.id === id))
                            .filter(Boolean);
                        const scriptsInScopedFolders = allFolders
                            .flatMap((f) => f.scriptIds)
                            .map((id) => allScripts.find((s) => s.id === id))
                            .filter((s) => s && allScopedScripts.includes(s));
                        const finalScoped = [
                            ...scriptsInScopedFolders,
                            ...newScopedOrder,
                        ];
                        await writeExtensionField(
                            this_chid,
...
jQuery(async () => {
    ...
    const settingsHtml = $(
        await renderExtensionTemplateAsync("regex", "dropdown"),
    );
    ...
    $("#regex_expand_all").on("click", async () => {
        extension_settings.regex_folders.forEach(
            (f) => (f.isCollapsed = false),
        );
        saveSettingsDebounced();
        await loadRegexScripts();
    });
    ...
    $("#open_scoped_editor").on("click", () => {
        if (this_chid === undefined)
            return toastr.error(t`No character selected.`);
        if (selected_group)
            return toastr.error(t`Cannot edit scoped scripts in group chats.`);
        onRegexEditorOpenClick(false, true);
    });
    $("#import_regex_file").on("change", async function () {
        let target = "global";
        const template = $(
            await renderExtensionTemplateAsync("regex", "importTarget"),
        );
        template
            .find("#regex_import_target_global")
            .on("input", () => (target = "global"));
        template
            .find("#regex_import_target_scoped")
    ...
    function getSelectedScripts() {
        const scripts = getRegexScripts();
        const selector =
            "#regex_container .regex-script-label:has(.regex_bulk_checkbox:checked)";
        const selectedIds = Array.from(document.querySelectorAll(selector))
            .map((e) => $(e).data("scriptId"))
            .filter((id) => id);
    ...
    $("#bulk_enable_regex").on("click", async () => {
        const scripts = getSelectedScripts().filter(
            (script) => script.disabled,
        );
        if (scripts.length === 0)
            return toastr.warning(t`No regex scripts selected for enabling.`);
    ...
    $("#bulk_disable_regex").on("click", async () => {
        ...
        if (scripts.length === 0)
            return toastr.warning(t`No regex scripts selected for disabling.`);
    ...
    $("#bulk_delete_regex").on("click", async () => {
        ...
        if (scripts.length === 0)
            return toastr.warning(t`No regex scripts selected for deletion.`);
        const confirm = await callGenericPopup(
    ...
    $("#bulk_export_regex").on("click", async () => {
        const scripts = getSelectedScripts();
        if (scripts.length === 0)
            return toastr.warning(t`No regex scripts selected for export.`);
    ...
    const localEnumProviders = {
        regexScripts: () =>
            getRegexScripts().map((script) => {
                const isGlobal = (extension_settings.regex ?? []).some(
                    (x) => x.id === script.id,
                );
                return new SlashCommandEnumValue(
                    script.scriptName,
                    `${enumIcons.getStateIcon(!script.disabled)} [${isGlobal ? "global" : "scoped"}] ${script.findRegex}`,
                    isGlobal ? enumTypes.enum : enumTypes.name,
                    isGlobal ? "G" : "S",
                );
            }),
    };
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            ...
            helpString:
                '<div>Toggles the state of a specified regex script.</div><div><strong>Example:</strong><ul><li><pre><code class="language-stscript">/regex-toggle MyScript</code></pre></li><li><pre><code class="language-stscript">/regex-toggle state=off Character-specific Script</code></pre></li></ul></div>',
        }),
    );
