...
async function loadRegexScripts() {
    ...
    function renderFolder(container, folder) {
        ...
        folderHtml.find('.rename-folder').on('click', async function () {
            const newName = await callGenericPopup(t`Enter new folder name:`, POPUP_TYPE.INPUT, folder.name);
...
async function createFolder() {
    const folderName = await callGenericPopup(t`Enter folder name:`, POPUP_TYPE.INPUT, 'New Folder');
...
function makeSortable() {
    $('.regex-script-container').sortable({
        ...
        stop: async function (event, ui) {
            ...
            // Remove the script from its original folder, if any
            ...
            if (newFolderId) {
                // The item was dropped into a folder
                ...
                // The item was dropped into a root container.
                // We need to update the main arrays based on the new DOM order.
...
jQuery(async () => {
    ...
    $('#create_folder').on('click', () => createFolder());
