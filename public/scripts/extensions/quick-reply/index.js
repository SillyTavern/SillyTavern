...
export const debug = (...msg) =>
    _VERBOSE ? console.debug("[QR2]", ...msg) : null;
export const log = (...msg) => (_VERBOSE ? console.log("[QR2]", ...msg) : null);
export const warn = (...msg) =>
    _VERBOSE ? console.warn("[QR2]", ...msg) : null;
...
const loadSets = async () => {
    ...
    if (response.ok) {
        ...
        for (const set of setList) {
            if (set.version !== 2) {
                ...
                set.qrList = set.quickReplySlots.map((slot, idx) => {
                    ...
                    qr.executeOnGroupMemberDraft =
                        slot.autoExecute_groupMemberDraft ?? false;
            ...
            if (set.version == 2) {
                QuickReplySet.list.push(
                    QuickReplySet.from(JSON.parse(JSON.stringify(set))),
                );
            }
        }
        // need to load QR lists after all sets are loaded to be able to resolve context menu entries
        setList.forEach((set, idx) => {
            QuickReplySet.list[idx].qrList = set.qrList.map((it) =>
                QuickReply.from(it),
            );
    ...
    if (!extension_settings.quickReplyV2) {
        if (!extension_settings.quickReply) {
            ...
            extension_settings.quickReplyV2 = {
                isEnabled:
                    extension_settings.quickReply.quickReplyEnabled ?? false,
                isCombined: false,
                isPopout: false,
                config: {
                    setList: [
...
const init = async () => {
    ...
    window["executeQuickReplyByName"] = async (
        name,
        args = {},
        options = {},
    ) => {
...
const onChatChanged = async (chatIdx) => {
    ...
    if (chatIdx) {
        settings.chatConfig = QuickReplyConfig.from(
            chat_metadata.quickReply ?? {},
        );
    } else {
...
eventSource.on(event_types.CHAT_CREATED, (...args) =>
