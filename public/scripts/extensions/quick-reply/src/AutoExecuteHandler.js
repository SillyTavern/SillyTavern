...
export class AutoExecuteHandler {
    ...
    checkExecute() {
        return (
            ...
            !this.preventAutoExecuteStack.slice(-1)[0]
        );
    }
    ...
    async handleStartup() {
        ...
        const qrList = [
            ...this.settings.config.setList
                .map((link) =>
                    link.set.qrList.filter((qr) => qr.executeOnStartup),
                )
                .flat(),
    ...
    async handleUser() {
        ...
        const qrList = [
            ...this.settings.config.setList
                .map((link) => link.set.qrList.filter((qr) => qr.executeOnUser))
                .flat(),
            ...(this.settings.chatConfig?.setList
                ?.map((link) =>
                    link.set.qrList.filter((qr) => qr.executeOnUser),
                )
                ?.flat() ?? []),
        ];
        await this.performAutoExecute(qrList);
    ...
    async handleAi() {
        ...
        const qrList = [
            ...this.settings.config.setList
                .map((link) => link.set.qrList.filter((qr) => qr.executeOnAi))
                .flat(),
            ...(this.settings.chatConfig?.setList
                ?.map((link) => link.set.qrList.filter((qr) => qr.executeOnAi))
                ?.flat() ?? []),
    ...
    async handleChatChanged() {
        ...
        const qrList = [
            ...this.settings.config.setList
                .map((link) =>
                    link.set.qrList.filter((qr) => qr.executeOnChatChange),
                )
                .flat(),
            ...(this.settings.chatConfig?.setList
                ?.map((link) =>
            ...
            ...this.settings.config.setList
                .map((link) =>
                    link.set.qrList.filter(
                        (qr) => qr.executeOnGroupMemberDraft,
                    ),
                )
                .flat(),
            ...(this.settings.chatConfig?.setList
    ...
    async handleNewChat() {
        ...
        const qrList = [
            ...this.settings.config.setList
                .map((link) =>
                    link.set.qrList.filter((qr) => qr.executeOnNewChat),
                )
                .flat(),
            ...(this.settings.chatConfig?.setList
                ?.map((link) =>
            ...
            ...this.settings.config.setList
                .map((link) =>
                    link.set.qrList.filter(
                        (qr) =>
                            qr.automationId &&
                            automationIds.includes(qr.automationId),
                    ),
                )
                .flat(),
            ...(this.settings.chatConfig?.setList
