...
export class SlashCommandHandler {
    ...
    init() {
        ...
        const localEnumProviders = {
            ...
            qrEntries: (executor) =>
                QuickReplySet.get(
                    String(
                        executor.namedArgumentList.find((x) => x.name == "set")
                            ?.value,
                    ),
                )?.qrList.map((qr) => {
            ...
            qrExecutables: () => {
                ...
                const globalQrs = globalSetList
                    .map((link) =>
                        link.set.qrList.map((qr) => ({ set: link.set, qr })),
                    )
                    .flat();
                const chatQrs =
                    chatSetList
                        ?.map((link) =>
                            link.set.qrList.map((qr) => ({
                                set: link.set,
                                qr,
                            })),
                        )
        ...
        SlashCommandParser.addCommandObject(
            SlashCommand.fromProps({
                name: "qr",
                callback: (_, value) =>
                    this.executeQuickReplyByIndex(Number(value)),
                unnamedArgumentList: [
        ...
        SlashCommandParser.addCommandObject(
            SlashCommand.fromProps({
                name: "qr-set-list",
                callback: (_, value) =>
                    JSON.stringify(this.listSets(value ?? "all")),
        ...
        SlashCommandParser.addCommandObject(
            SlashCommand.fromProps({
                ...
                helpString:
                    "Gets a list of the names of all quick replies in this quick reply set.",
            }),
        );
            ...
            SlashCommand.fromProps({
                ...
                namedArgumentList: [
                    SlashCommandNamedArgument.fromProps({
                        name: "set",
                        description:
                            "name of the QR set, e.g., set=PresetName1",
                        typeList: [ARGUMENT_TYPE.STRING],
                        isRequired: true,
                        enumProvider: localEnumProviders.qrSets,
                    }),
            ...
            SlashCommand.fromProps({
                ...
                namedArgumentList: [
                    ...
                    ...qrArgs.map((it) => {
                        if (it.name == "label") {
                            const clone =
                                SlashCommandNamedArgument.fromProps(it);
            ...
            SlashCommand.fromProps({
                ...
                namedArgumentList: [
                    SlashCommandNamedArgument.fromProps({
                        name: "set",
                        description:
                            "Name of QR set to add the context menu to",
                        typeList: [ARGUMENT_TYPE.STRING],
                        isRequired: true,
                        enumProvider: localEnumProviders.qrSets,
                    }),
                    SlashCommandNamedArgument.fromProps({
                        name: "label",
                        description:
                            "Label of Quick Reply to add the context menu to",
                        typeList: [ARGUMENT_TYPE.STRING],
                        enumProvider: localEnumProviders.qrEntries,
                    }),
                    SlashCommandNamedArgument.fromProps({
                        name: "id",
                        description:
                            "Numeric ID of Quick Reply to add the context menu to, e.g. id=42",
                        typeList: [ARGUMENT_TYPE.NUMBER],
                        enumProvider: localEnumProviders.qrIds,
                    }),
                    new SlashCommandNamedArgument(
            ...
            SlashCommand.fromProps({
                ...
                namedArgumentList: [
                    SlashCommandNamedArgument.fromProps({
                        name: "set",
                        description:
                            "Name of QR set to remove the context menu from",
                        typeList: [ARGUMENT_TYPE.STRING],
                        isRequired: true,
                        enumProvider: localEnumProviders.qrSets,
                    }),
                    SlashCommandNamedArgument.fromProps({
                        name: "label",
                        description:
                            "Label of Quick Reply to remove the context menu from",
                        typeList: [ARGUMENT_TYPE.STRING],
                        enumProvider: localEnumProviders.qrEntries,
                    }),
                    SlashCommandNamedArgument.fromProps({
                        name: "id",
                        description:
                            "Numeric ID of Quick Reply to remove the context menu from, e.g. id=42",
                        typeList: [ARGUMENT_TYPE.NUMBER],
                        enumProvider: localEnumProviders.qrIds,
                    }),
                ],
        ...
        SlashCommandParser.addCommandObject(
            SlashCommand.fromProps({
                ...
                callback: (args, value) => {
                    if (!args.from)
                        throw new Error("/import requires from= to be set.");
                    if (!value)
                        throw new Error(
                            "/import requires the unnamed argument to be set.",
                        );
                    let qr = [
                        ...this.api.listGlobalSets(),
                        ...this.api.listChatSets(),
                    ]
                    ...
                    if (qr) {
                        ...
                        const testCandidates = (executor) => {
                            return (
                                (executor.namedArgumentList.find(
                                    ...
                                    executor.unnamedArgumentList[0]
                                        .value instanceof
                                        SlashCommandClosure) ||
                                (!executor.namedArgumentList.find(
                                    (arg) => arg.name == "key",
                        ...
                        const candidates = closure.executorList
                            ...
                            .map((executor) => ({
                                ...
                                value: executor.unnamedArgumentList[
                                    executor.namedArgumentList.find(
                                        (arg) => arg.name == "key",
                                    )
                                        ? 0
                        ...
                        for (let i = 0; i < value.length; i++) {
                            ...
                            if (!pick)
                                throw new Error(
                                    `No scoped closure named "${srcName}" found in "${args.from}"`,
                                );
                ...
                namedArgumentList: [
                    SlashCommandNamedArgument.fromProps({
                        name: "from",
                        description:
                            "Quick Reply to import from (QRSet.QRLabel)",
                        typeList: ARGUMENT_TYPE.STRING,
                        isRequired: true,
                    }),
                ],
    ...
    getSetByName(name) {
        ...
        if (!set) {
            toastr.error(
                `No Quick Reply Set with the name "${name}" could be found.`,
            );
        }
        return set;
    }
    ...
    toggleGlobalSet(name, args = {}) {
        try {
            this.api.toggleGlobalSet(
                name,
                isTrueBoolean(args.visible ?? "true"),
            );
        } catch (ex) {
            toastr.error(ex.message);
        }
                ...
                args.id !== undefined ? Number(args.id) : (args.label ?? ""),
                {
                    ...
                    isHidden:
                        args.hidden === undefined
                            ? undefined
                            : isTrueBoolean(args.hidden),
                    executeOnStartup:
                        args.startup === undefined
                            ? undefined
                            : isTrueBoolean(args.startup),
                    executeOnUser:
                        args.user === undefined
                            ? undefined
                            : isTrueBoolean(args.user),
                    executeOnAi:
                        args.bot === undefined
                            ? undefined
                            : isTrueBoolean(args.bot),
                    executeOnChatChange:
                        args.load === undefined
                            ? undefined
                            : isTrueBoolean(args.load),
                    executeOnGroupMemberDraft:
                        args.group === undefined
                            ? undefined
                            : isTrueBoolean(args.group),
                    executeOnNewChat:
                        args.new === undefined
                            ? undefined
                            : isTrueBoolean(args.new),
                    automationId: args.automationId ?? "",
                },
            );
        } catch (ex) {
    ...
    async updateSet(name, args) {
        try {
            await this.api.updateSet(args.name ?? name ?? "", {
                disableSend:
                    args.nosend !== undefined
                        ? isTrueBoolean(args.nosend)
                        : undefined,
                placeBeforeInput:
                    args.before !== undefined
                        ? isTrueBoolean(args.before)
                        : undefined,
                injectInput:
                    args.inject !== undefined
                        ? isTrueBoolean(args.inject)
                        : undefined,
            });
        } catch (ex) {
            toastr.error(ex.message);
