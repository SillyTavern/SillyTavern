...
export class QuickReply {
     ...
     */
    static from(props) {
        props.contextList = (props.contextList ?? []).map(
            (/** @type {any} */ it) => QuickReplyContextLink.from(it),
        );
    ...
    get hasContext() {
        return (
            this.contextList &&
            this.contextList.filter((it) => it.set).length > 0
        );
    }
        ...
        if (!this.dom) {
            const root = document.createElement("div");
            {
                ...
                const lbl = document.createElement("div");
                {
                    ...
                    if (this.icon && !this.showLabel)
                        lbl.classList.add("qr--hidden");
            ...
            const item = document.createElement("div");
            {
                ...
                const actions = document.createElement("div");
                {
                    ...
                    const cut = document.createElement("div");
                    {
                        ...
                        cut.title =
                            "Cut quick reply to clipboard (copy and remove)";
    ...
    async showEditor() {
        const response = await fetch(
            "/scripts/extensions/quick-reply/html/qrEditor.html",
            { cache: "no-store" },
        );
        if (response.ok) {
            ...
            let switcherList;
            dom.querySelector("#qr--modal-switcher").addEventListener(
                "click",
                (evt) => {
                    if (switcherList) {
                        switcherList.remove();
            ...
            /**@type {HTMLElement}*/
            const messageSyntaxInner = dom.querySelector(
                "#qr--modal-messageSyntaxInner",
            );
            this.editorSyntax = messageSyntaxInner;
            /**@type {HTMLInputElement}*/
            const wrap = dom.querySelector("#qr--modal-wrap");
            wrap.checked = JSON.parse(
                accountStorage.getItem("qr--wrap") ?? "false",
            );
            wrap.addEventListener("click", () => {
                accountStorage.setItem(
                    "qr--wrap",
                    JSON.stringify(wrap.checked),
                );
                updateWrap();
            });
            const updateWrap = () => {
            ...
            const updateSyntaxEnabled = () => {
                if (syntax.checked) {
                    ...
                    ).classList.remove("qr--noSyntax");
                } else {
                    dom.querySelector("#qr--modal-messageHolder").classList.add(
                        "qr--noSyntax",
                    );
                }
            };
            ...
            tabSize.value = JSON.parse(
                accountStorage.getItem("qr--tabSize") ?? "4",
            );
            const updateTabSize = () => {
            ...
            tabSize.addEventListener("change", () => {
                accountStorage.setItem(
                    "qr--tabSize",
                    JSON.stringify(Number(tabSize.value)),
                );
                updateTabSize();
            });
            /**@type {HTMLInputElement}*/
            const executeShortcut = dom.querySelector(
                "#qr--modal-executeShortcut",
            );
            executeShortcut.checked = JSON.parse(
                accountStorage.getItem("qr--executeShortcut") ?? "true",
            );
            executeShortcut.addEventListener("click", () => {
                accountStorage.setItem(
                    "qr--executeShortcut",
                    JSON.stringify(executeShortcut.checked),
                );
            });
            /**@type {HTMLInputElement}*/
            const syntax = dom.querySelector("#qr--modal-syntax");
            syntax.checked = JSON.parse(
                accountStorage.getItem("qr--syntax") ?? "true",
            );
            syntax.addEventListener("click", () => {
                accountStorage.setItem(
                    "qr--syntax",
                    JSON.stringify(syntax.checked),
                );
                updateSyntaxEnabled();
            });
                ...
                navigator.keyboard
                    ...
                    .then(
                        (it) =>
                            (dom.querySelector(
                                "#qr--modal-commentKey",
                            ).textContent = it.get("Backslash")),
                    );
            } else {
                dom.querySelector("#qr--modal-commentKey")
                    .closest("small")
                    .remove();
            }
            this.editorMessageLabel = dom.querySelector(
            ...
            const updateMessageDebounced = debounce(
                (value) => this.updateMessage(value),
                10,
            );
            ...
            message.addEventListener("keydown", async (evt) => {
                ...
                if (
                    ...
                    if (
                        ...
                        message.value.substring(start, end).includes("\n")
                    ) {
                        evt.stopImmediatePropagation();
                        evt.stopPropagation();
                        const lineStart = getLineStart();
                        message.selectionStart = lineStart;
                        const affectedLines = message.value
                    ...
                    if (delta > 0) {
                        ...
                        message.selectionStart =
                            start - (affectedLines[0].startsWith("\t") ? 1 : 0);
                    ...
                    const comment = parser.commandIndex.findLast(
                        (it) =>
                            it.name == "*" &&
                            ((it.start <= start && it.end >= start) ||
                                (it.start <= end && it.end >= end)),
                    );
                    if (comment) {
                        // uncomment
                        let content = message.value.slice(
                            comment.start + 1,
                            comment.end - 1,
                        );
                        ...
                        message.selectionStart =
                            start -
                            (start >= comment.start ? 2 + offsetStart : 0);
                        message.selectionEnd =
                            end -
                            2 -
                            offsetStart -
                            (end >= comment.end ? 2 + offsetEnd : 0);
                    } else {
                        // comment
                        const lineStart = getLineStart();
            ...
            const removeBreakpoint = (bp) => {
                ...
                if (preBreakPointStart <= start) {
                    ...
                    preBreakPointStart > start &&
                    preBreakPointEnd < end
                ) {
            ...
            const toggleBreakpoint = () => {
                ...
                const parser = new SlashCommandParser();
                parser.parse(message.value, false);
                const cmdIdx = parser.commandIndex.findLastIndex(
                    (it) => it.start <= idx,
                );
                if (cmdIdx > -1) {
                    ...
                    if (cmd instanceof SlashCommandBreakPoint) {
                        ...
                        parser.commandIndex[cmdIdx - 1] instanceof
            ...
            const addCtxItem = (
                ...
                /**@type {HTMLElement} */
                // @ts-ignore
                const itemDom = tpl.content
                    .querySelector(".qr--ctxItem")
                    .cloneNode(true);
                {
                    ...
                    itemDom
                        .querySelector(".qr--delete")
                        .addEventListener("click", () => {
                            itemDom.remove();
                            this.contextList.splice(
                                this.contextList.indexOf(link),
                                1,
                            );
                            this.updateContext();
                        });
            ...
            const onContextSort = () => {
                this.contextList = Array.from(
                    linkList.querySelectorAll(".qr--ctxItem"),
                ).map((it, idx) => {
                    const link =
                        this.contextList[Number(it.getAttribute("data-order"))];
            ...
            // auto-exec
            /**@type {HTMLInputElement}*/
            const preventAutoExecute = dom.querySelector(
                "#qr--preventAutoExecute",
            );
            ...
            /**@type {HTMLInputElement}*/
            const executeOnChatChange = dom.querySelector(
            ...
            /**@type {HTMLInputElement}*/
            const executeOnGroupMemberDraft = dom.querySelector(
                "#qr--executeOnGroupMemberDraft",
            );
            executeOnGroupMemberDraft.checked = this.executeOnGroupMemberDraft;
            executeOnGroupMemberDraft.addEventListener("click", () => {
                this.executeOnGroupMemberDraft =
                    executeOnGroupMemberDraft.checked;
                this.updateContext();
            });
            /**@type {HTMLInputElement}*/
            ...
            /**@type {HTMLElement}*/
            const executeProgress = dom.querySelector(
            ...
            executeBtnPause.addEventListener("click", async () => {
                if (this.abortController) {
                    if (this.abortController.signal.paused) {
                        this.abortController.continue(
                            "Continue button clicked",
                        );
                        this.editorExecuteProgress.classList.remove(
                            "qr--paused",
                        );
                    } else {
                ...
                if (
                    muts.find(
                        (it) =>
                            ...
                            [...it.removedNodes].find((n) =>
                                n.contains(inputMirror),
                            ),
                    )
                ) {
            ...
            resizeHandle.addEventListener("pointerdown", (evt) => {
                ...
                const dragListener = debounce((evt) => {
                    ...
                    dom.querySelector("#qr--qrOptions").style.setProperty(
                        "--width",
                        `${w}px`,
                    );
                }, 5);
    ...
    async executeFromEditor() {
        ...
        const syntax = this.editorDom.querySelector(
            "#qr--modal-messageSyntaxInner",
        );
        const updateScroll = (evt) => {
            ...
            this.debugController.onBreakPoint = async (closure, executor) => {
                ...
                const buildVars = (scope, isCurrent = false) => {
                    ...
                    const wrap = document.createElement("div");
                    {
                        ...
                        if (isCurrent) {
                            const executor =
                                this.debugController.cmdStack.slice(-1)[0];
                            {
                                // named args
                                const namedTitle =
                                    document.createElement("div");
                                ...
                                const keys = new Set([
                                    ...Object.keys(
                                        this.debugController.namedArguments ??
                                            {},
                                    ),
                                    ...(executor.namedArgumentList ?? []).map(
                                        (it) => it.name,
                                    ),
                                ]);
                                for (const key of keys) {
                                    ...
                                    const item = document.createElement("div");
                                    {
                                            ...
                                            document.createElement("div");
                                        {
                                            ...
                                            const val =
                                                executor.namedArgumentList.find(
                                                    (it) => it.name == key,
                                                )?.value;
                                            if (
                                                val instanceof
                            ...
                            }
                            {
                                ...
                                while (
                                    unnamed.length <
                                        executor.unnamedArgumentList?.length ??
                                    0
                                )
                                    unnamed.push(undefined);
                                unnamed = unnamed.map((it, idx) => [
                                    executor.unnamedArgumentList?.[idx],
                                    it,
                                ]);
                        ...
                        for (const key of Object.keys(scope.variables)) {
                            ...
                            const item = document.createElement("div");
                            {
                                item.classList.add("qr--var");
                                if (isHidden)
                                    item.classList.add("qr--isHidden");
                                const k = document.createElement("div");
                                {
                                    k.classList.add("qr--key");
                                    k.textContent = key;
                ...
                const buildStack = () => {
                    const wrap = document.createElement("div");
                    {
                        ...
                        for (const executor of this.debugController.cmdStack.toReversed()) {
                            ei++;
                            const c =
                                this.debugController.stack.toReversed()[ei];
                            const item = document.createElement("div");
                            {
                                ...
                                const src = document.createElement("div");
                                {
                                    ...
                                    const line = closure.fullText
                                        .slice(0, executor.start)
                                        .split("\n").length;
    ...
    updateEditorProgress(done, total) {
        this.editorExecuteProgress.style.setProperty(
            "--prog",
            `${(done / total) * 100}`,
        );
    }
