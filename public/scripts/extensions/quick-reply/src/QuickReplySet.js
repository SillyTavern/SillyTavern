...
export class QuickReplySet {
    ...
    async debug(qr) {
        ...
        closure.onProgress = (done, total) =>
            qr.updateEditorProgress(done, total);
    ...
    hookQuickReply(qr) {
        ...
        qr.onInsertBefore = (qrJson) => {
            ...
            if (qr.settingsDom) {
                qr.settingsDom.insertAdjacentElement(
                    "beforebegin",
                    newQr.settingsDom,
                );
            }
        ...
        qr.onTransfer = async () => {
            ...
            const copyBtn = document.createElement("div");
            {
                ...
                (dlg.ok ?? dlg.okButton).insertAdjacentElement(
                    "afterend",
                    copyBtn,
                );
            }
            ...
            if (dlg.result == POPUP_RESULT.AFFIRMATIVE) {
                const qrs = QuickReplySet.list.find(
                    (it) => it.name == sel.value,
                );
    ...
    async delete() {
        ...
        if (response.ok) {
            ...
            if (idx > -1) {
                ...
                warn(
                    `Deleted Quick Reply Set was not found in the list of sets: ${this.name}`,
                );
            }
        } else {
