...
export class SettingsUi {
    ...
    async render() {
        if (!this.dom) {
            const response = await fetch(
                "/scripts/extensions/quick-reply/html/settings.html",
                { cache: "no-store" },
            );
            if (response.ok) {
    ...
    prepareGeneralSettings() {
        ...
        this.showPopoutButton.addEventListener("click", () =>
            this.onShowPopoutButton(),
        );
    }
    ...
    prepareQrEditor() {
        ...
        this.dom
            .querySelector("#qr--set-new")
            .addEventListener("click", async () => this.addQrSet());
    ...
    onQrSetChange() {
        this.currentQrSet =
            QuickReplySet.get(this.currentSet.value) ?? new QuickReplySet();
    ...
    async onGlobalSetListSort() {
        this.settings.config.setList = Array.from(
            this.globalSetList.children,
        ).map((it, idx) => {
            const set =
                this.settings.config.setList[
                    Number(it.getAttribute("data-order"))
                ];
    ...
    async onChatSetListSort() {
        this.settings.chatConfig.setList = Array.from(
            this.chatSetList.children,
        ).map((it, idx) => {
            const set =
    ...
    async onQrListSort() {
        this.currentQrSet.qrList = Array.from(
            this.qrList.querySelectorAll(".qr--set-item"),
        ).map((it, idx) => {
    ...
    async doDeleteQrSet(qrs) {
        ...
        if (this.settings.chatConfig) {
            for (
                let i = this.settings.chatConfig.setList.length - 1;
                i >= 0;
                i--
            ) {
    ...
    async renameQrSet() {
        ...
        if (newName && newName.length > 0) {
            ...
            if (existingSet) {
                toastr.error(
                    `A Quick Reply Set named "${newName}" already exists.`,
                );
                return;
            }
            ...
            const option = this.currentSet.querySelector(
                `#qr--set option[value="${oldName}"]`,
            );
            ...
            if (oldQrs) {
                ...
                const idx = QuickReplySet.list.findIndex(
                    (it) =>
                        it.name
                            .toLowerCase()
                            .localeCompare(name.toLowerCase()) == 1,
                );
                if (idx > -1) {
                    QuickReplySet.list.splice(idx, 0, qrs);
                } else {
                ...
                const opt = document.createElement("option");
                {
                    ...
                    opt.textContent = qrs.name;
                    if (idx > -1) {
                        this.currentSet.children[idx].insertAdjacentElement(
                            "beforebegin",
                            opt,
                        );
                    } else {
    ...
    async importSingleQrSet(/**@type {File}*/ file) {
        ...
        try {
            ...
            if (
                ...
                toastr.error(
                    `The file "${file.name}" does not appear to be a valid quick reply set.`,
                );
                warn(
                    `The file "${file.name}" does not appear to be a valid quick reply set.`,
                );
                ...
                if (oldQrs) {
                    ...
                    const idx = QuickReplySet.list.findIndex(
                        (it) =>
                            it.name
                                .toLowerCase()
                                .localeCompare(qrs.name.toLowerCase()) == 1,
                    );
                    if (idx > -1) {
                        QuickReplySet.list.splice(idx, 0, qrs);
                    } else {
    ...
    exportQrSet() {
        const blob = new Blob([JSON.stringify(this.currentQrSet)], {
    ...
    async duplicateQrSet() {
        ...
        if (newName && newName.length > 0) {
            ...
            const newQrSet = QuickReplySet.from(
                JSON.parse(JSON.stringify(this.currentQrSet)),
            );
            newQrSet.name = newName;
            newQrSet.qrList = this.currentQrSet.qrList.map((qr) =>
                QuickReply.from(JSON.parse(JSON.stringify(qr))),
            );
            ...
            const idx = QuickReplySet.list.findIndex(
                (it) =>
                    it.name
                        .toLowerCase()
                        .localeCompare(newName.toLowerCase()) == 1,
            );
            if (idx > -1) {
