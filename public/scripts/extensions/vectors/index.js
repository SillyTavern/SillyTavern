...
const getBatchSize = () =>
    ["transformers", "palm", "ollama"].includes(settings.source) ? 1 : 5;
    ...
    try {
        ...
        while (!finished) {
            ...
            const averageElapsed =
                lastElapsed.reduce((a, b) => a + b, 0) / lastElapsed.length; // average time needed to process one item
...
async function synchronizeChat(batchSize = 5) {
    ...
    try {
        ...
        let newVectorItems = hashedMessages.filter(
            (x) => !hashesInCollection.includes(x.hash),
        );
        ...
        if (settings.summarize) {
            newVectorItems = await summarize(
                newVectorItems,
        ...
        if (newVectorItems.length > 0) {
            const chunkedBatch = splitByChunks(
                newVectorItems.slice(0, batchSize),
            );

            console.log(
                `Vectors: Found ${newVectorItems.length} new items. Processing ${batchSize}...`,
            );
...
async function processFiles(chat) {
    try {
        ...
        for (const message of chat) {
            ...
            const queryText = await getQueryText(chat, "file");
            const fileChunks = await retrieveFileChunks(
                queryText,
                collectionId,
            );
...
async function ingestDataBankAttachments(source) {
    ...
    for (const file of dataBank) {
        ...
        const chunkSize =
            file.size > thresholdLength ? settings.chunk_size_db : -1;
        await vectorizeFile(
            fileText,
            file.name,
            collectionId,
            chunkSize,
            settings.overlap_percent_db,
        );
    }
...
async function injectDataBankChunks(queryText, collectionIds) {
    try {
        ...
        console.debug(
            `Vectors: Retrieved ${collectionIds.length} Data Bank collections`,
            queryResults,
        );
        let textResult = "";
...
async function retrieveFileChunks(queryText, collectionId) {
    console.debug(
        `Vectors: Retrieving file chunks for collection ${collectionId}`,
        queryText,
    );
...
async function vectorizeFile(
    ...
    try {
        if (
            settings.translate_files &&
            typeof globalThis.translate === "function"
        ) {
        ...
        const batchSize = getBatchSize();
        const toastBody = $("<span>").text(
            "This may take a while. Please wait...",
        );
        toast = toastr.info(
            toastBody,
            `Ingesting file ${escapeHtml(fileName)}`,
            {
                closeButton: false,
                escapeHtml: false,
                timeOut: 0,
        ...
        const chunks =
            settings.only_custom_boundary && settings.force_chunk_delimiter
                ...
                : splitRecursive(fileText, chunkSize, delimiters).map(
                      (x, y, z) =>
                          overlapSize > 0
                              ? overlapChunks(x, y, z, overlapSize)
                              : x,
                  );
        console.debug(
        ...
        toastr.clear(toast);
        toastr.error(String(error), "Failed to vectorize file", {
            preventDuplicates: true,
        });
...
async function rearrangeChat(chat, _contextSize, _abort, type) {
    try {
        ...
        if (chat.length < settings.protect) {
            console.debug(
                `Vectors: Not enough messages to rearrange (less than ${settings.protect})`,
            );
            return;
        }
...
function overlapChunks(chunk, index, chunks, overlapSize) {
    ...
    const nextOverlap =
        trimToEndSentence(nextChunk?.substring(0, halfOverlap)) || "";
    const prevOverlap =
        trimToStartSentence(
            prevChunk?.substring(prevChunk.length - halfOverlap),
        ) || "";
    const overlappedChunk = [prevOverlap, chunk, nextOverlap]
        .filter((x) => x)
        .join(" ");
...
const onChatEvent = debounce(
    async () => await moduleWorker.update(),
...
async function getQueryText(chat, initiator) {
    ...
    if (
        ...
        hashedMessages = await summarize(
            hashedMessages,
            settings.summary_source,
        );
    }
    ...
    switch (settings.source) {
        ...
        case "koboldcpp": {
            const { embeddings, model } =
                await createKoboldCppEmbeddings(items);
...
async function getSavedHashes(collectionId) {
    ...
    if (!response.ok) {
        throw new Error(
            `Failed to get saved hashes for collection ${collectionId}`,
        );
    }
...
async function insertVectorItems(collectionId, items) {
    ...
    if (!response.ok) {
        throw new Error(
            `Failed to insert vector items for collection ${collectionId}`,
        );
    }
}
...
function throwIfSourceInvalid() {
    if (
        ...
        (settings.source === "mistral" &&
            !secret_state[SECRET_KEYS.MISTRALAI]) ||
        (settings.source === "togetherai" &&
            !secret_state[SECRET_KEYS.TOGETHERAI]) ||
        (settings.source === "nomicai" && !secret_state[SECRET_KEYS.NOMICAI]) ||
        (settings.source === "cohere" && !secret_state[SECRET_KEYS.COHERE])
    ) {
        throw new Error("Vectors: API key missing", {
            cause: "api_key_missing",
        });
    }

    if (
        vectorApiRequiresUrl.includes(settings.source) &&
        settings.use_alt_endpoint
    ) {
        if (!settings.alt_endpoint_url) {
            throw new Error("Vectors: API URL missing", {
                cause: "api_url_missing",
            });
        }
    } else {
        if (
    ...
    if (
        ...
        throw new Error("Vectors: API model missing", {
            cause: "api_model_missing",
        });
    }

    if (settings.source === "extras" && !modules.includes("embeddings")) {
        throw new Error("Vectors: Embeddings module missing", {
            cause: "extras_module_missing",
        });
    }

    if (
        settings.source === "webllm" &&
        (!isWebLlmSupported() || !settings.webllm_model)
    ) {
        throw new Error("Vectors: WebLLM is not supported", {
            cause: "webllm_not_supported",
        });
    }
}
...
async function deleteVectorItems(collectionId, hashes) {
    ...
    if (!response.ok) {
        throw new Error(
            `Failed to delete vector items for collection ${collectionId}`,
        );
    }
}
 ...
 */
async function queryMultipleCollections(
    collectionIds,
    searchText,
    topK,
    threshold,
...
async function purgeFileVectorIndex(fileUrl) {
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Could not delete vector index for collection ${collectionId}`,
            );
        }

        console.log(
            `Vectors: Purged vector index for collection ${collectionId}`,
        );
    } catch (error) {
...
function loadWebLlmModels() {
    return executeWithWebLlmErrorHandling(() => {
        ...
        for (const model of models) {
            $("#vectors_webllm_model").append(
                $("<option>", { value: model.id, text: model.toString() }),
            );
        }
        if (
            !settings.webllm_model ||
            !models.some((x) => x.id === settings.webllm_model)
        ) {
            if (models.length) {
...
async function createWebLlmEmbeddings(items) {
    ...
    return executeWithWebLlmErrorHandling(async () => {
        const embeddings = await webllmProvider.embedTexts(
            items,
            settings.webllm_model,
        );
...
async function createKoboldCppEmbeddings(items) {
    ...
    if (
        ...
        !data.model ||
        data.embeddings.length !== items.length
    ) {
    ...
    for (let i = 0; i < data.embeddings.length; i++) {
        if (
            !Array.isArray(data.embeddings[i]) ||
            data.embeddings[i].length === 0
        ) {
            throw new Error(
...
async function onViewStatsClick() {
    ...
    for (const message of chat) {
        if (
            hashesInCollection.includes(
                getStringHash(substituteParams(message.mes)),
            )
        ) {
    ...
    try {
        ...
        function getChunkSize(file) {
            ...
            if (dataBank.includes(file)) {
                ...
                const thresholdLength = settings.size_threshold_db * 1024;
                // Use chunk size from settings if file is larger than threshold
                return file.size > thresholdLength
                    ? settings.chunk_size_db
                    : -1;
            }
...
async function activateWorldInfo(chat) {
    ...
    for (const world in groupedEntries) {
        ...
        const deletedHashes = hashesInCollection.filter(
            (x) =>
                !groupedEntries[world].some(
                    (y) => getStringHash(y.content) === x,
                ),
        );

        if (newEntries.length > 0) {
            console.log(
                `Vectors: Found ${newEntries.length} new WI entries for world ${world}`,
            );
        ...
        if (deletedHashes.length > 0) {
            console.log(
                `Vectors: Deleted ${deletedHashes.length} old hashes for world ${world}`,
            );
    ...
    console.log(
        `Vectors: Activated ${activatedEntries.length} WI entries`,
        activatedEntries,
    );
...
jQuery(async () => {
    ...
    settings.source =
        settings.source !== "local" ? settings.source : "transformers";
    ...
    $("#vectors_enabled_chats")
        ...
        .on("input", () => {
            settings.enabled_chats = $("#vectors_enabled_chats").prop(
                "checked",
            );
    ...
    $("#vectors_enabled_files")
        ...
        .on("input", () => {
            settings.enabled_files = $("#vectors_enabled_files").prop(
                "checked",
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
    ...
    $("#vector_altEndpoint_address")
        ...
        .on("change", () => {
            settings.alt_endpoint_url = String(
                $("#vector_altEndpoint_address").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
            ...
            customButtons: [
                {
                    ...
                    action: async () => {
                        ...
                        $("#api_key_nomicai").toggleClass(
                            "success",
                            !!secret_state[SECRET_KEYS.NOMICAI],
                        );
                        saveSettingsDebounced();
                    },
                },
    ...
    $("#vectors_togetherai_model")
        ...
        .on("change", () => {
            settings.togetherai_model = String(
                $("#vectors_togetherai_model").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $(`input[name="vectors_position"][value="${settings.position}"]`).prop(
        "checked",
        true,
    );
    $('input[name="vectors_position"]').on("change", () => {
        settings.position = Number(
            $('input[name="vectors_position"]:checked').val(),
        );
        Object.assign(extension_settings.vectors, settings);
        saveSettingsDebounced();
    });
    ...
    $("#vectors_size_threshold")
        ...
        .on("input", () => {
            settings.size_threshold = Number(
                $("#vectors_size_threshold").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_summarize_user")
        ...
        .on("input", () => {
            settings.summarize_sent = !!$("#vectors_summarize_user").prop(
                "checked",
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
    ...
    $("#vectors_summary_source")
        ...
        .on("change", () => {
            settings.summary_source = String(
                $("#vectors_summary_source").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_summary_prompt")
        ...
        .on("input", () => {
            settings.summary_prompt = String(
                $("#vectors_summary_prompt").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_message_chunk_size")
        ...
        .on("input", () => {
            settings.message_chunk_size = Number(
                $("#vectors_message_chunk_size").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_size_threshold_db")
        ...
        .on("input", () => {
            settings.size_threshold_db = Number(
                $("#vectors_size_threshold_db").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_chunk_count_db")
        ...
        .on("input", () => {
            settings.chunk_count_db = Number(
                $("#vectors_chunk_count_db").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_overlap_percent")
        ...
        .on("input", () => {
            settings.overlap_percent = Number(
                $("#vectors_overlap_percent").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_overlap_percent_db")
        ...
        .on("input", () => {
            settings.overlap_percent_db = Number(
                $("#vectors_overlap_percent_db").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_file_template_db")
        ...
        .on("input", () => {
            settings.file_template_db = String(
                $("#vectors_file_template_db").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $(
        `input[name="vectors_file_position_db"][value="${settings.file_position_db}"]`,
    ).prop("checked", true);
    $('input[name="vectors_file_position_db"]').on("change", () => {
        settings.file_position_db = Number(
            $('input[name="vectors_file_position_db"]:checked').val(),
        );
        Object.assign(extension_settings.vectors, settings);
        saveSettingsDebounced();
    });
    ...
    $("#vectors_file_depth_role_db")
        ...
        .on("input", () => {
            settings.file_depth_role_db = Number(
                $("#vectors_file_depth_role_db").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_translate_files")
        ...
        .on("input", () => {
            settings.translate_files = !!$("#vectors_translate_files").prop(
                "checked",
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
    ...
    $("#vectors_enabled_for_all")
        ...
        .on("input", () => {
            settings.enabled_for_all = !!$("#vectors_enabled_for_all").prop(
                "checked",
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
    ...
    $("#vectors_score_threshold")
        ...
        .on("input", () => {
            settings.score_threshold = Number(
                $("#vectors_score_threshold").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    $("#vectors_force_chunk_delimiter")
        ...
        .on("input", () => {
            settings.force_chunk_delimiter = String(
                $("#vectors_force_chunk_delimiter").val(),
            );
            Object.assign(extension_settings.vectors, settings);
            saveSettingsDebounced();
        });
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            ...
            callback: async (args, query) => {
                const clamp = (v) =>
                    Number.isNaN(v) ? null : Math.min(1, Math.max(0, v));
                const threshold = clamp(
                    Number(args?.threshold ?? settings.score_threshold),
                );
                const validateCount = (v) =>
                    Number.isNaN(v) || !Number.isInteger(v) || v < 1 ? null : v;
                const count =
                    validateCount(Number(args?.count)) ??
                    settings.chunk_count_db;
                const source = String(args?.source ?? "");
                const attachments = source
                    ? getDataBankAttachmentsForSource(source, false)
                    : getDataBankAttachments(false);
                const collectionIds = await ingestDataBankAttachments(
                    String(source),
                );
                const queryResults = await queryMultipleCollections(
                    collectionIds,
                ...
                // @ts-ignore
                return slashCommandReturnHelper.doReturn(
                    args.return ?? "object",
                    urls,
            ...
            namedArgumentList: [
                ...
                SlashCommandNamedArgument.fromProps({
                    ...
                    enumList: [
                        ...
                        ...slashCommandReturnHelper.enumList({
                            allowObject: true,
                        }),
                    ],
                    forceEnum: true,
