    ...
    updateCurrentAssets().then(async function () {
        fetch(url, { cache: "no-cache" })
            ...
            .then(async function (json) {
                ...
                $("#assets_search").val("");
                $("#assets_type_select").append(
                    $("<option />", { value: "", text: t`All` }),
                );

                for (const type of assetTypes) {
                    const option = $("<option />", {
                        value: type,
                        text: t([KNOWN_TYPES[type] || type]),
                    });
                ...
                for (const assetType of assetTypes) {
                    let assetTypeMenu = $("<div />", {
                        id: "assets_audio_ambient_div",
                        class: "assets-list-div",
                    });
                    assetTypeMenu.attr("data-type", assetType);
                    assetTypeMenu
                        .append(
                            `<h3>${KNOWN_TYPES[assetType] || assetType}</h3>`,
                        )
                        .hide();
                    ...
                    for (const i in availableAssets[assetType].sort(
                        (a, b) =>
                            a?.name &&
                            b?.name &&
                        ...
                        let element = $("<div />", {
                            id: elemId,
                            class: "asset-download-button right_menu_button",
                        });
                        const label = $(
                            '<i class="fa-fw fa-solid fa-download fa-lg"></i>',
                        );
                        ...
                        const url = isValidUrl(asset["url"])
                            ? asset["url"]
                            : "";
                        const title =
                            assetType === "extension"
                                ? t`Extension repo/guide:` + ` ${url}`
                        ...
                        const toolTag =
                            assetType === "extension" && asset["tool"];
                                ...
                                `<div class="flex-container flexFlowColumn flexNoGap">
                                    ...
                                    (toolTag
                                        ? '<span class="tag" title="' +
                                          t`Adds a function tool` +
                                          '"><i class="fa-solid fa-sm fa-wrench"></i> ' +
                                          t`Tool` +
                                          "</span>"
                                        : "") +
                                    `</span>
                        ...
                        if (assetType === "character") {
                            if (asset.highlight) {
                                assetBlock
                                    .find(".asset-name")
                                    .append(
                                        '<i class="fa-solid fa-sm fa-trophy"></i>',
                                    );
                            }
                            assetBlock
                                .find(".asset-name")
                                .prepend(
                                    `<div class="avatar"><img src="${asset["url"]}" alt="${displayName}"></div>`,
                                );
                        }
            ...
            .catch((error) => {
                ...
                console.error(error);
                toastr.error(
                    "Problem with assets URL",
                    DEBUG_PREFIX + "Cannot get assets list",
                );
                $("#assets-connect-button").addClass(
...
async function openCharacterBrowser(forceDefault) {
    const url = forceDefault
        ? ASSETS_JSON_URL
        : String($("#assets-json-url-field").val());
    ...
    if (!characters.length) {
        toastr.error(
            "No characters found in the assets list",
            "Character browser",
        );
        return;
    }
    ...
    const template = $(
        await renderExtensionTemplateAsync(MODULE_NAME, "market", {}),
    );
    ...
    for (const character of characters.sort((a, b) =>
        ...
        const downloadButton = characterElement.find(
            ".characterAssetDownloadButton",
        );
...
jQuery(async () => {
    ...
    const windowTemplate = await renderExtensionTemplateAsync(
        MODULE_NAME,
        "window",
        {},
    );
    ...
    installHintButton.on("click", async function () {
        ...
        toastr.info(
            t`Click the flashing button to install extensions.`,
            t`How to install extensions?`,
        );
    });
            ...
            (await Popup.show.confirm(
                    ...
                    `</span><var>${url}</var>`,
                {
                    customInputs: [
                        {
                            id: "assets-remember",
                            label: "Don't ask again for this URL",
                        },
                    ],
                    onClose: (popup) => {
                        if (popup.result) {
                            const rememberValue =
                                popup.inputResults.get("assets-remember");
