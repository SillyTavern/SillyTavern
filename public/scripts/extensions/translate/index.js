...
function showKeysButton() {
    ...
    const providerOptionalUrl = LOCAL_URL.includes(
        extension_settings.translate.provider,
    );
    ...
    $("#deepl_api_endpoint").toggle(
        extension_settings.translate.provider === "deepl",
    );
}
...
const handleMessageReasoningEdit = createEventHandler(
    translateMessageReasoningEdit,
    () => true,
);
const handleMessageReasoningDelete = createEventHandler(
    removeReasoningDisplayText,
    () => true,
);
...
jQuery(async () => {
    ...
    const buttonHtml = await renderExtensionTemplateAsync(
        "translate",
        "buttons",
    );
    ...
    $("#translate_url_button").on("click", async () => {
        ...
        const savedUrl = secret_state[secretKey]
            ? await findSecret(secretKey)
            : "";

        const url = await callGenericPopup(
            popupText,
            POPUP_TYPE.INPUT,
            savedUrl,
            {
    ...
    eventSource.on(
        ...
        handleMessageReasoningEdit,
    );
    eventSource.on(
        event_types.MESSAGE_REASONING_DELETED,
        handleMessageReasoningDelete,
    );
    ...
    SlashCommandParser.addCommandObject(
        SlashCommand.fromProps({
            ...
            namedArgumentList: [
                ...
                SlashCommandNamedArgument.fromProps({
                    ...
                    enumProvider: () =>
                        Array.from(
                            document
                                .getElementById("translation_provider")
            ...
            callback: async (args, value) => {
                const target =
                    args?.target &&
                    Object.values(languageCodes).includes(String(args.target))
                        ? String(args.target)
                        : extension_settings.translate.target_language;
                const provider =
                    args?.provider || extension_settings.translate.provider;
