...
export async function getMultimodalCaption(base64Img, prompt) {
    const useReverseProxy =
        [
            ...
            "vertexai",
            "xai",
        ].includes(extension_settings.caption.multimodal_api) &&
    ...
    const noPrefix = ["ollama"].includes(
    ...
    const isKoboldCpp =
        extension_settings.caption.multimodal_api === "koboldcpp";
    const isVllm = extension_settings.caption.multimodal_api === "vllm";
    const base64Bytes = base64Img.length * 0.75;
    const compressionLimit = 2 * 1024 * 1024;
    const thumbnailNeeded = [
        "google",
        "openrouter",
        "mistral",
        "groq",
    ...
    if (
        (thumbnailNeeded && base64Bytes > compressionLimit) ||
        isOoba ||
        isKoboldCpp
    ) {
        const maxSide = 1024;
        base64Img = await createThumbnail(
            base64Img,
            maxSide,
            maxSide,
            "image/jpeg",
    ...
    if (isCustom) {
        ...
        requestBody.custom_include_headers =
...
function throwIfInvalidModel(useReverseProxy) {
    ...
    if (
        altEndpointEnabled &&
        ["llamacpp", "ooba", "koboldcpp", "vllm", "ollama"].includes(
            multimodalApi,
        ) &&
        !altEndpointUrl
    ) {
    ...
    if (multimodalApi === "vertexai" && !useReverseProxy) {
        ...
        if (authMode === "express") {
            ...
            if (!secret_state[SECRET_KEYS.VERTEXAI]) {
                throw new Error(
                    "Google Vertex AI API key is not set for Express mode.",
                );
            }
...
export class ConnectionManagerRequestService {
        ...
        try {
            switch (selectedApiMap.selected) {
                case "openai": {
                    ...
                    const proxyPreset = proxies.find(
                        (p) => p.name === profile.proxy,
                    );
                ...
                case "textgenerationwebui": {
                    ...
                    return await context.TextCompletionService.processRequest(
                        ...
                        {
                            instructName: includeInstruct
                                ? profile.instruct
                                : undefined,
                            presetName: includePreset
                                ? profile.preset
                                : undefined,
                            instructSettings: includeInstruct
                                ? instructSettings
                                : undefined,
                        },
                        extractData,
                ...
                default: {
                    throw new Error(
                        `Unknown API type ${selectedApiMap.selected}`,
                    );
                }
            }
        } catch (error) {
    ...
    static handleDropdown(
        ...
        if (!dropdown || !dropdown.length) {
            throw new Error(
                `Could not find dropdown with selector ${selector}`,
            );
        }
        ...
        const groups = {};
        for (const [apiType, groupLabel] of Object.entries(
        ...
        for (const [apiType, groupProfiles] of Object.entries(
            sortedProfilesByGroup,
        )) {
        ...
        const selectedProfile = profiles.find(
            (p) => p.id === initialSelectedProfileId,
        );
        if (selectedProfile) {
        ...
        context.eventSource.on(
            ...
            async (oldProfile, newProfile) => {
                ...
                const oldOption = group.querySelector(
                    `option[value="${oldProfile.id}"]`,
                );
                if (oldOption) {
                    oldOption.remove();
                }
        ...
        context.eventSource.on(
            ...
            async (profile) => {
                ...
                const group = groups[CONNECT_API_MAP[profile.api].selected];
                const optionToRemove = group.querySelector(
                    `option[value="${profile.id}"]`,
                );
                if (optionToRemove) {
