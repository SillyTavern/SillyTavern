...
export class KokoroTtsProvider {
    ...
    async loadSettings(settings) {
        if (settings.modelId !== undefined)
            this.settings.modelId = settings.modelId;
        if (settings.dtype !== undefined) this.settings.dtype = settings.dtype;
        if (settings.device !== undefined)
            this.settings.device = settings.device;
        if (settings.voiceMap !== undefined)
            this.settings.voiceMap = settings.voiceMap;
        if (settings.defaultVoice !== undefined)
            this.settings.defaultVoice = settings.defaultVoice;
        if (settings.speakingRate !== undefined)
            this.settings.speakingRate = settings.speakingRate;

        $("#kokoro_model_id")
            .val(this.settings.modelId)
            .on("input", this.onSettingsChange.bind(this));
        $("#kokoro_dtype")
            .val(this.settings.dtype)
            .on("change", this.onSettingsChange.bind(this));
        $("#kokoro_device")
            .val(this.settings.device)
            .on("change", this.onSettingsChange.bind(this));
        $("#kokoro_speaking_rate")
            .val(this.settings.speakingRate)
            .on("input", this.onSettingsChange.bind(this));
        $("#kokoro_speaking_rate_output").text(
            this.settings.speakingRate + "x",
        );
    }
            ...
            try {
                ...
                const initPromise = new Promise((initResolve, initReject) => {
                    const timeoutId = setTimeout(() => {
                        initReject(
                            new Error("Worker initialization timed out"),
                        );
    ...
    handleWorkerMessage(event) {
        const { action, success, ready, error, requestId, blobUrl } =
            event.data;
        ...
        switch (action) {
            case "initialized":
                {
                    const initRequest =
                        this.pendingRequests.get("initialization");
                    if (initRequest) {
                        if (success) {
                            initRequest.resolve(true);
                        } else {
                            initRequest.reject(
                                new Error(error || "Initialization failed"),
                            );
                        }
                        ...
                        if (success) {
                            fetch(blobUrl)
                                ...
                                .catch((error) => {
                                    request.reject(
                                        new Error(
                                            "Failed to fetch TTS audio blob: " +
                                                error,
                                        ),
                                    );
                                });
                            ...
                            request.reject(
                                new Error(error || "TTS generation failed"),
                            );
                        }
    ...
    async onSettingsChange() {
        ...
        this.settings.speakingRate = parseFloat(
            $("#kokoro_speaking_rate").val().toString(),
        );
    ...
    getVoice(voiceName) {
        ...
        const actualVoiceName = this.voices.includes(voiceName)
            ? voiceName
            : defaultVoice;
        return {
