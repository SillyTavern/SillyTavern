...
class CoquiTtsProvider {
    ...
    async loadSettings(settings) {
        ...
        await fetch(
            "/scripts/extensions/tts/coqui_api_models_settings_full.json",
        )
            .then((response) => response.json())
            .then((json) => {
                ...
                console.debug(
                    DEBUG_PREFIX,
                    "initialized coqui-api full model list to",
                    coquiApiModelsFull,
                );
                /*
    ...
    updateCustomVoices() {
        ...
        for (let voiceName in this.settings.voiceMapDict) {
            ...
            if (voiceId["model_language"] != null)
                this.settings.customVoices[voiceName] +=
                    "[" + voiceId["model_language"] + "]";

            if (voiceId["model_speaker"] != null)
                this.settings.customVoices[voiceName] +=
                    "[" + voiceId["model_speaker"] + "]";
        }
        ...
        for (const voiceName in this.settings.voiceMapDict) {
            $("#coqui_voicename_select").append(
                new Option(voiceName, voiceName),
            );
        }
    ...
    async onAddClick() {
        ...
        if (
            ...
            ("languages" in
                modelDict[model_language][model_dataset][model_label])
        ) {
            toastr.error(
        ...
        if (model_origin == "coqui-api") {
            ...
            for (let language in coquiApiModels) {
                ...
                $("#coqui_api_language").append(
                    new Option(languageLabel, language),
                );
                console.log(
                    DEBUG_PREFIX,
                    "added language",
    ...
    async onModelNameChange() {
        ...
        const model_settings =
            modelDict[model_language][model_dataset][model_name];
        ...
        if ("languages" in model_settings) {
            ...
            for (let i = 0; i < model_settings["languages"].length; i++) {
                const language_label = JSON.stringify(
                    model_settings["languages"][i],
                ).replaceAll('"', "");
                $("#coqui_api_model_settings_language").append(
                    new Option(language_label, i),
                );
            }
        } else {
        ...
        if ("speakers" in model_settings) {
            ...
            for (let i = 0; i < model_settings["speakers"].length; i++) {
                const speaker_label = JSON.stringify(
                    model_settings["speakers"][i],
                ).replaceAll('"', "");
                $("#coqui_api_model_settings_speaker").append(
                    new Option(speaker_label, i),
                );
            }
        } else {
        ...
        if (model_state == "installed") {
            $("#coqui_api_model_install_status").text(
                "Model already installed on extras server",
            );
            $("#coqui_api_model_install_button").hide();
        } else {
            let action = "download";
            if (model_state == "corrupted") {
                ...
                $("#coqui_api_model_install_status").text(
                    "Model not found on extras server",
                );
            }
            ...
            $("#coqui_api_model_install_button")
                ...
                .on("click", async function () {
                    try {
                        ...
                        let apiResult = await CoquiTtsProvider.installModel(
                            model_id,
                            action,
                        );
                        ...
                        if (apiResult["status"] == "done") {
                            $("#coqui_api_model_install_status").text(
                                "Model installed and ready to use!",
                            );
                            $("#coqui_api_model_install_button").hide();
                        ...
                        toastr.error(
                            error,
                            DEBUG_PREFIX + " error with model download",
                            {
                                timeOut: 10000,
                                extendedTimeOut: 20000,
                                preventDuplicates: true,
    ...
    */
    static async getLocalModelList() {
        ...
        if (!apiResult.ok) {
            toastr.error(
                apiResult.statusText,
                DEBUG_PREFIX + " Get local model list request failed",
            );
            throw new Error(
                `HTTP ${apiResult.status}: ${await apiResult.text()}`,
            );
    ...
    async fetchTtsVoiceObjects() {
        const voiceIds = Object.keys(this.settings.voiceMapDict).map(
            (voice) => ({ name: voice, voice_id: voice, preview_url: false }),
        );
        return voiceIds;
    }
...
async function initLocalModels() {
    ...
    if (!coquiLocalModelsReceived) {
        ...
        for (const model_dataset of coquiLocalModels)
            $("#coqui_local_model_name").append(
                new Option(model_dataset, model_dataset),
            );
