...
class AllTalkTtsProvider {
    ...
    constructor() {
        ...
        this.settings = {
            provider_endpoint:
                this.settings.provider_endpoint || "http://localhost:7851",
            server_version: this.settings.server_version || "v2",
            language: this.settings.language || "en",
            voiceMap: this.settings.voiceMap || {},
            at_generation_method:
                this.settings.at_generation_method || "standard_generation",
            narrator_enabled: this.settings.narrator_enabled || "false",
            at_narrator_text_not_inside:
                this.settings.at_narrator_text_not_inside || "narrator",
            narrator_voice_gen:
                this.settings.narrator_voice_gen || "Please set a voice",
            rvc_character_voice:
...
<span>- <strong>Text-generation-webui</strong> users - Uncheck <strong>Enable TTS</strong> in the TGWUI interface, or you will hear 2x voices and file names being generated.</span>
    ...
    async loadSettings(settings) {
        ...
        if (Object.keys(settings).length === 0) {
            ...
            for (const key in settings) {
                if (key in this.settings) {
                    ...
                    console.debug(
                        `Ignoring non-user-configurable setting: ${key}`,
                    );
                }
            }
        }
        ...
        $("#at_narrator_text_not_inside").val(
            this.settings.at_narrator_text_not_inside,
        );
    ...
    applySettingsToHTML() {
        ...
        const textNotInsideSelect = document.getElementById(
            "at_narrator_text_not_inside",
        );
        const generationMethodSelect = document.getElementById(
            "at_generation_method",
        );
        ...
        if (atNarratorSelect) {
            const ttsPassAsterisksCheckbox =
                document.getElementById("tts_pass_asterisks");
            const ttsNarrateQuotedCheckbox =
                document.getElementById("tts_narrate_quoted");
            const ttsNarrateDialoguesCheckbox = document.getElementById(
            ...
            this.settings.narrator_enabled =
                this.settings.narrator_enabled.toString();
        }
        ...
        const isStreamingEnabled =
            this.settings.at_generation_method === "streaming_enabled";
        if (isStreamingEnabled) {
            ...
            if (textNotInsideSelect)
                textNotInsideSelect.disabled = !this.settings.narrator_enabled;
            if (narratorVoiceSelect)
                narratorVoiceSelect.disabled = !this.settings.narrator_enabled;
        }
    }
    ...
    async checkReady() {
        try {
            const response = await fetch(
                `${this.settings.provider_endpoint}/api/ready`,
            );
    ...
    async fetchTtsVoiceObjects() {
        const response = await fetch(
            `${this.settings.provider_endpoint}/api/voices`,
        );
        if (!response.ok) {
    ...
    async fetchRvcVoiceObjects() {
        ...
        try {
            const response = await fetch(
                `${this.settings.provider_endpoint}/api/rvcvoices`,
            );
            if (!response.ok) {
                const errorText = await response.text();
            ...
            if (!data || !data.rvcvoices) {
                ...
                throw new Error(
                    "Invalid data format received from /api/rvcvoices",
                );
            }
    ...
    async updateSettingsFromServer() {
        try {
            const response = await fetch(
                `${this.settings.provider_endpoint}/api/currentsettings`,
            );
            if (!response.ok) {
                throw new Error(
                    `Failed to fetch current settings: ${response.statusText}`,
                );
            }
            const currentSettings = await response.json();
            currentSettings.models_available.sort((a, b) =>
                a.name.localeCompare(b.name),
            );

            this.settings.enginesAvailable = currentSettings.engines_available;
            this.settings.currentEngineLoaded =
                currentSettings.current_engine_loaded;
            ...
            this.settings.deepspeed_available =
                currentSettings.deepspeed_available;
    ...
    updateRvcVoiceDropdowns() {
        ...
        const rvcCharacterVoiceSelect = document.getElementById(
            "rvc_character_voice",
        );
        ...
        const rvcNarratorVoiceSelect =
            document.getElementById("rvc_narrator_voice");
    ...
    setupEventListeners() {
        ...
        const modelSelect = document.getElementById("switch_model");
        if (modelSelect) {
            const debouncedModelSelectChange = debounce(
                onModelSelectChange,
                1400,
            );
        ...
        const rvcCharacterPitchSelect = document.getElementById(
            "rvc_character_pitch",
        );
        ...
        const rvcNarratorPitchSelect =
            document.getElementById("rvc_narrator_pitch");
        ...
        if (deepspeedCheckbox) {
            ...
            const debouncedHandleDeepSpeedChange = debounce(
                handleDeepSpeedChange,
                300,
            );
            deepspeedCheckbox.addEventListener(
                "change",
                debouncedHandleDeepSpeedChange,
            );
        }
        ...
        if (lowVramCheckbox) {
            ...
            const debouncedHandleLowVramChange = debounce(
                handleLowVramChange,
                300,
            );
            lowVramCheckbox.addEventListener(
                "change",
                debouncedHandleLowVramChange,
            );
        }
        ...
        const atGenerationMethodSelect = document.getElementById(
            "at_generation_method",
        );
        ...
        try {
            ...
            if (
                ...
                postData.append(
                    ...
                    this.settings.rvc_character_voice,
                );
                postData.append(
                    "rvccharacter_pitch",
                    this.settings.rvc_character_pitch || "0",
                );
            }

            // Making the POST request
            const response = await fetch(
                `${this.settings.provider_endpoint}/api/previewvoice/`,
                {
                    method: "POST",
                    headers: {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error(
                    "previewTtsVoice Error Response Text:",
                    errorText,
                );
                throw new Error(`HTTP ${response.status}: ${errorText}`);
            }
            ...
            if (data.output_file_url) {
                ...
                console.warn(
                    "previewTtsVoice No output file URL received in the response",
                );
                throw new Error("No output file URL received in the response");
            }
        } catch (error) {
            console.error(
                "previewTtsVoice Exception caught during preview generation:",
                error,
            );
            throw error;
        }
    ...
    async generateTts(inputText, voiceId) {
        try {
            if (this.settings.at_generation_method === "streaming_enabled") {
                ...
                // For standard method
                const outputUrl = await this.fetchTtsGeneration(
                    inputText,
                    voiceId,
                );
                ...
                if (!audioResponse.ok) {
                    throw new Error(
                        `HTTP ${audioResponse.status}: Failed to fetch audio data`,
                    );
                }
    ...
    async fetchTtsGeneration(inputText, voiceId) {
        ...
        if (this.settings.server_version === "v2") {
            ...
            if (this.settings.rvc_narrator_voice !== "Disabled") {
                requestBody.append(
                    ...
                    this.settings.rvc_narrator_voice,
                );
                requestBody.append(
                    "rvcnarrator_pitch",
                    this.settings.rvc_narrator_pitch || "0",
                );
            }
        }
        ...
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                console.error(
                    "fetchTtsGeneration Error Response Text:",
                    errorText,
                );
                throw new Error(`HTTP ${response.status}: ${errorText}`);
            }
