...
async function migrateSystemPromptFromInstructMode() {
    if ("system_prompt" in power_user.instruct) {
        ...
        const existingPromptName = system_prompts.find(
            (x) => x.content === prompt,
        )?.name;
        ...
        if (existingPromptName) {
            ...
            const data = {
                name: `[Migrated] ${power_user.instruct.preset}`,
                content: prompt,
            };
...
export async function checkForSystemPromptInInstructTemplate(name, template) {
    ...
    if ("system_prompt" in template && template.system_prompt) {
        const existingName = system_prompts.find(
            (x) => x.content === template.system_prompt,
        )?.name;
...
function toggleSystemPromptDisabledControls() {
    $enabled
        .parent()
        .find("i")
        .toggleClass("toggleEnabled", !!power_user.sysprompt.enabled);
...
function selectSystemPromptCallback(args, name) {
    ...
    let foundName = systemPromptNames.find(
        ...
        SlashCommand.fromProps({
            ...
            namedArgumentList: [
                ...
                SlashCommandNamedArgument.fromProps({
                    name: "forceGet",
                    description:
                        "Force getting a name even if system prompt is disabled",
                    typeList: [ARGUMENT_TYPE.BOOLEAN],
                    defaultValue: "false",
                    enumList: commonEnumProviders.boolean("trueFalse")(),
                }),
