...
const setMainPromptTokenCounterDebounced = debounce(
    async (value) =>
        $("#extension_floating_prompt_token_counter").text(
            await getTokenCountAsync(value),
        ),
    debounce_timeout.relaxed,
);
const setCharaPromptTokenCounterDebounced = debounce(
    async (value) =>
        $("#extension_floating_chara_token_counter").text(
            await getTokenCountAsync(value),
        ),
    debounce_timeout.relaxed,
);
...
function loadSettings() {
    ...
    chat_metadata[metadata_keys.role] =
        chat_metadata[metadata_keys.role] ??
        extension_settings.note.defaultRole ??
        DEFAULT_ROLE;
...
function onANMenuItemClick() {
    if (!selected_group && this_chid === undefined) {
        toastr.warning(
            t`Select a character before trying to use Author's Note`,
            "",
            { timeOut: 2000 },
        );
        return;
...
export function initAuthorsNote() {
    ...
    $("#extension_floating_allow_wi_scan").on(
        "input",
        onAllowWIScanCheckboxChanged,
    );
    ...
    MacrosParser.registerMacro(
        ...
        () =>
            this_chid !== undefined
                ? (extension_settings.note.chara.find(
                      (e) => e.name === getCharaFilename(),
                  )?.prompt ?? "")
                : "",
