import { main_api, saveSettingsDebounced, callPopup } from "../script.js";
...
async function showSamplerSelectPopup() {
    ...
    html.attr("id", "sampler_view_list").addClass(
        "flex-container flexFlowColumn",
    );
    html.append(await renderTemplateAsync("samplerSelector"));

    const listContainer = $(
        '<div id="apiSamplersList" class="flex-container flexNoGap"></div>',
    );
        ...
        .on("change", async function () {
            ...
            if ($(this).prop("checked") === false) {
                ...
                if (previousState === "shown") {
                    ...
                    power_user?.selectSamplers?.forceShown.splice(
                        power_user?.selectSamplers?.forceShown.indexOf(
                            samplerName,
                        ),
                        1,
                    );
                ...
                if (previousState === "hidden") {
                    ...
                    relatedDOMElement.removeData("selectsampler");
                    $(this).parent().find(".sampler_name").removeAttr("style");
                    power_user?.selectSamplers?.forceHidden.splice(
                        power_user?.selectSamplers?.forceHidden.indexOf(
                            samplerName,
                        ),
                        1,
                    );
                    ...
                    $(this)
                        .parent()
                        .find(".sampler_name")
                        .attr("style", forcedOnColoring);
                    console.log(power_user.selectSamplers.forceShown);
                }
            }
            await saveSettingsDebounced();

            const shouldDisplay = $(this).prop("checked")
                ? targetDisplayType
                : "none";
            ...
            console.log(
                samplerName,
                relatedDOMElement.data("selectsampler"),
                shouldDisplay,
            );
        });
}
...
async function listSamplers(main_api, arrayOnly = false) {
    ...
    if (main_api === "textgenerationwebui") {
        ...
        availableSamplers = availableSamplers.filter(
            (sampler) => !valuesToRemove.has(sampler),
        );
    ...
    const samplersListHTML = availableSamplers.reduce((html, sampler) => {
        ...
        const isInDefaultState = () => {
            if (isVisibleInDOM && isInForceShownArray) {
                return false;
            } else if (!isVisibleInDOM && isInForceHiddenArray) {
                return false;
            } else {
                return true;
            }
        ...
        const shouldBeChecked = () => {
            if (isInForceHiddenArray) {
                ...
                return isVisibleInDOM;
            }
        };
        console.log(
            sampler,
            targetDOMelement.prop("id"),
            isInDefaultState(),
            isInForceShownArray,
            isInForceHiddenArray,
            shouldBeChecked(),
        );
