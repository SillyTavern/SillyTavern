...
import { addEphemeralStoppingString, chat_styles, context_presets, flushEphemeralStoppingStrings, performFuzzySearch, playMessageSound, power_user } from './power-user.js';
...
export function initDefaultSlashCommands() {
    ...
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
        ...
        <div>
            ${t`Example: <pre><code>/genstream profile=my-profile-id reasoning=true Summarize the following text</code></pre>`}
        </div>
    `,
    }));
    SlashCommandParser.addCommandObject(SlashCommand.fromProps({
...
/**
 * Callback for the /genstream command
...
async function generateStreamCallback(args, value) {
    if (!value) {
        console.warn('WARN: No argument provided for /genstream command');
        return '';
    }
    ...
    if (context.extensionSettings.disabledExtensions.includes('connection-manager')) {
        toastr.error(t`Connection Manager is required for /genstream. Use /gen or /genraw instead.`);
        return '';
    }
