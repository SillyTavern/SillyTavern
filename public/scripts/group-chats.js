 ...
 * @param {ChatMessage[]} [chatData] Optionally save the specified chat.
 * @returns {Promise<void>} A promise that resolves when the group chat has been saved.
 */
async function saveGroupChat(groupId, shouldSaveGroup, force = false, chatData = chat) {
    const group = groups.find(x => x.id == groupId);
    const chat_id = group.chat_id;
    ...
    const response = await fetch('/api/chats/group/save', {
        method: 'POST',
        headers: getRequestHeaders(),
        body: JSON.stringify({ id: chat_id, chat: [chatHeader, ...chatData], force: force }),
    });
