...
async function updatePlugins() {
    ...
    console.log(
        `Found ${color.cyan(directories.length)} directories in ./plugins`,
    );
    ...
    for (const directory of directories) {
        try {
            ...
            if (!isRepo) {
                console.log(
                    `Directory ${color.yellow(directory)} is not a Git repository`,
                );
                continue;
            }
            ...
            if (log.total === 0) {
                console.log(
                    `Plugin ${color.blue(directory)} is already up to date`,
                );
                continue;
            }
...
async function installPlugin(pluginName) {
    try {
        const pluginPath = path.join(
            pluginsPath,
            path.basename(pluginName, ".git"),
        );

        if (fs.existsSync(pluginPath)) {
            return console.log(
                color.yellow(`Directory already exists at ${pluginPath}`),
            );
        }

        await git().clone(pluginName, pluginPath, { "--depth": 1 });
        console.log(
            `Plugin ${color.green(pluginName)} installed to ${color.cyan(pluginPath)}`,
        );
    } catch (error) {
        console.error(
            color.red(`Failed to install plugin ${pluginName}`),
            error,
        );
    }
}
