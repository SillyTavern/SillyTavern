...
- Creator upload flow for draft assets, bounded metadata/tags, payload shape validation, file/pasted JSON type auto-detect, and submit-for-review.
...
- User report action with bounded reason/details body, admin report queue with reported dates, bounded resolution notes, and resolve workflow for marketplace moderation.
...
- `npm run marketplace:export:api` generates a Markdown reference from the current market, wallet, and public health routes, including key permission/privacy, upload validation, and report length notes, with tests locking the full MVP route set; pass `--out` to write it to a file.
