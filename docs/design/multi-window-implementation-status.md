...
3. Settings-dirty UX (`340bd6e7d`): the ⚠️ prompt is diff-based — the deferred save's payload is normalized (per-window / profile-owned / file-owned fields stripped, see `normalizeSettingsPayload`) and compared against the last-persisted baseline; only a real global change prompts, reverting clears it. `onSettingsDirty` fans the deferred-save signal out to the persona diff-sync and profile dirty check, so entity edits persist in ~1s (intervals remain as backstops).
4. **Remaining niggles:** lease records (and their revisions) are dropped when the last holder releases — fine today, but revision-based staleness for transiently-leased entities (characters etc.) would need persistent revisions. Persona *selection* is per-window as of `8f98c58e7` (`mw_user_avatar` in sessionStorage; the working-copy `persona_description*` fields are stripped from the dirty check — only the persona Global Settings prompt). Group-chat binding stamping is untested (shares the char-chat code path via the global `chat_metadata`). The bind/refresh toasts race their expiry in automated checks — human-visible, tool-checks must sample promptly.
