...
export function redirectDeprecatedEndpoints(app) {
    ...
    function redirect(src, destination) {
        app.use(src, (req, res) => {
            console.warn(
                `API endpoint ${src} is deprecated; use ${destination} instead`,
            );
...
export class ServerStartup {
    ...
    #verifySslOptions() {
        ...
        if (!this.cliArgs.keyPath) {
            this.#fatal(
                "Error: SSL key path is required when using HTTPS. Check your config",
            );
        }
    ...
    #handleServerListenFail({ v6Failed, v4Failed, useIPv6, useIPv4 }) {
        if (v6Failed && !useIPv4) {
            this.#fatal(
                "Error: Failed to start server on IPv6 and IPv4 disabled",
            );
        }

        if (v4Failed && !useIPv6) {
            this.#fatal(
                "Error: Failed to start server on IPv4 and IPv6 disabled",
            );
        }
    ...
    async start() {
        ...
        if (
            this.cliArgs.enableIPv6 === "auto" ||
            this.cliArgs.enableIPv4 === "auto"
        ) {
            const ipQuery = await getHasIP();
            let hasIPv6 = false,
                hasIPv4 = false;

            hasIPv6 = this.cliArgs.listen
                ? ipQuery.hasIPv6Any
                : ipQuery.hasIPv6Local;
            ...
            hasIPv4 = this.cliArgs.listen
                ? ipQuery.hasIPv4Any
                : ipQuery.hasIPv4Local;
        ...
        const [v6Failed, v4Failed] = await this.#startHTTPorHTTPS(
            useIPv6,
            useIPv4,
        );
