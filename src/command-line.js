    ...
    parse(args) {
        const cliArguments = yargs(hideBin(args))
            ...
            .option("autorunHostname", {
                ...
                default: null,
                describe:
                    "Sets the autorun hostname, probably best left on 'auto'.\nUse values like 'localhost', 'st.example.com'",
            })
            ...
            .option("requestProxyBypass", {
                type: "array",
                describe:
                    "Request proxy bypass list (space separated list of hosts)",
            })
            .parseSync();
        ...
        const result = {
            ...
            dataRoot:
                cliArguments.dataRoot ??
                getConfigValue("dataRoot", this.default.dataRoot),
            port:
                cliArguments.port ??
                getConfigValue("port", this.default.port, "number"),
            listen:
                cliArguments.listen ??
                getConfigValue("listen", this.default.listen, "boolean"),
            listenAddressIPv6:
                cliArguments.listenAddressIPv6 ??
                getConfigValue(
                    "listenAddress.ipv6",
                    this.default.listenAddressIPv6,
                ),
            listenAddressIPv4:
                cliArguments.listenAddressIPv4 ??
                getConfigValue(
                    "listenAddress.ipv4",
                    this.default.listenAddressIPv4,
                ),
            enableIPv4:
                stringToBool(cliArguments.enableIPv4) ??
                stringToBool(
                    getConfigValue("protocol.ipv4", this.default.enableIPv4),
                ) ??
                this.default.enableIPv4,
            enableIPv6:
            ...
            dnsPreferIPv6:
                ...
                getConfigValue(
                    "dnsPreferIPv6",
                    this.default.dnsPreferIPv6,
                    "boolean",
                ),
            autorun:
                cliArguments.autorun ??
                getConfigValue("autorun", this.default.autorun, "boolean"),
            autorunHostname:
                cliArguments.autorunHostname ??
                getConfigValue("autorunHostname", this.default.autorunHostname),
            autorunPortOverride:
                cliArguments.autorunPortOverride ??
                getConfigValue(
                    "autorunPortOverride",
                    this.default.autorunPortOverride,
                    "number",
                ),
            enableCorsProxy:
            ...
            disableCsrf:
                ...
                getConfigValue(
                    ...
                    this.default.disableCsrf,
                    "boolean",
                ),
            ssl:
                cliArguments.ssl ??
                getConfigValue("ssl.enabled", this.default.ssl, "boolean"),
            certPath:
                cliArguments.certPath ??
                getConfigValue("ssl.certPath", this.default.certPath),
            keyPath:
                cliArguments.keyPath ??
                getConfigValue("ssl.keyPath", this.default.keyPath),
            whitelistMode:
                cliArguments.whitelist ??
                getConfigValue(
                    "whitelistMode",
                    this.default.whitelistMode,
                    "boolean",
                ),
            avoidLocalhost:
            ...
            requestProxyEnabled:
                ...
                getConfigValue(
                    ...
                    this.default.requestProxyEnabled,
                    "boolean",
                ),
            requestProxyUrl:
                ...
                getConfigValue(
                    "requestProxy.url",
                    this.default.requestProxyUrl,
                ),
            requestProxyBypass:
                cliArguments.requestProxyBypass ??
                getConfigValue(
                    "requestProxy.bypass",
                    this.default.requestProxyBypass,
                ),
            getIPv4ListenUrl: function () {
                const isValid = ipRegex
                    .v4({ exact: true })
                    .test(this.listenAddressIPv4);
                return new URL(
                    (this.ssl ? "https://" : "http://") +
                        (this.listen
                            ? isValid
            ...
            getIPv6ListenUrl: function () {
                const isValid = ipRegex
                    .v6({ exact: true })
                    .test(this.listenAddressIPv6);
                return new URL(
                    (this.ssl ? "https://" : "http://") +
                        (this.listen
                            ? isValid
                                ? this.listenAddressIPv6
                                : "[::]"
                            : "[::1]") +
                        (":" + this.port),
                );
            ...
            getAutorunHostname: async function ({ useIPv6, useIPv4 }) {
                if (this.autorunHostname === "auto") {
                    let localhostResolve = await canResolve(
                        "localhost",
                        useIPv6,
                        useIPv4,
                    );
                    ...
                    if (useIPv6 && useIPv4) {
                        return this.avoidLocalhost || !localhostResolve
                            ? "[::1]"
                            : "localhost";
                    }
            ...
            getAutorunUrl: function (hostname) {
                const autorunPort =
                    this.autorunPortOverride >= 0
                        ? this.autorunPortOverride
                        : this.port;
                return new URL(
                    (this.ssl ? "https://" : "http://") +
                        hostname +
