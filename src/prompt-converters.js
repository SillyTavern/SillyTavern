...
const PROMPT_PLACEHOLDER = getConfigValue(
    "promptPlaceholder",
    "Let's get started.",
);
...
export function convertClaudePrompt(
    ...
    if (messages.length > 0) {
        ...
        if (excludePrefixes) {
            messages
                .slice(0, -1)
                .forEach((message) => (message.role = "system"));
        } else {
...
export function convertClaudeMessages(
        ...
        for (i = 0; i < messages.length; i++) {
            ...
            if (names.userName && messages[i].name === "example_user") {
                if (!messages[i].content.startsWith(`${names.userName}: `)) {
                    messages[i].content =
                        `${names.userName}: ${messages[i].content}`;
                }
            }
            if (names.charName && messages[i].name === "example_assistant") {
                if (
                    ...
                    messages[i].content =
                        `${names.charName}: ${messages[i].content}`;
                }
            }
    ...
    for (let i = 0; i < messages.length; i++) {
        if (
            ...
            messages[i].content.some((c) => c.type === "image")
        ) {
            ...
            messages[j].content.push(
                ...messages[i].content.filter((c) => c.type === "image"),
            );
            messages[i].content = messages[i].content.filter(
                (c) => c.type !== "image",
            );
        }
    }
    ...
    messages.forEach((message) => {
        if (
            ...
            mergedMessages[mergedMessages.length - 1].content.push(
                ...message.content,
            );
        } else {
    ...
    messages.forEach((msg, index) => {
        ...
        if (msg.name) {
            ...
            if (msg.role == "system" && msg.name == "example_user") {
                if (
                    names.userName &&
                    !msg.content.startsWith(`${names.userName}: `)
                ) {
            ...
            if (
                msg.role !== "system" &&
                !msg.content.startsWith(`${msg.name}: `)
            ) {
        ...
        if (message.name) {
            message.content.forEach((part) => {
                ...
                if (message.name === "example_user") {
                    if (
                        names.userName &&
                        !part.text.startsWith(`${names.userName}: `)
                    ) {
...
export function convertMistralMessages(messages, names) {
    ...
    const prefixEnabled = getConfigValue(
        "mistral.enablePrefix",
        false,
        "boolean",
    );
...
export function convertXAIMessages(messages, names) {
    ...
    messages.forEach((msg) => {
        ...
        const matchingRule = needsCharNamePrefix.find(
            (rule) =>
                msg.role === rule.role &&
                (!rule.name || msg.name === rule.name) &&
                rule.condition,
        );

        if (matchingRule) {
            const prefix =
                msg.role === "system" && msg.name === "example_user"
                    ? names.userName
                    : names.charName;
...
export function mergeMessages(
    messages,
    names,
    { strict = false, placeholders = false, single = false } = {},
) {
    let mergedMessages = [];
    ...
    if (contentTokens.size > 0) {
        mergedMessages.forEach((message) => {
            const hasValidToken = Array.from(contentTokens.keys()).some(
                (token) => message.content.includes(token),
            );
                ...
                splitContent.forEach((content) => {
                    if (contentTokens.has(content)) {
                        ...
                        if (
                            ...
                            mergedContent[mergedContent.length - 1].type ===
                                "text"
                        ) {
        ...
        if (mergedMessages.length && placeholders) {
            if (
                ...
                (mergedMessages.length === 1 ||
                    mergedMessages[1].role !== "user")
            ) {
                mergedMessages.splice(1, 0, {
                    role: "user",
                    content: PROMPT_PLACEHOLDER,
                });
            } else if (
                mergedMessages[0].role !== "system" &&
                mergedMessages[0].role !== "user"
            ) {
                mergedMessages.unshift({
                    role: "user",
                    content: PROMPT_PLACEHOLDER,
