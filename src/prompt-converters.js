...
/**
 * Adds cache_control to the system prompt for OpenRouter Gemini requests.
 *
 * @param {object[]} messages Array of messages
 */
export function cachingSystemPromptForOpenRouterGemini(messages) {
    if (!Array.isArray(messages) || messages.length === 0) {
        return;
    }
    ...
    if (Array.isArray(systemMessage.content)) {
        ...
        for (let i = systemMessage.content.length - 1; i >= 0; i--) {
            if (systemMessage.content[i]?.type === 'text') {
                systemMessage.content[i].cache_control = { type: 'ephemeral' };
                return;
            }
        }
    } else if (typeof systemMessage.content === 'string') {
