...
router.post("/disable", requireAdminMiddleware, async (request, response) => {
    try {
        ...
        if (request.body.handle === request.user.profile.handle) {
            ...
            return response
                .status(400)
                .json({ error: "Cannot disable yourself" });
        }
...
router.post("/create", requireAdminMiddleware, async (request, response) => {
    try {
        ...
        const handle = lodash.kebabCase(
            String(request.body.handle).toLowerCase().trim(),
        );
        ...
        if (handles.some((x) => x === handle)) {
            console.warn(
                "Create user failed: User with that handle already exists",
            );
        ...
        const password = request.body.password
            ? getPasswordHash(request.body.password, salt)
            : "";
...
router.post("/delete", requireAdminMiddleware, async (request, response) => {
    try {
        ...
        if (request.body.purge) {
            ...
            await fsPromises.rm(directories.root, {
                recursive: true,
                force: true,
            });
        }
...
router.post("/slugify", requireAdminMiddleware, async (request, response) => {
    try {
        ...
        const text = lodash.kebabCase(
            String(request.body.text).toLowerCase().trim(),
        );
