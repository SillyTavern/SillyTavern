...
async function getPathToTokenizer(model, fallbackModel) {
    ...
    try {
        ...
        if (!IS_DOWNLOAD_ALLOWED) {
            throw new Error(
                "Downloading tokenizers is disabled, the model is not cached",
            );
        }
...
class SentencePieceTokenizer {
    ...
    async get() {
        ...
        try {
            const pathToModel = await getPathToTokenizer(
                this.#model,
                this.#fallbackModel,
            );
            ...
            console.info(
                "Instantiated the tokenizer for",
                path.parse(pathToModel).name,
            );
            return this.#instance;
        } catch (error) {
            console.error(
                "Sentencepiece tokenizer failed to load: " + this.#model,
                error,
            );
            return null;
        }
...
class WebTokenizer {
    ...
    async get() {
        ...
        try {
            ...
            console.error(
                "Web tokenizer failed to load: " + this.#model,
                error,
            );
            return null;
        }
...
const spp_nerd_v2 = new SentencePieceTokenizer(
    "src/tokenizers/nerdstash_v2.model",
);
...
const commandRTokenizer = new WebTokenizer(
    "https://github.com/SillyTavern/SillyTavern-Tokenizers/raw/main/command-r.json",
    "src/tokenizers/llama3.json",
);
const commandATokenizer = new WebTokenizer(
    "https://github.com/SillyTavern/SillyTavern-Tokenizers/raw/main/command-a.json",
    "src/tokenizers/llama3.json",
);
const qwen2Tokenizer = new WebTokenizer(
    "https://github.com/SillyTavern/SillyTavern-Tokenizers/raw/main/qwen2.json",
    "src/tokenizers/llama3.json",
);
const nemoTokenizer = new WebTokenizer(
    "https://github.com/SillyTavern/SillyTavern-Tokenizers/raw/main/nemo.json",
    "src/tokenizers/llama3.json",
);
const deepseekTokenizer = new WebTokenizer(
    "https://github.com/SillyTavern/SillyTavern-Tokenizers/raw/main/deepseek.json",
    "src/tokenizers/llama3.json",
);
...
export function countWebTokenizerTokens(tokenizer, messages) {
    ...
    const convertedPrompt = convertClaudePrompt(
        messages,
        false,
        "",
        false,
...
function createSentencepieceEncodingHandler(tokenizer) {
    ...
    return async function (request, response) {
        try {
            ...
            const instance = await tokenizer?.get();
            const { ids, count } = await countSentencepieceTokens(
                tokenizer,
                text,
            );
...
router.post(
    "/nerdstash_v2/encode",
    createSentencepieceEncodingHandler(spp_nerd_v2),
);
...
router.post(
    "/claude/encode",
    createWebTokenizerEncodingHandler(claude_tokenizer),
);
router.post(
    "/llama3/encode",
    createWebTokenizerEncodingHandler(llama3_tokenizer),
);
router.post("/qwen2/encode", createWebTokenizerEncodingHandler(qwen2Tokenizer));
router.post(
    "/command-r/encode",
    createWebTokenizerEncodingHandler(commandRTokenizer),
);
router.post(
    "/command-a/encode",
    createWebTokenizerEncodingHandler(commandATokenizer),
);
router.post("/nemo/encode", createWebTokenizerEncodingHandler(nemoTokenizer));
router.post(
    "/deepseek/encode",
    createWebTokenizerEncodingHandler(deepseekTokenizer),
);
...
router.post(
    "/nerdstash_v2/decode",
    createSentencepieceDecodingHandler(spp_nerd_v2),
);
...
router.post(
    "/claude/decode",
    createWebTokenizerDecodingHandler(claude_tokenizer),
);
router.post(
    "/llama3/decode",
    createWebTokenizerDecodingHandler(llama3_tokenizer),
);
router.post("/qwen2/decode", createWebTokenizerDecodingHandler(qwen2Tokenizer));
router.post(
    "/command-r/decode",
    createWebTokenizerDecodingHandler(commandRTokenizer),
);
router.post(
    "/command-a/decode",
    createWebTokenizerDecodingHandler(commandATokenizer),
);
router.post("/nemo/decode", createWebTokenizerDecodingHandler(nemoTokenizer));
router.post(
    "/deepseek/decode",
    createWebTokenizerDecodingHandler(deepseekTokenizer),
);
...
router.post("/openai/count", async function (req, res) {
    try {
        ...
        const tokensPerName = queryModel.includes("gpt-3.5-turbo-0301")
            ? -1
            : 1;
        const tokensPerMessage = queryModel.includes("gpt-3.5-turbo-0301")
            ? 4
            : 3;
        ...
        console.error(
            "An error counting tokens, using fallback estimation method",
            error,
        );
...
router.post("/remote/kobold/count", async function (request, response) {
    ...
    try {
        ...
        if (!result.ok) {
            console.warn(
                `API returned error: ${result.status} ${result.statusText}`,
            );
