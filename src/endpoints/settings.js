...
const ENABLE_EXTENSIONS = !!getConfigValue(
    "extensions.enabled",
    true,
    "boolean",
);
...
const ENABLE_ACCOUNTS = !!getConfigValue(
    "enableUserAccounts",
    false,
    "boolean",
);
...
function readAndParseFromDirectory(directoryPath, fileExtension = ".json") {
    ...
    files.forEach((item) => {
        try {
            ...
            parsedFiles.push(
                fileExtension == ".json" ? JSON.parse(file) : file,
            );
        } catch {
            // skip
        }
...
function backupUserSettings(handle, preventDuplicates) {
    ...
    const backupFile = path.join(
        ...
        `${getFilePrefix(handle)}${generateTimestamp()}.json`,
    );
...
router.post("/get", (request, response) => {
    ...
    const movingUIPresets = readAndParseFromDirectory(
    ...
    const instruct = readAndParseFromDirectory(
        request.user.directories.instruct,
    );
    const context = readAndParseFromDirectory(request.user.directories.context);
    const sysprompt = readAndParseFromDirectory(
        request.user.directories.sysprompt,
    );
    const reasoning = readAndParseFromDirectory(
        request.user.directories.reasoning,
    );
...
router.post("/get-snapshots", async (request, response) => {
    try {
        ...
        const userSnapshots = snapshots.filter((x) =>
            x.startsWith(userFilesPattern),
        );

        const result = userSnapshots.map((x) => {
            const stat = fs.statSync(
                path.join(request.user.directories.backups, x),
            );
...
router.post(
    ...
    async (request, response) => {
        try {
            ...
            if (
                !request.body.name ||
                !request.body.name.startsWith(userFilesPattern)
            ) {
                return response
                    .status(400)
                    .send({ error: "Invalid snapshot name" });
            }
