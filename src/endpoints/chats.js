...
function backupChat(directory, name, chat) {
    try {
        ...
        const backupFile = path.join(
            directory,
            `chat_${name}_${generateTimestamp()}.jsonl`,
        );
...
function importKoboldLiteChat(_userName, _characterName, data) {
    ...
    const header = {
        ...
        character_name: String(data.savedsettings.chatopponent).split(
            "||$||",
        )[0],
    };
    // Format messages
...
router.post(
    ...
    async function (request, response) {
        try {
            const directoryName = String(request.body.avatar_url).replace(
                ".png",
                "",
            );
            ...
            if (checkIntegrity && !request.body.force) {
                ...
                const isIntact = await checkChatIntegrity(
                    filePath,
                    integritySlug,
                );
                if (!isIntact) {
                    console.error(
                        `Chat integrity check failed for ${filePath}`,
                    );
                    return response.status(400).send({ error: "integrity" });
                }
            }
            writeFileAtomicSync(filePath, jsonlData, "utf8");
            getBackupFunction(request.user.profile.handle)(
        ...
        try {
            ...
            for (const chatFile of chatFiles) {
                ...
                const lastMesDate =
                    ...
                    Math.round(fs.statSync(chatFile.path).mtimeMs);
