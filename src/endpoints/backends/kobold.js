...
router.post("/generate", async function (request, response_generate) {
    ...
    if (request.body.api_server.indexOf("localhost") != -1) {
        request.body.api_server = request.body.api_server.replace(
            "localhost",
            "127.0.0.1",
        );
    }
        ...
        try {
            ...
            switch (error?.status) {
                ...
                case 503: // retry in case of temporary service issue, possibly caused by a queue failure?
                    console.warn(
                        `KoboldAI is busy. Retry attempt ${i + 1} of ${MAX_RETRIES}...`,
                    );
        ...
        await Promise.all([
            ...
            fetch(`${api_server}/v1/info/version`)
                .then((response) => {
                    if (!response.ok)
                        throw new Error(
                            `Kobold API error: ${(response.status, response.statusText)}`,
                        );
...
router.post("/transcribe-audio", async function (request, response) {
    try {
        ...
        const fileBase64 = fs
            .readFileSync(request.file.path)
            .toString("base64");
