...
router.post("/status", async function (request, response) {
    ...
    try {
        ...
        const isPossiblyLmStudio =
            modelsReply.headers.get("x-powered-by") === "Express";
...
router.post("/props", async function (request, response) {
    ...
    try {
        ...
        props["chat_template_hash"] = createHash("sha256")
            .update(props["chat_template"])
            .digest("hex");
...
router.post("/generate", async function (request, response) {
    ...
    try {
        ...
        request.socket.on("close", async function () {
            if (
                request.body.api_type === TEXTGEN_TYPES.KOBOLDCPP &&
        ...
        if (request.body.api_type === TEXTGEN_TYPES.TOGETHERAI) {
            request.body = _.pickBy(request.body, (_, key) =>
                TOGETHERAI_KEYS.includes(key),
            );
        ...
        if (request.body.api_type === TEXTGEN_TYPES.INFERMATICAI) {
            request.body = _.pickBy(request.body, (_, key) =>
                INFERMATICAI_KEYS.includes(key),
            );
            args.body = JSON.stringify(request.body);
        }
        ...
        if (request.body.api_type === TEXTGEN_TYPES.OLLAMA) {
            const keepAlive = Number(
                getConfigValue("ollama.keepAlive", -1, "number"),
            );
            const numBatch = Number(
                getConfigValue("ollama.batchSize", -1, "number"),
            );
            if (numBatch > 0) {
            ...
            args.body = JSON.stringify({
                ...
                raw: true,
                options: _.pickBy(request.body, (_, key) =>
                    OLLAMA_KEYS.includes(key),
                ),
            });
        }

        if (
            request.body.api_type === TEXTGEN_TYPES.OLLAMA &&
            request.body.stream
        ) {
            ...
            if (completionsReply.ok) {
                ...
                const errorBody = {
                    error: true,
                    status: completionsReply.status,
                    response: text,
                };
