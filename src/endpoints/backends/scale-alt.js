...
router.post("/generate", async function (request, response) {
    ...
    try {
        ...
        const result = await fetch(
            "https://dashboard.scale.com/spellbook/api/trpc/v2.variant.run",
            {
                method: "POST",
                headers: {
        ...
        if (!result.ok) {
            ...
            return response
                .status(500)
                .send({ error: { message: result.statusText } });
        }
        ...
        if (!output) {
            ...
            return response
                .sendStatus(500)
                .send({ error: { message: "Empty response" } });
        }
