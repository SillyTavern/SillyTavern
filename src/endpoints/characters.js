...
const useShallowCharacters = !!getConfigValue(
    "performance.lazyLoadCharacters",
    false,
    "boolean",
);
const useDiskCache = !!getConfigValue(
...
class DiskCache {
    ...
    async #syncCacheEntries() {
        try {
            ...
            const directories = [...this.syncQueue].map((entry) =>
                getUserDirectories(entry),
            );
        ...
        try {
            ...
            for (const dir of directoriesList) {
                ...
                for (const file of files.filter(
                    ...
                    validKeys.add(
                        path.parse(cache.getDatumPath(cacheKey)).base,
                    );
                }
            }
...
async function tryReadImage(imgPath, crop) {
    try {
        ...
        let finalWidth = rawImg.bitmap.width,
            finalHeight = rawImg.bitmap.height;
...
const processCharacter = async (item, directories, { shallow }) => {
    try {
        ...
        character["create_date"] =
            jsonObject["create_date"] ||
            humanizedISO8601DateTime(charStat.ctimeMs);
...
function charaFormatData(data, directories) {
    ...
    const getAlternateGreetings = (data) => {
        ...
        if (typeof data.alternate_greetings === "string")
            return [data.alternate_greetings];
        return [];
    };
    ...
    if (data.extensions) {
        try {
            ...
            _.set(
                char,
                "data.extensions",
                deepMerge(char.data.extensions, extensions),
            );
        } catch {
        ...
        const originalEntry = {
            ...
            extensions: {
                ...
                match_persona_description:
                    entry.matchPersonaDescription ?? false,
                ...
                match_character_depth_prompt:
                    entry.matchCharacterDepthPrompt ?? false,
...
async function importFromYaml(uploadPath, context, preservedFileName) {
    ...
    const fileName =
        preservedFileName ||
        getPngName(yamlData.name, context.request.user.directories);
...
async function importFromCharX(uploadPath, { request }, preservedFileName) {
    ...
    if (Array.isArray(assets) && assets.length) {
        for (const asset of assets.filter(
            (x) => x.type === "icon" && typeof x.uri === "string",
        )) {
            const pathNoProtocol = String(
                asset.uri.replace(/^(?:\/\/|[^/]+)*\//, ""),
            );
    ...
    const fileName =
        preservedFileName || getPngName(card.name, request.user.directories);
    const result = await writeCharacterData(
        avatar,
        JSON.stringify(card),
        fileName,
...
async function importFromJson(uploadPath, { request }, preservedFileName) {
    ...
    if (jsonData.spec !== undefined) {
        ...
        if (jsonData.creator_notes) {
            jsonData.creator_notes = jsonData.creator_notes.replace(
        ...
        const pngName =
            preservedFileName ||
            getPngName(jsonData.name, request.user.directories);
        let char = {
            name: jsonData.name,
            description: jsonData.description ?? "",
            creatorcomment:
                jsonData.creatorcomment ?? jsonData.creator_notes ?? "",
        ...
        const pngName =
            preservedFileName ||
            getPngName(jsonData.char_name, request.user.directories);
        let char = {
...
router.post("/create", async function (request, response) {
    try {
        ...
        const char = JSON.stringify(
            charaFormatData(request.body, request.user.directories),
        );
        const internalName =
...
router.post(
    ...
    async function (request, response) {
        ...
        const oldChatsPath = path.join(
            request.user.directories.chats,
            oldInternalName,
        );
        const newChatsPath = path.join(
            request.user.directories.chats,
            newInternalName,
        );
        ...
        try {
            ...
            const oldData = getCharaCardV2(
                JSON.parse(rawOldData),
                request.user.directories,
            );
...
router.post(
    ...
    async function (request, response) {
        ...
        if (!request.body) {
            ...
            return response
                .status(400)
                .send("Error: no response body detected");
        }
    ...
    async function (request, response) {
        try {
            ...
            if (!pngStringData) {
                ...
                return response
                    .status(400)
                    .send("Error: invalid character file.");
            }
...
router.post("/all", async function (request, response) {
    try {
        ...
        const processingPromises = pngFiles.map((file) =>
            processCharacter(file, request.user.directories, {
...
router.post(
    ...
    async function (request, response) {
        try {
            ...
            const data = await processCharacter(
                item,
                request.user.directories,
                { shallow: false },
            );
...
router.post(
    ...
    async function (request, response) {
        try {
            ...
            const characterDirectory = request.body.avatar_url.replace(
                ".png",
                "",
            );
            const chatsDirectory = path.join(
            ...
            if (request.body.simple) {
                return response.send(
                    jsonFiles.map((file) => ({ file_name: file })),
                );
            }
...
function getPreservedName(request) {
    return typeof request.body.preserved_name === "string" &&
        request.body.preserved_name.length > 0
            ...
            switch (request.body.format) {
                ...
                case "json": {
                    try {
                        ...
                        const jsonObject = getCharaCardV2(
                            JSON.parse(json),
                            request.user.directories,
                        );
                        ...
                        return response
                            .type("json")
                            .send(JSON.stringify(jsonObject, null, 4));
                    } catch {
                        return response.sendStatus(400);
                    }
                }
