...
router.post("/google", async (request, response) => {
    try {
        ...
        const { generateRequestUrl, normaliseResponse } =
        ...
        const translateResponse = normaliseResponse(
            JSON.parse(decodeBuffer(buffer)),
        );
...
router.post("/onering", async (request, response) => {
    try {
        ...
        if (!secretUrl && url === ONERING_URL_DEFAULT) {
            console.info(
                "OneRing URL is using default value.",
                ONERING_URL_DEFAULT,
            );
        }
...
router.post("/deeplx", async (request, response) => {
    try {
        ...
        if (!secretUrl && url === DEEPLX_URL_DEFAULT) {
            console.info(
                "DeepLX URL is using default value.",
                DEEPLX_URL_DEFAULT,
            );
        }
