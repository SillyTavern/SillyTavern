...
import {
    ...
    getNomicAIVector,
} from "../vectors/nomicai-vectors.js";
import {
    getOpenAIVector,
    getOpenAIBatchVector,
} from "../vectors/openai-vectors.js";
import {
...
} from "../vectors/extras-vectors.js";
import {
    getMakerSuiteVector,
    getMakerSuiteBatchVector,
} from "../vectors/makersuite-vectors.js";
import {
    getCohereVector,
    getCohereBatchVector,
} from "../vectors/cohere-vectors.js";
import {
    getLlamaCppVector,
    getLlamaCppBatchVector,
} from "../vectors/llamacpp-vectors.js";
import { getVllmVector, getVllmBatchVector } from "../vectors/vllm-vectors.js";
import {
    getOllamaVector,
    getOllamaBatchVector,
} from "../vectors/ollama-vectors.js";
...
async function getBatchVector(
    ...
    for (let batch of batches) {
        switch (source) {
            ...
            case "palm":
                results.push(
                    ...(await getMakerSuiteBatchVector(batch, directories)),
                );
                break;
            case "cohere":
                results.push(
                    ...(await getCohereBatchVector(
                        batch,
                        isQuery,
                        directories,
                        sourceSettings.model,
...
async function multiQueryCollection(
    ...
    for (const result of sortedResults) {
        ...
        groupedResults[result.collectionId].hashes.push(
            Number(result.result.item.metadata.hash),
        );
        groupedResults[result.collectionId].metadata.push(
            result.result.item.metadata,
        );
    }
    ...
    if (error instanceof SyntaxError && !req.query.regenerated) {
        ...
        if (collectionId && source) {
            ...
            if (exists) {
                ...
                console.warn(
                    `Corrupted index detected at ${path}, regenerating...`,
                );
