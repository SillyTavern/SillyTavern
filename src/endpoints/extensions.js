...
router.post("/install", async (request, response) => {
    ...
    try {
        ...
        if (global && !request.user.profile.admin) {
            ...
            return response
                .status(403)
                .send("Forbidden: No permission to install global extensions.");
        }
        ...
        const extensionPath = path.join(
            basePath,
            sanitize(path.basename(url, ".git")),
        );
        ...
        console.info(
            `Extension has been cloned to ${extensionPath} from ${url} at ${branch || "(default)"} branch`,
        );
...
router.post("/update", async (request, response) => {
    ...
    try {
        ...
        if (global && !request.user.profile.admin) {
            ...
            return response
                .status(403)
                .send("Forbidden: No permission to update global extensions.");
        }
...
router.post("/branches", async (request, response) => {
    try {
        ...
        const isShallow =
            (await git.revparse(["--is-shallow-repository"])) === "true";
        if (isShallow) {
    ...
    try {
        ...
        if (String(branch).startsWith("origin/")) {
            ...
            if (branches.all.includes(localBranch)) {
                console.info(
                    `Branch ${localBranch} already exists locally, checking it out`,
                );
        ...
        if (!branches.all.includes(branch)) {
            ...
            return response
                .status(404)
                .send(`Branch ${branch} does not exist locally`);
        }
...
router.post("/move", async (request, response) => {
    try {
        ...
        if (!extensionName || !source || !destination) {
            return response
                .status(400)
                .send("Bad Request. Not all required parameters are provided.");
        }
        ...
        if (!request.user.profile.admin) {
            ...
            return response
                .status(403)
                .send("Forbidden: No permission to move extensions.");
        }
        ...
        if (
            ...
            return response
                .status(404)
                .send("Source directory does not exist.");
        }

        if (fs.existsSync(destinationPath)) {
            console.error(
                `Destination directory already exists at ${destinationPath}`,
            );
            return response
                .status(409)
                .send("Destination directory already exists.");
        }
        ...
        if (source === destination) {
            ...
            return response
                .status(409)
                .send("Source and destination directories are the same.");
        }
        ...
        console.info(
            `Extension has been moved from ${sourcePath} to ${destinationPath}`,
        );
...
router.post("/delete", async (request, response) => {
    ...
    try {
        ...
        if (global && !request.user.profile.admin) {
            ...
            return response
                .status(403)
                .send("Forbidden: No permission to delete global extensions.");
        }
...
router.get("/discover", function (request, response) {
    ...
    const builtInExtensions = fs
        ...
        .filter((f) =>
            fs
                .statSync(path.join(PUBLIC_DIRECTORIES.extensions, f))
                .isDirectory(),
        )
    ...
    const userExtensions = fs
        ...
        .filter((f) =>
            fs
                .statSync(path.join(request.user.directories.extensions, f))
                .isDirectory(),
        )
    ...
    const globalExtensions = fs
        ...
        .filter((f) =>
            fs
                .statSync(path.join(PUBLIC_DIRECTORIES.globalExtensions, f))
                .isDirectory(),
        )
