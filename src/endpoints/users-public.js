...
const PREFER_REAL_IP_HEADER = getConfigValue(
    "rateLimiting.preferRealIpHeader",
    false,
    "boolean",
);
...
router.post("/login", async (request, response) => {
    try {
        ...
        if (
            user.password &&
            user.password !== getPasswordHash(request.body.password, user.salt)
        ) {
...
router.post("/recover-step1", async (request, response) => {
    try {
        ...
        console.log();
        console.log(
            color.blue(`${user.name}, your password recovery code is: `) +
                color.magenta(mfaCode),
        );
        console.log();
