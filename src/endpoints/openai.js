...
router.post("/caption-image", async (request, response) => {
    try {
        ...
        if (
            ...
            !noKeyTypes.includes(request.body.api)
        ) {
            ...
            messages: [
                {
                    ...
                    content: [
                        ...
                        {
                            type: "image_url",
                            image_url: { url: request.body.image },
                        },
                    ],
                },
            ],
        ...
        const captionSystemPrompt = getConfigValue(
            "openai.captionSystemPrompt",
        );
        ...
        if (
            ["koboldcpp", "vllm", "llamacpp", "ooba"].includes(request.body.api)
        ) {
        ...
        if (!result.ok) {
            ...
            console.warn(
                "Multimodal captioning request failed",
                result.statusText,
                text,
            );
