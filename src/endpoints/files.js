...
router.post("/upload", async (request, response) => {
    try {
        ...
        const url = clientRelativePath(
            request.user.directories.root,
            pathToUpload,
        );
        console.info(
            `Uploaded file: ${url} from ${request.user.profile.handle}`,
        );
...
router.post("/delete", async (request, response) => {
    try {
        ...
        console.info(
            `Deleted file: ${request.body.path} from ${request.user.profile.handle}`,
        );
    ...
    try {
        ...
        for (const url of request.body.urls) {
            ...
            if (!pathToVerify.startsWith(request.user.directories.files)) {
                console.warn(
                    `File verification: Invalid path: ${pathToVerify}`,
                );
                continue;
            }
